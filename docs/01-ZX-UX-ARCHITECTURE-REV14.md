...
Revision: 14 - C48 SDK target-native compatibility-canary correction
...
- deferred final-cell wrap, nested mutation, safe cursor service, and the 448-byte kernel
  ...
  matrices; the Revision-14 `CAPS SHIFT + 1` `EDIT` -> `0x1B` tty mapping and its strict
  separation from `CAPS SHIFT + SPACE` BREAK are delta-certified by the mandatory R14.00
