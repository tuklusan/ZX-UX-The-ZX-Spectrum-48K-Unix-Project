...
Revision: 13 - EDIT/ESC input contract and Phase-2/Phase-3 certification-transition rebaseline
...
- H06 pins the read-only SDK at commit `9ca3c6d6b5dd4b6e2351c1800afbd47d1d77e411`
  with `usr/src` tree `f629dcc1d156b83bf08ed171b9273e3cbb621ad1`; recursive discovery
  accounts for every `.c` path/byte/hash: exactly 54 C sources, 53 positive executable
  programs, and the deliberate compile-negative `security/seckern.c` fixture;
- every positive SDK corpus source/build unit is compiled by target-native C48, linked by
  native `ld`, and executed under ZX-UX; no host-only compile/link result satisfies H06;
- the deliberate compile-negative SDK fixture is rejected for its intended C48 safety
  reason and is recorded as an expected negative row rather than silently skipped;
- every executed SDK corpus program retains deterministic debugger/register/RAM/process
  and exit/result proof plus at least one screenshot; screenshots are mandatory
  supplemental artifacts and never a screenshot-only PASS oracle;
- P11.45 evidence records the pinned SDK source commit/tree and a complete 54-row
  source-relative SHA-256/build-unit/result map, and P11.48/P12.26 reject any missing,
  duplicate, changed, skipped, or unaccounted corpus source.
...
- deferred final-cell wrap, nested mutation, safe cursor service, and the 448-byte kernel
  ...
  matrices; the Revision-13 `CAPS SHIFT + 1` `EDIT` -> `0x1B` tty mapping and its strict
  separation from `CAPS SHIFT + SPACE` BREAK are delta-certified by the mandatory R13.00
...
150. H06 SDK `usr/src` corpus admission is a separate P11.45/P11.48/P12.26 compiler
    acceptance obligation; host-only compilation or screenshot-only evidence cannot
    satisfy it.
