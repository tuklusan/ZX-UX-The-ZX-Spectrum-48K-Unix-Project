...
Status: Revision-15 Phase-2/Phase-3 transition implementation and certification plan
Revision: 06
Architecture baseline: `docs/01-ZX-UX-ARCHITECTURE-REV15.md`
Architecture SHA-256: `C80B0626D58D79B133DE90EE64E598421618CE5C503D6F5D34C866E332E4C398`
...
This document is subordinate to Revision 15 for the R15.00 transition and all Phase-3
and later work. It does not amend the architecture. If this document conflicts with
Revision 15 in that epoch, Revision 15 wins and this document must be corrected before
...
that historical epoch; R15.00 is the mandatory one-time transition before P3.01.
...
recorded in its retained evidence. As part of R15.00, the current verifier is advanced
to require `docs/01-ZX-UX-ARCHITECTURE-REV15.md` with SHA-256
`c80b0626d58d79b133de90ee64e598421618ce5c503d6f5d34c866e332e4c398` for R15.00 and every later certification, while the project-local
...
P2.24 proceeds under Revision 12 / Revision 03; after R15.00, work proceeds in
Revision-15 / Revision-06 phase order. A later phase does not begin because its code
looks fun.

Revision 06 has two explicit certification epochs. Historical evidence through P2.24
remains bound to Revision 12 / Revision 03 and to the source commits recorded in those
evidence files. R15.00 is the sole bridge into the Revision-15 / Revision-06 epoch. On
bridge PASS, historical records remain unchanged and are admitted only as prerequisite
evidence. The known Revision-15 EDIT/ESC delta touches already-completed Phase-1
keyboard/console behavior; R15.00 therefore delta-implements and delta-recertifies that
...
against Revision 15 / Revision 06.
...
requires `<STEP-ID>.build.json` and `<STEP-ID>.test.json`. R15.00 follows the same
...
boundaries; R15.00 additionally requires its own `<STEP-ID>.result.json`; Phase 0 also
...
Beginning with R15.00, every new durable build/test/result record additionally contains
`implementation_plan_sha256`, the SHA-256 of canonical
`docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV06.md`. R15.00 computes that value from the
current file and freezes it in `R15.00.result.json`; the plan does not embed its own
digest. Every later Phase-3-through-Phase-12 certification must prove both that the
current REV06 bytes hash to the value frozen by R15.00 and that its own
`implementation_plan_sha256` field equals that value. Any later REV06 byte change
...
R15.00 uses the same anti-self-reference discipline explicitly. First create one clean
**bridge source-candidate commit** containing the complete intended Revision-15/06
kernel/documentation/tooling/test changes but no new `R15.00.*.json` durable records.
...
three frozen R15.00 durable records to `v1/dist/certification/`. The admission commit hash
...
# 3. Fixed Architecture Contracts Used by the Revision-15 Epoch

From R15.00 onward, the implementation must preserve, among the rest of Revision 15,
...
- the final system-tape order is frozen by Revision 15 Section 19.8 and is not a
...
## R15.00 - Revision-12/03 to Revision-15/06 Phase-3 admission bridge

1. **Purpose / REV15 requirement:** Transition only after the complete Phase-2 gate. P2.24 is the last ordinary certification step under Revision 12 / Revision 03; this bridge preserves all E0.01-P2.24 evidence unchanged, implements and delta-recertifies the bounded Revision-15 EDIT/ESC change that touches already-completed Phase-1 keyboard/console code and ABI documentation, proves the patched current tree still satisfies the carried-forward Phase-1/Phase-2 prerequisites, advances the certification tooling to the new architecture/plan epoch, and is mandatory before P3.01 can begin. P3.01 is the first ordinary phase step certified against Revision 15 / Revision 06.
2. **Exact implementation work:** Perform one fail-closed transition transaction. It must (a) prove historical `docs/01-ZX-UX-ARCHITECTURE-REV12.md` and `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV03.md` still match Git blob IDs `56f509189de369038258cca0f61ae0f59ddd4a04` and `173c82c30c3787a0a020833b49e3e486e570840e`, SHA-256 `a90d523f62a95e8cba6af0312b596a2d5f6bc1aa2ef92f39bb391509b7c15e1b` and `067d96de9a8154d7f6e01bd5b96a3f35a9319a6c046f19125aea361182f055f7`, with canonical LF bytes; (b) prove the retained E0.01-P2.24 dependency chain, `v1/dist/certification/P2.24.build.json`, `v1/dist/certification/P2.24.test.json`, and `v1/dist/certification/phase-2.json` aggregate evidence are internally consistent and PASS under that historical identity, without modifying or relabeling them; (c) prove current `docs/01-ZX-UX-ARCHITECTURE-REV15.md` hashes exactly to `c80b0626d58d79b133de90ee64e598421618ce5c503d6f5d34c866e332e4c398`, prove this Revision-06 plan names that exact architecture identity, compute the current SHA-256 of canonical `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV06.md`, and freeze that computed plan digest in every R15.00 durable record and especially `R15.00.result.json` without embedding a self-referential plan hash in this document; (d) generate complete machine-readable REV12->REV15 and REV03->REV06 delta reports against those exact historical bytes and prove every historical E0.01-P2.24 step body in REV06 is byte-identical to REV03; after normalizing only mechanical authority references from REV12/Revision 12 to REV15/Revision 15, prove the complete existing-step plan delta is substantive only in `P3.01`, `P3.03`, `P3.21`, `P9.05`, `P9.15`, `P9.23`, `P9.24`, `P11.24`, and `P11.48`, plus the new `R15.00` step and transition/traceability tables required by those changes; any other substantive existing-step or unrelated plan-semantic delta fails the bridge; (e) classify the only allowlisted pre-P3 **architecture/target-behavior** impact of Revision 15 as the EDIT/ESC delta to behavior historically owned by P1.15, P1.31, P1.34 and the P1.41 aggregate, plus the corresponding `v1/docs/abi.md` public-contract mirror; any other pre-P3 architecture, target-behavior, ABI, oracle, or evidence-strength delta fails the bridge and requires replay from the earliest affected historical step; (f) separately classify the only allowlisted **transition-tooling** changes as `tools/scripts/verify-environment.py`, `v1/tools-host/test-driver/run.py`, `v1/tools-host/test-driver/driver_core.py`, `v1/tools-host/test-driver/evidence.py`, new `v1/tools-host/test-driver/revision15_bridge.py`, `v1/docs/test-plan.md`, `v1/dist/certification/README.md`, transition-only fixtures, and R15.00 evidence; prove those diffs are limited to the REV15 architecture identity switch, REV06 plan-identity recording/enforcement, R15.00 registration/execution, prospective step-ID/evidence validation through P12, non-admitting regression mode, and tests for those changes—any unrelated tooling behavior change fails the bridge; (g) update the current kernel input path so IM2 recognizes only exact `CAPS SHIFT + SPACE` BREAK, ordinary task-context tty decoding maps only exact `CAPS SHIFT + 1` `EDIT` to target byte `0x1B`, tty32 and tty64 agree, no BASIC EDIT/extended-mode token leaks, `CAPS SHIFT + SYMBOL SHIFT` is not an ESC alias, `SYS_CON_GETKEY` returns H=0,L=0x1B for EDIT to the tty owner, BREAK remains cancellation and never returns 0x1B, and `v1/docs/abi.md` states those exact public input semantics without assigning output-control meaning to `0x1B`; (h) delta-recertify those semantics with raw-matrix, owner/non-owner, blocked-input wake, cursor-safe-service, IY-restoration, ABI-documentation consistency, and ISR call-graph assertions equivalent in strength to the affected historical Phase-1 gates; (i) after applying the complete bridge source/documentation/tooling patch, run a **non-admitting current-tree regression** of the full functional/static/format/size/emulator/negative/rollback assertion sets owned by the P1.41 and P2.24 aggregate build/test suites against the patched tree, excluding only their historical admission-identity assertions for historical architecture/plan/source-commit/evidence identities because those immutable identities are verified separately under (a)-(b); no functional, format, size, emulator, negative, rollback, or behavioral oracle may be skipped; record all regression assertions/hashes inside R15.00 scratch/test evidence rather than overwriting, regenerating, or relabeling any historical P1/P2 evidence; both aggregate assertion sets must remain PASS on the patched current tree before Phase 3 may be admitted; (j) advance `tools/scripts/verify-environment.py` and `driver_core.py` from the REV12 architecture path/hash to the exact REV15 architecture identity for R15.00 and later certification source state, while leaving historical E0/P0/P1/P2 evidence untouched; extend `evidence.py` prospectively so its step-ID grammar accepts `R15.00` and Phase IDs through P12, and so `implementation_plan_sha256` is required and validated for R15.00 and every new P3-P12 durable record while retained historical E0-P2 records remain valid without that new field; later-record validation must compare the field and the current REV06 file hash to the digest frozen in `R15.00.result.json`; (k) because the bridge changes certification tooling, run the current REV15 environment verifier and the E0.01 manifest-digest negative behavior, rerun the E0.03 root-relocation/argv-list/hard-timeout runner self-tests, and rerun the E0.04 evidence-validator positive/negative protocol against the patched tooling, including exact consistency of `v1/docs/test-plan.md` and `v1/dist/certification/README.md` with the prospective `implementation_plan_sha256`/R15.00 schema, rejection of an invalid `R15.00` step ID form, a missing/wrong prospective plan hash, and a later record whose plan hash/current REV06 bytes differ from the bridge result; record these only as R15.00 regression subresults, never as replacement historical E0 evidence; (l) prove prospective ownership continues through P3.03/P3.21, P9.05/P9.15/P9.23-P9.24, P11.24/P11.48, the architecture coverage/invariant/mandatory-test ledgers, and P12.26 release replay; and (m) use the Section-2 two-stage admission transaction: certify one clean bridge source-candidate commit, then admit only the frozen R15.00 records in a separate evidence-only commit, with no record attempting to embed the hash of the commit that contains itself.
3. **Files/artifacts created or modified:** `v1/src/kernel/interrupt.asm`; `v1/src/kernel/keyboard.asm`; `v1/src/kernel/console.asm`; `v1/src/kernel/syscall.asm`; `v1/docs/abi.md`; `tools/scripts/verify-environment.py`; `v1/tools-host/test-driver/run.py`; `v1/tools-host/test-driver/driver_core.py`; `v1/tools-host/test-driver/evidence.py`; new `v1/tools-host/test-driver/revision15_bridge.py`; `v1/docs/test-plan.md`; `v1/dist/certification/README.md`; transition keyboard/console/tooling/current-tree-regression fixtures and R15.00 evidence. Historical REV12, REV03, E0/P0/P1/P2 evidence, and Phase-0/1/2 aggregate manifests are read-only inputs; historical-only `foundation_rr.py` identity constants are not rewritten merely to make old E0 records look current.
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step R15.00`.
5. **Host-side static checks:** Exact old/new document identities; historical evidence/source-commit/prerequisite consistency through P2.24; byte-identical REV03 step bodies through P2.24; complete normalized REV03->REV06 delta restricted to the nine named substantive prospective step bodies plus R15.00/required transition tables; separate complete allowlists for the target/ABI delta and transition-tooling delta; exact `v1/docs/abi.md` mirror of the new `SYS_CON_GETKEY`/BREAK semantics; exact REV15 architecture source-state identity in the current verifier/driver; prospective REV06 digest recording/enforcement without self-reference; exact `v1/docs/test-plan.md` and `v1/dist/certification/README.md` agreement with the prospective evidence schema; evidence validator accepts R15.00/P0-P12 syntax as appropriate and preserves historical records while requiring the plan digest for R15.00/P3-P12; no mutation/relabeling of historical evidence; no unowned pre-P3 REV15 semantic change; no unrelated verifier/runner/evidence behavior change; no ISR-to-ROM keyboard/decoder edge; current-tree P1.41/P2.24 regression results are recorded only under R15.00.
6. **Emulator test artifact:** Generate/run a deterministic SNA transition fixture exercising raw keyboard rows, IM2 BREAK sampling, task-context EDIT decode, tty32/tty64 parity, `SYS_CON_GETKEY`, blocked-input wake/cursor service, tty ownership, and IY preservation; additionally execute the complete P1.41 and P2.24 aggregate emulator suites in R15.00 non-admitting regression mode against the patched current tree.
...
8. **Expected PASS result:** The transition ends exactly with `ZX-UX REV15 PHASE-3 BASELINE BRIDGE PASS`; the result records immutable REV12/REV03 identities, exact REV15 architecture identity, the computed canonical REV06 SHA-256, the certified historical P2.24 source commit and Phase-2 PASS, the new bridge source commit/candidate identity, the four affected historical owner IDs `P1.15,P1.31,P1.34,P1.41`, the exact target/ABI delta, the exact nine-step prospective plan-delta allowlist, the separate transition-tooling delta, `v1/docs/abi.md` consistency, evidence-protocol documentation consistency, current-verifier/E0.01/E0.03/E0.04 tooling-regression results, full patched-tree P1.41/P2.24 non-admitting regression results, transition test hashes, the exact clean bridge source-candidate commit certified by all three records, and an explicit statement that historical evidence remains historical while R15.00 alone certifies the bounded delta, current-tree prerequisite regression, and the REV15/REV06 identity pair before Phase 3.
9. **Negative/failure test:** Independently mutate one historical document byte, one retained evidence identity, remove/red P2.24 or the Phase-2 aggregate, change the current REV15 digest, change one REV06 byte after the bridge result, introduce any unallowlisted pre-P3 architecture/target/ABI/oracle delta, introduce an unallowlisted substantive REV06 step/table delta, introduce an unrelated verifier/test-driver/evidence change, omit `driver_core.py`'s REV15 source-state switch, make `evidence.py` reject valid R15.00 or accept a prospective record with a missing/wrong plan hash, leave `v1/docs/test-plan.md` or `v1/dist/certification/README.md` stale/contradictory for the new evidence schema, attempt to make an R15.00 record's certified source identity depend on the hash of the evidence-only commit containing that record, omit or contradict the `v1/docs/abi.md` EDIT/BREAK contract, accept EDIT as BREAK, decode EDIT in IM2, map BREAK/another chord to `0x1B`, make tty32/tty64 disagree, expose a BASIC EDIT token, permit non-owner input theft, omit the verifier baseline switch, make the E0.01/E0.03/E0.04 tooling regression fail, improperly require historical aggregate replay to satisfy current REV15/REV06 identity instead of its historical identity, skip any non-identity P1.41/P2.24 current-tree regression assertion, or make any such regression assertion fail; each must fail the bridge for the intended reason.
10. **Check-in gate:** Historical P2.24 and Phase-2 aggregate evidence PASS under exact REV12/REV03. Create one clean bridge source-candidate commit containing only the intended bridge kernel/documentation/tooling/tests changes; then, against that exact commit, bridge build + static + targeted emulator + changed-tooling regression + full non-admitting P1.41/P2.24 current-tree regression + all negative tests PASS under exact REV15 and the computed current REV06 identity. No historical doc/evidence bytes change. Scratch R15.00 records identify that exact source-candidate commit and freeze both current document identities. Admit only those exact records in a subsequent evidence-only commit; no durable record embeds or claims the hash of the commit that contains itself. End clean. Phase 3 is forbidden until the evidence-only R15.00 admission commit exists.
11. **Evidence:** Retain `v1/dist/certification/R15.00.build.json`, `v1/dist/certification/R15.00.test.json`, and `v1/dist/certification/R15.00.result.json`. All three contain `implementation_plan_sha256` and identify the same clean bridge source-candidate commit; the test/result records contain the non-admitting P1.41/P2.24 and E0.01/E0.03/E0.04 regression subresults. No regression run may write a historical step/phase evidence path. This result is the sole durable transition decision; it supplements, never rewrites, the historical E0-P2.24 evidence and is the plan/architecture identity anchor for every later certification.
...
1. **Purpose / REV15 requirement:** §§6.3,11.1,49; first ordinary Phase-3 step after mandatory R15.00 transition PASS
...
1. **Purpose / REV15 requirement:** §11.1
...
1. **Purpose / REV15 requirement:** §§11,13,32
...
1. **Purpose / REV15 requirement:** §32
...
1. **Purpose / REV15 requirement:** §11
...
1. **Purpose / REV15 requirement:** §11.3, §49
...
1. **Purpose / REV15 requirement:** §11, §49
...
1. **Purpose / REV15 requirement:** §§12.1,49
...
1. **Purpose / REV15 requirement:** §12
...
1. **Purpose / REV15 requirement:** §§12.1-12.2
...
1. **Purpose / REV15 requirement:** §§9.2,10.1,11.5
...
1. **Purpose / REV15 requirement:** §10.1, §49
...
1. **Purpose / REV15 requirement:** §49 acceptance
...
1. **Purpose / REV15 requirement:** §10.1, §13.5, §49
...
1. **Purpose / REV15 requirement:** §18
...
1. **Purpose / REV15 requirement:** §18, §27.3
...
1. **Purpose / REV15 requirement:** §§10.1,11.2
...
1. **Purpose / REV15 requirement:** §§11.1-11.2,27.3
...
1. **Purpose / REV15 requirement:** §§11.4-11.6,18
...
1. **Purpose / REV15 requirement:** §§11.5,18,31
...
1. **Purpose / REV15 requirement:** §11, §41.3A
...
1. **Purpose / REV15 requirement:** §10.1
...
1. **Purpose / REV15 requirement:** §18, §4.3B
...
1. **Purpose / REV15 requirement:** §10.1, §18
...
1. **Purpose / REV15 requirement:** §27.3
...
1. **Purpose / REV15 requirement:** §§11.4,27.3
...
1. **Purpose / REV15 requirement:** §27.3, §41.3A
...
1. **Purpose / REV15 requirement:** §10.1, §27.3
...
1. **Purpose / REV15 requirement:** §18.5, §27.3
...
1. **Purpose / REV15 requirement:** §4.4, §27.3
...
1. **Purpose / REV15 requirement:** §7, §27.3
...
1. **Purpose / REV15 requirement:** §10.1, §33
...
1. **Purpose / REV15 requirement:** §41.3A
...
1. **Purpose / REV15 requirement:** §18.6, §32; invariants 108/115
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All three listings, types, states, zero lengths, ordering and parent IDs match REV15 byte-for-byte.
...
1. **Purpose / REV15 requirement:** §10.1, §18, §50
...
1. **Purpose / REV15 requirement:** §50 acceptance
...
1. **Purpose / REV15 requirement:** §19, §51
...
1. **Purpose / REV15 requirement:** §19
...
1. **Purpose / REV15 requirement:** §19, §27.3
...
1. **Purpose / REV15 requirement:** §5.1, §51
...
1. **Purpose / REV15 requirement:** §19.7, §29
...
1. **Purpose / REV15 requirement:** §19, §20
...
1. **Purpose / REV15 requirement:** §7, §19, §27.3
...
1. **Purpose / REV15 requirement:** §51 acceptance
...
1. **Purpose / REV15 requirement:** §51
...
1. **Purpose / REV15 requirement:** §40.3
...
1. **Purpose / REV15 requirement:** §5.5, §20, §41.1
...
1. **Purpose / REV15 requirement:** §20.2
...
1. **Purpose / REV15 requirement:** §20.2A
...
1. **Purpose / REV15 requirement:** §20.3
...
1. **Purpose / REV15 requirement:** §20.4, §52
...
1. **Purpose / REV15 requirement:** §20.7
...
1. **Purpose / REV15 requirement:** §18, §20.7
...
1. **Purpose / REV15 requirement:** §20.5, §33, §52
...
1. **Purpose / REV15 requirement:** §20.5, §31, §52
...
1. **Purpose / REV15 requirement:** §20.3, §31
...
1. **Purpose / REV15 requirement:** §20.12
...
1. **Purpose / REV15 requirement:** §13.5, §20.12
...
1. **Purpose / REV15 requirement:** §20.4, §20.12
...
1. **Purpose / REV15 requirement:** §§20.12,21; invariants 99/111
...
1. **Purpose / REV15 requirement:** §13, §20.12
...
1. **Purpose / REV15 requirement:** §52 acceptance
2. **Exact implementation work:** Freeze host parser/AST and target execution vectors for quoting/operators/expansion/PATH/builtins/case. Golden vectors are derived from REV15 userland contracts only; no host-shell quoting, field-splitting, globbing, substitution, precedence, PATH, environment or parent-builtin behavior may be borrowed implicitly.
...
9. **Negative/failure test:** Any unsupported construct accidentally accepted fails. Run divergence vectors chosen to behave differently in common host shells; ZX-UX must follow REV15, not the host.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV15 requirement:** §20.12; invariants 110/118
...
1. **Purpose / REV15 requirement:** §52 acceptance
2. **Exact implementation work:** Aggregate login/core parser, echo pipe, PATH matrix, builtin E_NOTSUP in pipe/bg, redirection rollback, child errors and mixed-case behavior, plus REV15 shell-facing time rows: normal boot begins with valid revision-0 launch epoch; no normal `date: not set`; diagnostic invalid fixture remains explicit; date -s revision semantics and boot-valid cron/cal expectations remain delegated to their owning utility gates.
...
1. **Purpose / REV15 requirement:** §10.1, §15, §53
...
1. **Purpose / REV15 requirement:** §3.3, §15
...
1. **Purpose / REV15 requirement:** §15.1, §15.3
...
1. **Purpose / REV15 requirement:** §15.1, §15.4
...
1. **Purpose / REV15 requirement:** §15
...
1. **Purpose / REV15 requirement:** §§10.1,17
...
1. **Purpose / REV15 requirement:** §§14.6,20.6
...
1. **Purpose / REV15 requirement:** §14.7, §20.6
...
1. **Purpose / REV15 requirement:** §20.6
...
1. **Purpose / REV15 requirement:** §§4.3,14.8A,16.1-16.2
...
1. **Purpose / REV15 requirement:** §§10.1,16.1,16.3
...
1. **Purpose / REV15 requirement:** §16.2-16.3
...
1. **Purpose / REV15 requirement:** §16.5
...
1. **Purpose / REV15 requirement:** §16.4, §53
...
1. **Purpose / REV15 requirement:** §17, §53
...
1. **Purpose / REV15 requirement:** §53 acceptance
...
1. **Purpose / REV15 requirement:** §§17,20.5-20.7,53; invariant 78
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (ls)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/ls contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (cat)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/cat contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (cp)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/cp contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (mv)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/mv contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (rm)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/rm contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (pack)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/pack contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (unpack)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/unpack contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (grep)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/grep contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (wc)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/wc contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (head)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/head contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (tail)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/tail contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (cmp)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/cmp contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (true)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/true contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (false)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/false contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (sleep)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/sleep contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (which)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/which contract; no case folding. `which ls` can report the exact external path; builtin-only name emits no bytes and status 1; catalog-only lookup does not move cassette.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (env)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/env contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (hexdump)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/hexdump contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.4,33,54 (ps)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Builtin behavior and exit status match exact REV15 Section-33/ps contract; no case folding; no external `/bin/ps` object or BCAT slot exists.
...
1. **Purpose / REV15 requirement:** §§20.4,33,54 (mem)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Builtin behavior and exit status match exact REV15 Section-33/mem contract including `-c`; no case folding; no external `/bin/mem` object or BCAT slot exists.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (udg)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/udg contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (gfxdemo)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/gfxdemo contract; no case folding.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/stty contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (date)
2. **Exact implementation work:** Implement external `date` on the REV15 wall clock. Normal successful boot reads valid TIME1 and prints exact `YYYY-MM-DD hh:mm:ss` derived from the current launch-epoch-based software clock. `date -s "YYYY-MM-DD hh:mm:ss"` validates Gregorian 1970..2099, calls SYS_TIME_SET, increments revision on every successful set, and resets subsecond. Exact literal `date: not set` is retained only for explicit diagnostic invalid-state tests, never the normal production boot oracle.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/date contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (crontab)
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (man)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/man contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (cal)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/cal contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (uptime)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/uptime contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (whoami)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/whoami contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (uname)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/uname contract; no case folding. Output bytes are exactly ASCII `ZX-UX z80 48k` followed by one LF byte (0x0A), with no legacy `ZX48-UX` spelling.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (fortune)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/fortune contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (banner)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/banner contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (rev)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/rev contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (yes)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/yes contract; no case folding.
...
1. **Purpose / REV15 requirement:** §§20.5,33,54 (demo)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV15 Section-33/demo contract; no case folding.
...
1. **Purpose / REV15 requirement:** §54 acceptance
...
1. **Purpose / REV15 requirement:** §20.9, §41.7; invariants 105/109/119
...
1. **Purpose / REV15 requirement:** §22, §55
...
1. **Purpose / REV15 requirement:** §22
...
1. **Purpose / REV15 requirement:** §22.6
...
1. **Purpose / REV15 requirement:** §20.9, §55 acceptance
...
1. **Purpose / REV15 requirement:** §22.5, §22.6A
...
1. **Purpose / REV15 requirement:** §22.6A, §41.8
...
1. **Purpose / REV15 requirement:** §22, including §§22.7-22.8, and §55
...
1. **Purpose / REV15 requirement:** §55 acceptance, §44
...
1. **Purpose / REV15 requirement:** §23, §56
...
1. **Purpose / REV15 requirement:** §23
...
1. **Purpose / REV15 requirement:** §56
...
1. **Purpose / REV15 requirement:** §23, §38
...
1. **Purpose / REV15 requirement:** §23, §38, §56
...
1. **Purpose / REV15 requirement:** §23
2. **Exact implementation work:** Serialize deterministic OBJ1 from the fully validated assembler result. Symbol-record order is an implementation policy frozen by deterministic native-assembler tests and is NOT an architecture-prescribed sort order; relocation records MUST obey REV15's strictly increasing, non-overlapping offset rule. Compute the OBJ1 body/header CRCs over the exact stored bytes and run the independent host inspector before transactional publication.
3. **Files/artifacts created or modified:** as.asm
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P10.18`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Two identical native assemblies produce byte-identical OBJ1; the host inspector validates all records/offsets/CRCs and relocation records are strictly increasing/non-overlapping exactly as REV15 requires.
9. **Negative/failure test:** Any malformed symbol/relocation record, relocation-order/overlap violation, CRC mismatch, nondeterministic output for identical input, or documentation/test that falsely claims REV15 mandates symbol-table sorting fails before commit.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV15 requirement:** §24
...
1. **Purpose / REV15 requirement:** §24, §41.2A
...
1. **Purpose / REV15 requirement:** §24, §26, §41.2A
...
1. **Purpose / REV15 requirement:** §24, §56
...
1. **Purpose / REV15 requirement:** §56 acceptance
...
1. **Purpose / REV15 requirement:** §56, §44
...
Z80 code and must emit the REV15 OBJ1/ABI/runtime contracts. If the SDK and REV15
differ, REV15 wins and the difference must be recorded rather than silently copied.
...
constraints remain governed by REV15.
...
implemented or accepted, review that document end-to-end alongside REV15 and the pinned
...
REV15 remains the controlling ZX-UX architecture authority and the SDK repository
remains read-only.
...
conformance review against REV15, the applicable DOCX clauses, and the corresponding
behavior in the pinned Python SDK implementation. No discrepancy may be silently
accepted. If the DOCX conflicts with REV15, REV15 wins and the DOCX must be corrected
before the dependent step closes. If native behavior conflicts with REV15 or the
...
SDK disagree on behavior not already decided by REV15, resolve and record the intended
...
6. **Emulator test artifact:** N/A: this is a host-only contract/format/policy step; target behavior is closed by later dependent steps.
7. **Exact FUSE assertions/checkpoints:** N/A except where the host test deliberately launches a timeout/argv self-test.
8. **Expected PASS result:** Spec checklist against §25 names every supported/deferred language family, data-model rule and operator exclusion explicitly; no generic “subset” sentence substitutes for a missing item. The evidence records end-to-end review of `docs/04-C48 Language Specification Rev 0.11.docx`, every required correction, the pinned Python SDK commit, the reviewed SDK Python implementation/design/docs, every intentional REV15-over-SDK divergence, and every identified REV15/DOCX/SDK discrepancy with its recorded resolution.
9. **Negative/failure test:** Unsupported feature accidentally claimed, missing DOCX review/correction evidence, missing SDK provenance or Python implementation coverage, an unexplained REV15/SDK contract difference, or any unresolved REV15/DOCX/SDK discrepancy is documentation failure.
...
1. **Purpose / REV15 requirement:** §§25.5-25.6,57
...
1. **Purpose / REV15 requirement:** §25.7, §41.9, §57
...
1. **Purpose / REV15 requirement:** §25
...
1. **Purpose / REV15 requirement:** §25, §14
...
1. **Purpose / REV15 requirement:** §10.1, §25
...
1. **Purpose / REV15 requirement:** §25, invariant
...
1. **Purpose / REV15 requirement:** §25, §37
...
1. **Purpose / REV15 requirement:** §25.8; §§9-13
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All fifteen symbols resolve from the built-in libc48 archive and behavior matches the underlying REV15 syscall contracts, including short I/O and typed creation.
...
1. **Purpose / REV15 requirement:** §25, §42
...
1. **Purpose / REV15 requirement:** §2.8, §38, §57
...
1. **Purpose / REV15 requirement:** §2.8, §57
...
1. **Purpose / REV15 requirement:** §57
...
1. **Purpose / REV15 requirement:** §42, §57
...
1. **Purpose / REV15 requirement:** §57 acceptance
...
1. **Purpose / REV15 requirement:** §44, §41.3
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All FAST_REQUIRED allocations are satisfied; the <=20480 live-footprint measurement is physical, placement policy matches REV15, and the memory-short path reports `not enough memory for compiler` without corruption; no paper-only total.
...
1. **Purpose / REV15 requirement:** §25, §41.9
...
1. **Purpose / REV15 requirement:** §41.9
...
1. **Purpose / REV15 requirement:** §41.9; ZXP1/object invariants
...
1. **Purpose / REV15 requirement:** §10.1, §14, §25.8-25.9
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Formatted bytes/count/status match the REV15 ABI; no partial write on insufficient/invalid destination; IY/calculator state restored.
...
1. **Purpose / REV15 requirement:** §33, §42, §58
...
1. **Purpose / REV15 requirement:** §4.3B, §5.1, §20.5, §58
...
1. **Purpose / REV15 requirement:** §5.1, §58
...
1. **Purpose / REV15 requirement:** §4.3A, §58
...
1. **Purpose / REV15 requirement:** §19.8, §58
...
1. **Purpose / REV15 requirement:** §5.6, §19.8, §58
...
1. **Purpose / REV15 requirement:** §5.5, §41.1, §58
2. **Exact implementation work:** Execute the exact REV15 §5.5 thirty-step cold-boot sequence in order. Step 20 atomically initializes valid `TIME1` to `80 06 26 17 00 00` (1982-04-23 00:00:00, revision0, subsecond0) without touching independent SYS_TICKS; final font validation/pinning follows at its specified step; tty64 private cursor/wrap/mutation state is initialized before PID1; `EI` is step 29 and scheduler entry step 30. Retain machine-state evidence at every boundary and prove no accepted frame can observe a half-initialized wall clock.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic TAP/TZX artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All 30 ordered boot ownership/resource transitions match REV15 exactly; pre-first-frame TIME1 and independent ticks are exact; final font bytes are the P12.05 identity; first EI occurs only after all required state is valid.
...
1. **Purpose / REV15 requirement:** §58 items4-8, §63
...
1. **Purpose / REV15 requirement:** §58 item10
...
1. **Purpose / REV15 requirement:** §58 item11
...
1. **Purpose / REV15 requirement:** §58 item12
...
1. **Purpose / REV15 requirement:** §58 item13
...
1. **Purpose / REV15 requirement:** §58 item14
...
1. **Purpose / REV15 requirement:** §58 item15
...
1. **Purpose / REV15 requirement:** §58 item16
...
1. **Purpose / REV15 requirement:** §58 item17
...
1. **Purpose / REV15 requirement:** §58 item18, §42.15
...
1. **Purpose / REV15 requirement:** §19.9, §58
...
1. **Purpose / REV15 requirement:** §35.1, §44
...
1. **Purpose / REV15 requirement:** §40.4
...
1. **Purpose / REV15 requirement:** §40.4, §58
...
1. **Purpose / REV15 requirement:** §58 acceptance
...
6. **Emulator test artifact:** FUSE may be used only as regression evidence; it cannot close this hardware/analog gate.
7. **Exact FUSE assertions/checkpoints:** Not sufficient for PASS. Record real 48K-compatible hardware or hardware-faithful EAR/MIC-loop evidence.
8. **Expected PASS result:** Every item PASS; all within 48K.  The final acceptance record has exactly twenty numbered rows in REV15 order, each with target-state/evidence references, and all twenty PASS on the same official release tape and 48K candidate.
...
1. **Purpose / REV15 requirement:** §63
...
1. **Purpose / REV15 requirement:** §59
...
1. **Purpose / REV15 requirement:** §5.6, §19.8-19.9, §58
...
1. **Purpose / REV15 requirement:** handover Git rule
...
1. **Purpose / REV15 requirement:** §29; §41; invariants 23,25,29,32,39,55,71,92,99,111,133-135
...
1. **Purpose / REV15 requirement:** §30; invariant 55
...
1. **Purpose / REV15 requirement:** §58, §§40-44, §63
...
contract. No Revision-15 requirement may be left to implication.

| REV15 section | Primary implementation/certification coverage |
|---|---|
...
| §21 Cooperative Cancellation and kill | Historical P2 kill/started-state/lifecycle; R15.00 delta-certifies exact BREAK/EDIT separation before Phase 3; P6 foreground tty ownership/BREAK/kill/wait/jobs; P8 `yes` cooperative cancellation; P12 pipeline/background/concurrency tests |
...
| §41 Mandatory Test Matrix | R15.00 owns the pre-Phase-3 EDIT/BREAK delta rows; owning phase tests/correction matrices close all other rows; P12.26 performs the complete Section-41 replay; P12.27 adds the compatibility-emulator pass and physical gates where evidence class requires them |
...
| §47 Phase 1 - Resident Kernel Skeleton | Historical P1.01-P1.41 retain their REV12 evidence; R15.00 delta-certifies the REV15 EDIT/BREAK keyboard/console change that touches completed Phase-1 code before Phase 3 |
...
REV15**, not a copied/retyped list in this document. This avoids creating a second
...
## 5.2 Explicit REV15 §61 Invariant Ownership Ledger

The invariant prose itself remains authoritative only in canonical REV15. This table is deliberately one row per numbered invariant so an audit cannot hide a missing item inside a broad category. Each listed step must contain a positive or negative proof, or an explicit scope guard, for that invariant.

| REV15 invariant | Owning implementation/test step(s) |
|---:|---|
...
## 5.3 Exact REV15 §§9.2-10 Syscall ABI and Ownership Ledger
...
## 5.4 Exact REV15 §25.8 C48 Runtime Ownership Ledger

REV15 freezes a 73-function minimum public C48 runtime. This table is one row per required public symbol so none can disappear inside the phrase "libc wrappers". The owning step must build the symbol into the linker built-in libc48 archive and execute a positive/negative target test for it.
...
## 5.5 Exact REV15 §41 Mandatory Test Replay/Ownership Ledger

Every mandatory REV15 §41 acceptance item has exactly one stable replay row below, in the same subsection order. REV15 remains the authoritative wording; the middle column is a concise, semantically equivalent replay description and must never weaken, merge, or replace the canonical requirement. The listed phase owner closes the implementation-side contract; `P12.26` replays the canonical REV15 item on the release candidate. A missing, duplicate, reordered, merged-away, or semantically weakened row is a certification failure.

| REV15 test | Stable replay description of canonical acceptance item | Owning implementation/replay step(s) |
|---|---|---|
...
| §41.8 | Normal/insert/command-line mode transitions, including `EDIT`/`0x1B` escape/cancellation and BREAK separation. | `R15.00,P9.05,P9.15,P9.24,P12.26` |
...
| §41.11.3 | IM2 recognizes exact `CAPS SHIFT + SPACE` BREAK from raw matrix reads. | `P1.15,P1.31,R15.00,P12.26` |
| §41.11.3 | Non-BREAK key combinations, including `CAPS SHIFT + 1` `EDIT`, do not spuriously set `break_pending`. | `R15.00,P12.26` |
| §41.11.3 | IM2 contains no call edge to ROM KEYBOARD/decoder routines. | `P1.31,P12.26` |
| §41.11.3 | Ordinary tty input decodes `CAPS SHIFT + 1` `EDIT` as sole target byte `0x1B` in tty32/tty64; the same byte is observed through `SYS_CON_GETKEY`, `/dev/tty` reads, and C48 `getchar()` when stdin is tty; BREAK never returns `0x1B`. | `R15.00,P3.03,P11.24,P12.26` |
...
REV15 §62 questions below. A non-applicable answer must state why; a blank answer is a
failure.
...
This Revision-06 implementation plan is subordinate to canonical REV15 SHA-256
`8A4A2767D7D7F1A7568F5F5B660CE3DAD737767F69311366D1F6EC3C62EC940A` and becomes active only after the project SoP plus the anti-circular certification sequence below complete on these exact final document bytes.
...
1. a complete source-order REV15 Sections 1-65 architecture audit;
...
3. an adversarial cross-reference/ownership replay that starts again from the final
   ...
   baseline segmentation and R15.00 non-retroactivity, exercises contradictory/omitted
...
1. **Final certificate 1:** complete REV15 Sections 1-65 source-order audit, zero gaps.
2. **Final certificate 2:** independent complete REV15 Sections 1-65 audit, including
...
3. **Final certificate 3:** adversarial traceability replay over the same final bytes,
   including step/owner/reference resolution, R15.00 historical-baseline carry-forward,
...
for every REV15 requirement without knowingly strengthening, weakening, inventing, or
...
`E0 -> P0 -> P1 -> P2.01..P2.24 -> R15.00 -> P3.01..P3.21 -> P4 -> P5 -> P6 -> P7 -> P8 -> P9 -> P10 -> P11 -> P12`
