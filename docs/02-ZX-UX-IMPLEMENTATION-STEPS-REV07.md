...
Status: Revision-16 Phase-2/Phase-3 transition implementation and certification plan
Revision: 07
Architecture baseline: `docs/01-ZX-UX-ARCHITECTURE-REV16.md`
Architecture SHA-256: `24FE9D206C2F05BBC24F11544A5CB5A0B6AB104E9FC52E654A10C2D9734B008C`
...
This document is subordinate to Revision 16 for the R16.00 transition and all Phase-3
and later work. It does not amend the architecture. If this document conflicts with
...
Historical E0 certification through P2.24 remains bound to the Revision-12 identity
recorded in its retained evidence. As part of R16.00, the current verifier is advanced
to require `docs/01-ZX-UX-ARCHITECTURE-REV16.md` with SHA-256
`24fe9d206c2f05bbc24f11544a5cb5a0b6ab104e9fc52e654a10c2d9734b008c` for R16.00 and every later certification, while the project-local
SjASMPlus, Fuse, Fuse-utils, and 48K ROM remain selected by the same manifest.
...
Accuracy is the canonical goal. Efficiency is not a goal. Historical work through
P2.24 proceeds under Revision 12 / Revision 03; after R16.00, work proceeds in
Revision-16 / Revision-07 phase order. A later phase does not begin because its code
looks fun.
...
Z80 code and must emit the REV16 OBJ1/ABI/runtime contracts. If the SDK and REV16
differ, REV16 wins and the difference must be recorded rather than silently copied.
...
implemented or accepted, review that document end-to-end alongside REV16 and the pinned
...
6. **Emulator test artifact:** N/A: this is a host-only contract/format/policy step; target behavior is closed by later dependent steps.
7. **Exact FUSE assertions/checkpoints:** N/A except where the host test deliberately launches a timeout/argv self-test.
8. **Expected PASS result:** Spec checklist against §25 names every supported/deferred language family, data-model rule and operator exclusion explicitly; no generic “subset” sentence substitutes for a missing item. The evidence records end-to-end review of `docs/04-C48 Language Specification Rev 0.11.docx`, every required correction, the pinned Python SDK commit, the reviewed SDK Python implementation/design/docs, every intentional REV16-over-SDK divergence, and every identified REV16/DOCX/SDK discrepancy with its recorded resolution.
9. **Negative/failure test:** Unsupported feature accidentally claimed, missing DOCX review/correction evidence, missing SDK provenance or Python implementation coverage, an unexplained REV16/SDK contract difference, or any unresolved REV16/DOCX/SDK discrepancy is documentation failure.
...
1. **Purpose / REV16 requirement:** §§25.5-25.6,57
...
1. **Purpose / REV16 requirement:** §25.7, §41.9, §57
...
1. **Purpose / REV16 requirement:** §25
...
1. **Purpose / REV16 requirement:** §25, §14
...
1. **Purpose / REV16 requirement:** §10.1, §25
...
1. **Purpose / REV16 requirement:** §25, invariant
...
1. **Purpose / REV16 requirement:** §25, §37
...
1. **Purpose / REV16 requirement:** §25.8; §§9-13
...
1. **Purpose / REV16 requirement:** §25, §42
...
1. **Purpose / REV16 requirement:** §17, §25
...
1. **Purpose / REV16 requirement:** §2.8, §38, §57
...
1. **Purpose / REV16 requirement:** §2.8, §57
...
1. **Purpose / REV16 requirement:** §57
...
1. **Purpose / REV16 requirement:** §42, §57
...
1. **Purpose / REV16 requirement:** §57 acceptance
...
1. **Purpose / REV16 requirement:** §44, §41.3
...
1. **Purpose / REV16 requirement:** §25, §41.9
...
1. **Purpose / REV16 requirement:** §41.9
...
1. **Purpose / REV16 requirement:** §41.9; ZXP1/object invariants
...
1. **Purpose / REV16 requirement:** §10.1, §14, §25.8-25.9
...
1. **Purpose / REV16 requirement:** §42, §58
...
1. **Purpose / REV16 requirement:** §33, §42, §58
...
1. **Purpose / REV16 requirement:** §4.3B, §5.1, §20.5, §58
...
1. **Purpose / REV16 requirement:** §5.1, §58
...
1. **Purpose / REV16 requirement:** §4.3A, §58
...
1. **Purpose / REV16 requirement:** §19.8, §58
2. **Exact implementation work:** Freeze the official production tape manifest/order/transport with final assets. The `font4x8` M48O logical payload must decode to the exact P12.05 392-byte SHA-256-frozen bytes whether transported RAW or PACKED; no earlier Phase-0 font fixture may enter the release tape.
...
1. **Purpose / REV16 requirement:** §5.6, §19.8, §58
...
1. **Purpose / REV16 requirement:** §5.5, §41.1, §58
2. **Exact implementation work:** Execute the exact REV16 §5.5 thirty-step cold-boot sequence in order. Step 20 atomically initializes valid `TIME1` to `80 06 26 17 00 00` (1982-04-23 00:00:00, revision0, subsecond0) without touching independent SYS_TICKS; final font validation/pinning follows at its specified step; tty64 private cursor/wrap/mutation state is initialized before PID1; `EI` is step 29 and scheduler entry step 30. Retain machine-state evidence at every boundary and prove no accepted frame can observe a half-initialized wall clock.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic TAP/TZX artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All 30 ordered boot ownership/resource transitions match REV16 exactly; pre-first-frame TIME1 and independent ticks are exact; final font bytes are the P12.05 identity; first EI occurs only after all required state is valid.
...
1. **Purpose / REV16 requirement:** §58 items4-8, §63
...
1. **Purpose / REV16 requirement:** §58 item9
...
1. **Purpose / REV16 requirement:** §58 item10
...
1. **Purpose / REV16 requirement:** §58 item11
...
1. **Purpose / REV16 requirement:** §58 item12
...
1. **Purpose / REV16 requirement:** §58 item13
...
1. **Purpose / REV16 requirement:** §58 item14
...
1. **Purpose / REV16 requirement:** §58 item15
...
1. **Purpose / REV16 requirement:** §58 item16
...
1. **Purpose / REV16 requirement:** §58 item17
...
1. **Purpose / REV16 requirement:** §58 item18, §42.15
...
1. **Purpose / REV16 requirement:** §19.9, §58
...
1. **Purpose / REV16 requirement:** §44
...
1. **Purpose / REV16 requirement:** §35.1, §44
...
1. **Purpose / REV16 requirement:** §41
...
1. **Purpose / REV16 requirement:** §40.4
...
1. **Purpose / REV16 requirement:** §40.4, §58
...
1. **Purpose / REV16 requirement:** §58 acceptance
...
1. **Purpose / REV16 requirement:** §63
...
1. **Purpose / REV16 requirement:** §59
...
1. **Purpose / REV16 requirement:** §36, §37, §58
...
1. **Purpose / REV16 requirement:** §5.6, §19.8-19.9, §58
...
1. **Purpose / REV16 requirement:** handover Git rule
...
1. **Purpose / REV16 requirement:** §29; §41; invariants 23,25,29,32,39,55,71,92,99,111,133-135
...
1. **Purpose / REV16 requirement:** §30; invariant 55
...
1. **Purpose / REV16 requirement:** §43
...
1. **Purpose / REV16 requirement:** §58, §§40-44, §63
...
| §5 Boot Model | P0.07-P0.14 BASIC/SCREEN$/kernel/handoff/failure proof; P0.10 five-resource prefix contract; P1/P2/P3/P4/P5/P6 component prerequisites; P5 fixture tape; P12.03-P12.06 final frozen bootstrap resources; P12.07-P12.09 final tape and literal 30-step post-handoff boot sequence; P12.10 session handoff; P12.30 definitive boot acceptance |
...
for every REV16 requirement without knowingly strengthening, weakening, inventing, or
