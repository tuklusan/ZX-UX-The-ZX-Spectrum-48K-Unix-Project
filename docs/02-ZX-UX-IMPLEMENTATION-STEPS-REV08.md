...
Status: Revision-17 post-Phase-10 release-transport and native self-rebuild transition plan
Revision: 08
Architecture baseline: `docs/01-ZX-UX-ARCHITECTURE-REV17.md`
Architecture SHA-256: `D12BAF0B47A7F8CD2DCD60B82F100BA19F9FDDAABAD43728A004A07214716BF8`
Target: Original unexpanded 48K ZX Spectrum

---

Revision-08 controlled release-transport transition:

- preserves every historical E0.01-P2.24 record under REV12/REV03 and every R16.00/P3-P10 record under REV16/REV07 byte-for-byte;
- inserts R17.00 immediately after P10.36 and before P11.01 as the sole bridge into REV17/REV08;
- binds R17.00 and every future P11/P12 record to exact REV17/REV08 identities; no P11 record may be emitted under REV16/REV07;
- revises only future release-transport requirements to TZX-only reviewed fast loading, no production SCREEN$/`zx48uxscr`, no release TAP, exact 8192-byte kernel at 0xE000-0xFFFF, and handoff at 0xE003;
- requires deterministic TZX construction, independent turbo integrity derivation, independent embedded-kernel reconstruction, and real-time Fuse proof with fastload/traps/loader shortcuts disabled;
- requires exact three-way kernel identity `host-built == TZX-embedded == native-rebuilt` for all 8192 bytes;
- requires genuine `source -> native as -> OBJ1 -> native ld -> executable -> run` with native-produced OBJ1 and no host-preconstructed OBJ1/MEX1/kernel substitution;
- permits only prospective assembler/linker facilities required by REV17, including fixed/absolute kernel output if necessary, with rebuild output kept away from the executing kernel;
- requires exact byte comparison offsets 0..8191, all three SHA-256 values, and a controlled mutation mismatch-negative;
- preserves historical P10.34 evidence unchanged and does not recertify Phase 10;
- keeps Pasmo 0.5.3-7 release-scoped and outside shared certified toolchain/bootstrap/runtime-cache machinery; and
- forbids any Phase-11 implementation, qualification, evidence, media, admission, activation, or dispatch during the R17 migration/pre-release workflow.

Revision-07 controlled correction:

- supersedes frozen, never-activated Revision 06 without modifying it in place;
- records frozen REV15 SHA-256 `c80b0626d58d79b133de90ee64e598421618ce5c503d6f5d34c866e332e4c398`;
- records frozen REV06 actual SHA-256 `aeba14b8baa5387b98c158ac00ee9d81e93a162807769497068dac1adc0c8624`;
- records the blocking Revision-06 Section-8 defect: its header correctly named REV15,
  but Section 8 incorrectly bound the plan to obsolete Revision-13 SHA-256
  `8a4a2767d7d7f1a7568f5f5b660ce3dad737767f69311366d1f6ec3c62ec940a`;
- therefore R15.00 was never admissible and MUST NOT be used;
- carries forward the intended Revision-15 architecture semantics and bounded H06 contract
  unchanged as Revision 16, with no new target behavior;
- replaces the never-admitted R15.00 bridge with R16.00 bound to exact Revision 16 /
  Revision 07; and
- preserves every historical E0.01-P2.24 step body byte-for-byte from Revision 03.

Revision-06 controlled correction:

- supersedes frozen, never-activated Revision 05 without modifying it in place;
- records frozen REV14 SHA-256 `868d6605733061fec00fb82c051e9ab02915041e5e26ddc1563a3c488bc82324`;
- records frozen REV05 actual SHA-256 `2a7a84e01fd2016b4dfd5421d1510f2474ef7559ee1dea584bbb98d11811e047`;
- corrects Revision 05's stale embedded Revision-13 architecture digest and stale
  `Revision-14/04` bridge labels by binding this plan to exact Revision 15 instead;
- preserves the intended bounded H06 SDK hello-canary contract unchanged;
- renames the never-admitted bridge from R14.00 to R15.00 and binds it to the exact
  Revision-15 / Revision-06 authority pair; and
- preserves every historical E0.01-P2.24 step body byte-for-byte from Revision 03.

# 1. Certified Development Environment and Gate E0

This document preserves Revision 16 / Revision 07 as historical authority for R16.00 and P3-P10. After admitted R17.00, this document is subordinate to Revision 17 for all future P11/P12 work. It does not amend the architecture. If this document conflicts with
Revision 16 in that epoch, Revision 16 wins and this document must be corrected before
implementation continues. The step bodies from E0.01 through P2.24 are retained as the
historical Revision-03 plan and remain governed by immutable Revision 12 / Revision 03.
Their evidence is not retroactively relabeled. P2.24 is the final certification step of
that historical epoch; R16.00 is the mandatory one-time transition before P3.01.
...
Historical E0 certification through P2.24 remains bound to REV12/REV03. R16.00 and P3-P10 remain permanently bound to REV16/REV07. R17.00 and future P11/P12 certification use REV17/REV08. The project-local SjASMPlus, Fuse, Fuse-utils, and 48K ROM remain selected by the same shared manifest; release-scoped Pasmo 0.5.3-7 does not enter that manifest.
...
Accuracy is the canonical goal. Efficiency is not a goal. Historical work through P2.24 is governed by REV12/REV03; R16.00 and P3-P10 are governed permanently by REV16/REV07; after admitted R17.00, future P11/P12 work proceeds under REV17/REV08. A later phase does not begin because its code
looks fun.

Revision 07 has two explicit certification epochs. Historical evidence through P2.24
remains bound to Revision 12 / Revision 03 and to the source commits recorded in those
evidence files. R16.00 is the sole bridge into the Revision-16 / Revision-07 epoch. On
bridge PASS, historical records remain unchanged and are admitted only as prerequisite
evidence. The known Revision-16 EDIT/ESC delta touches already-completed Phase-1
keyboard/console behavior; R16.00 therefore delta-implements and delta-recertifies that
bounded change without rewriting P1 evidence. Any additional pre-P3 semantic change
outside the explicit bridge allowlist fails closed and requires replay from the earliest
affected historical step. P3.01 is the first ordinary phase step certified directly
against Revision 16 / Revision 07.
...
requires `<STEP-ID>.build.json` and `<STEP-ID>.test.json`. R16.00 follows the same
durable record contract at the Phase-2/Phase-3 boundary and is an explicit
certification-result boundary. E0.04 and P0.34 remain their historical result
boundaries; R16.00 additionally requires its own `<STEP-ID>.result.json`; Phase 0 also
requires `phase-0.json`. Human-readable `.log`
...
named assertions. Historical E0.01-P2.24 records retain that Revision-03 schema exactly.
Beginning with R16.00, every new durable build/test/result record additionally contains
`implementation_plan_sha256`, the SHA-256 of canonical
`docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV07.md`. R16.00 computes that value from the
current file and freezes it in `R16.00.result.json`; the plan does not embed its own
digest. Every later Phase-3-through-Phase-12 certification must prove both that the
current REV07 bytes hash to the value frozen by R16.00 and that its own
`implementation_plan_sha256` field equals that value. Any later REV07 byte change
therefore reopens the transition rather than silently changing the certification plan.
`v1/build` is disposable; `v1/dist` is not.

R16.00 uses the same anti-self-reference discipline explicitly. First create one clean
**bridge source-candidate commit** containing the complete intended Revision-16/06
kernel/documentation/tooling/test changes but no new `R16.00.*.json` durable records.
Certify that exact clean commit and write its scratch build/test/result records outside the
source worktree, with `source_commit` and `bridge_source_commit` equal to that candidate
commit. Only after all bridge gates PASS may an **evidence-only admission commit** add the
three frozen R16.00 durable records to `v1/dist/certification/`. The admission commit hash
is not embedded in a record contained by that same commit. P3.01 is forbidden until the
evidence-only admission commit exists and the worktree is clean.
...
# 3. Fixed Architecture Contracts Used by the Revision-16 Epoch

From R16.00 onward, the implementation must preserve, among the rest of Revision 16,
these high-risk contracts. Historical E0 through P2.24 remain certified against the
corresponding Revision-12 contracts and are not retroactively rewritten:
...
- from Phase 0 onward `font4x8` logical bytes are sourced directly from
  `v1/assets/font4x8-zxux.bin`, exactly 392 bytes and host-frozen at SHA-256
  `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`; no generated
  or non-final Phase-0 fixture is permitted, and P12.05 re-verifies rather than promotes this identity.
- successful normal cold boot initializes valid wall time to `1982-04-23 00:00:00`,
  exact TIME1 `80 06 26 17 00 00`, revision 0/subsecond 0 before first runtime `EI`;
  independent `SYS_TICKS` is never derived from epoch state.
- tty32/tty64 use deferred right-margin wrap with only real public coordinates,
  balanced nested screen mutation, and a 25-frame IM2 service-parity producer consumed
  only at safe non-ISR points.
- physical `CAPS SHIFT + 1` `EDIT` is the sole tty ESC-equivalent and decodes in ordinary
  task context as target byte `0x1B` in both tty modes; physical `CAPS SHIFT + SPACE`
  BREAK remains IM2-sampled cooperative cancellation and never aliases `0x1B`.
...
- the final system-tape order is frozen by Revision 16 Section 19.8 and is not a
...
1. **Purpose / REV12 requirement:** Establish the reproducible Linux environment required by REV12 host tooling assumptions before implementation or certification continues.
...
1. **Purpose / REV12 requirement:** REV12 at `docs/01-ZX-UX-ARCHITECTURE-REV12.md` is the sole architecture authority.
2. **Exact implementation work:** Hash the canonical on-disk REV12 architecture before implementation and compare it with frozen SHA-256 `a90d523f62a95e8cba6af0312b596a2d5f6bc1aa2ef92f39bb391509b7c15e1b`. The exact authority is `docs/01-ZX-UX-ARCHITECTURE-REV12.md`; superseded architecture files remain historical/read-only and may not satisfy E0.02. Architecture bytes are immutable during implementation unless a later architecture change request reopens this gate.
...
8. **Expected PASS result:** `docs/01-ZX-UX-ARCHITECTURE-REV12.md` hashes exactly to `a90d523f62a95e8cba6af0312b596a2d5f6bc1aa2ef92f39bb391509b7c15e1b` and no alternate/superseded architecture is consumed.
9. **Negative/failure test:** Supply an intentionally wrong path and, separately, an intentionally wrong expected digest; both must be rejected deterministically before any downstream gate runs.
...
1. **Purpose / REV12 requirement:** REV12 §§37,39-41; canonical Linux Python orchestration.
...
1. **Purpose / REV12 requirement:** REV12 ordered programming cycle and deterministic Git/evidence discipline.
...
1. **Purpose / REV12 requirement:** §§3-4,46,65
...
1. **Purpose / REV12 requirement:** §4.1, §5, §65
...
1. **Purpose / REV12 requirement:** §4.1, §46, §65
...
1. **Purpose / REV12 requirement:** §3, §14, §46, §65
...
1. **Purpose / REV12 requirement:** §§2.8,6,14,46,65
...
1. **Purpose / REV12 requirement:** §5.2, §46, §65
...
1. **Purpose / REV12 requirement:** §5.3, §44, §46, §65
...
1. **Purpose / REV12 requirement:** §§5.1-5.2,39,46,65
...
1. **Purpose / REV12 requirement:** §§4.3A-4.3B,5.1,5.6,65
2. **Exact implementation work:** Add contract-valid Phase-0 stubs and validate exact M48O names/order/types/targets: sh,font4x8,issue,crontab,bincat. Freeze the `issue` logical target bytes exactly as `ZX-UX - Inspired by Unix for the Sinclair ZX Spectrum 48K<LF>`, `64-column shell, native tools, C compiler, cassette storage<LF>`, `48K. One Z80. No excuses.<LF>`; freeze `crontab` as zero-length RAW and freeze the BCAT shape even before final binaries exist. `v1/assets/font4x8-zxux.bin` is the mandatory canonical Phase-0 font source. P0.10 MUST consume those checked-in bytes directly, require exact 392-byte F4X8 structure and host SHA-256 `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`, and reject any generated, alternate, or non-final font payload. P12.05 later re-verifies the same byte identity; it does not promote a different asset.
3. **Files/artifacts created or modified:** `v1/assets/font4x8-zxux.bin` (read-only canonical source consumed directly); `issue.txt`; `crontab.txt`; `bincat.bin`; maketap tests
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P0.10`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic TAP/TZX artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Host validator checks the exact five-resource contract, rejects case-folding, consumes `v1/assets/font4x8-zxux.bin` directly, and proves its exact 392-byte F4X8 structure and frozen SHA-256 without generating or substituting another font payload.
9. **Negative/failure test:** Swap resources or uppercase a name; substitute a generated/alternate font payload, mutate one canonical font byte, or change its length/header/hash; separately restore the legacy author/blog payload, mutate one issue byte, remove the final LF, introduce CRLF, or append extra data. Each case must be rejected deterministically.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §5, §41.1, §46, §65
...
1. **Purpose / REV12 requirement:** §5.4-5.5, §46, §65
...
1. **Purpose / REV12 requirement:** §5.3-5.5, §41.1, §65
...
1. **Purpose / REV12 requirement:** §§2.3,14.1,38,46
...
1. **Purpose / REV12 requirement:** §§13-14,46,65
...
1. **Purpose / REV12 requirement:** §§3.3,14-15,46
...
1. **Purpose / REV12 requirement:** §14, §15, §46, §65
...
1. **Purpose / REV12 requirement:** §§14,17,46,65
...
1. **Purpose / REV12 requirement:** §§14,19,46,65
...
1. **Purpose / REV12 requirement:** §§14.5,25,46,65
...
1. **Purpose / REV12 requirement:** §§14,25,46,65
...
1. **Purpose / REV12 requirement:** §14.6, §46, §65
...
1. **Purpose / REV12 requirement:** §§14.4,35,46,65
...
1. **Purpose / REV12 requirement:** §§14.8-14.8A,4.3
...
1. **Purpose / REV12 requirement:** §§2.12,27.3,41.3A,46,65
...
1. **Purpose / REV12 requirement:** §41.3A, §46
...
1. **Purpose / REV12 requirement:** §46 acceptance; §65 first deliverable
...
1. **Purpose / REV12 requirement:** §§4,5.5,47
...
1. **Purpose / REV12 requirement:** §§4.4,28,41.3,47
...
1. **Purpose / REV12 requirement:** §4.4
...
1. **Purpose / REV12 requirement:** §10.1, §41.3
...
1. **Purpose / REV12 requirement:** §§6.1-6.3,47
...
1. **Purpose / REV12 requirement:** §6.2, §47
...
1. **Purpose / REV12 requirement:** §§6.5-6.6,47
...
1. **Purpose / REV12 requirement:** §§9-10.1,47
...
1. **Purpose / REV12 requirement:** §47
...
1. **Purpose / REV12 requirement:** §§6.4-6.5,47
...
1. **Purpose / REV12 requirement:** §6.3, §47
2. **Exact implementation work:** Implement the canonical IM2 handler core and SYS_TICKS. Each accepted PAL/50-Hz frame increments the independent 32-bit frame counter modulo 2^32, mirrors ROM FRAMES as owned by P1.14, advances wall-clock subsecond/seconds only when wall time is valid, performs sleep/BREAK/event producer work, advances the cursor divider and toggles one-bit `cursor_service_parity` on each 25th accepted frame, and performs no screen/coordinate/wrap/mutation-depth work or arbitrary context switch. Both altreg preservation paths must include this producer. `SYS_TICKS` copies one coherent snapshot to HL as exactly four little-endian bytes without a torn interrupt update and without deriving from the boot epoch, wall validity, or TIME1 revision. Freeze the interrupt-safe parity handoff so a safe consumer can atomically fetch-and-clear without losing a transition posted immediately before/within/after the exclusion window.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Tick/FRAMES/wall-time/wake/BREAK/cursor producer responsibilities are bounded and exact; static/call-edge proof rejects forbidden ISR work; both altreg paths execute the timing producer and terminate correctly with RETI. SYS_TICKS writes exactly four coherent little-endian bytes, 0xFFFFFFFF->0 wraps modulo 2^32, and parity races lose or double-consume no transition.
9. **Negative/failure test:** Inject torn-snapshot timing, double interrupt accounting, a cursor bitmap/coordinate/wrap/mutation write from IM2, an altreg path that skips the producer, or a parity post at each handoff race boundary; any loss/double-consume/forbidden mutation fails deterministically.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §§14.8A,4.3,5.5,47
...
1. **Purpose / REV12 requirement:** §6.3, §21
...
1. **Purpose / REV12 requirement:** §§2.8,6.3,47
...
1. **Purpose / REV12 requirement:** §§6.3,14,47
...
1. **Purpose / REV12 requirement:** §§6.7,10.1,47
...
1. **Purpose / REV12 requirement:** §§6,10.1,47
...
1. **Purpose / REV12 requirement:** §§13.1A,13.5A,47
2. **Exact implementation work:** Implement the tty32 fallback core with the same REV12 deferred right-margin wrap state used by tty64: row 0..23 and col 0..31 are always real coordinates; writing the final real cell leaves the coordinate there and sets `wrap_pending`; the next printable first wraps/scrolls then draws; controls cancel pending before their own semantics; mode entry/change clears pending. No col32/row24 sentinel state may exist.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Golden printable/control vectors at interior, right edge and bottom row prove exact 32x24 fallback rendering, deferred-wrap state, one-row scroll, real coordinates only, and shared-console semantics.
9. **Negative/failure test:** Any BASIC-editor handoff, 22+2-line behavior, col32/row24 persistent sentinel, immediate wrap on last-cell write, control that fails to cancel pending, or write beyond row23/col31 fails.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §§4.3A,5.5,13.1,47
2. **Exact implementation work:** Validate the F4X8 payload structurally before PID1: magic/version/first_code/glyph_count/flags and exact 392-byte layout with 384 packed glyph bytes, high nibble earlier row, bit3 leftmost. Pin the validated resource FAST_REQUIRED and expose no physical address. Target Z80 code deliberately does not compute SHA-256. Host certification for P1.21 MUST consume the canonical `v1/assets/font4x8-zxux.bin` source already frozen at P0.10, prove exact 392-byte F4X8 structure and host SHA-256 `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`, and then prove target validation/pinning/render semantics. P12.05 later re-verifies the same byte identity; it does not promote a different Phase-1 fixture.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Host decoder proves the canonical `v1/assets/font4x8-zxux.bin` exact SHA-256 and target validator agrees on every header byte and all 96*8 scan rows; the exact canonical 392-byte resource pins FAST and renders 0x20..0x7F exactly.
9. **Negative/failure test:** Wrong magic/version/first-code/count/flags/length, altered nibble ordering, any canonical-source SHA-256 drift or legacy-fixture substitution, or inability to obtain FAST_REQUIRED storage causes explicit validation/boot failure with no COLD spill.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §§13.1-13.2,47
2. **Exact implementation work:** Implement tty64 as exactly 64x24 logical 4x8 cells over the 256x192 native bitmap: x_byte=col>>1; even columns replace high nibble bits7..4, odd columns low nibble bits3..0; for each scan row 0..7 use the canonical Spectrum scanline-address helper and preserve the neighboring nibble. Render target codes 0x20-0x7F from the pinned canonical `v1/assets/font4x8-zxux.bin` F4X8; a UDG shown in tty/editor UI occupies two logical text columns. Spectrum attributes remain physical 8x8 cells, so each pair of adjacent tty64 columns shares one attribute byte; shell/vi default to uniform text attributes, ink/paper/bright/flash changes affect both columns of the pair, and the bitmap cursor never changes attributes. This is the earliest real tty64 rendered-font step, so its test action MUST capture a Fuse screenshot, retain the matching PNG/SCR pair, and automatically inspect the captured SCR against an independently rendered 96-glyph canonical-font atlas, decode the corresponding retained PNG to pixels and require its complete 320x240 raster to match the same expected atlas and border layout, and fail on any mismatch; screenshot existence, filename pairing, or human-only inspection cannot satisfy the step.
...
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
11. **Evidence:** Retain `v1/dist/certification/P1.22.build.json` and `v1/dist/certification/P1.22.test.json`; the test record MUST name PASS assertions for the automated captured-frame visual inspection and retained PNG screenshot, record the SHA-256 and exact common captured-frame identity of `P1.22-font-atlas.png` and `P1.22-font-atlas.scr`, and the workflow artifact MUST retain both files. The Phase-1 finalizer independently verifies those files/hashes/assertions. Add `v1/dist/certification/P1.22.result.json` only when this step is explicitly designated a certification-result boundary; phase aggregate evidence is retained at the phase gate.

## P1.23 - tty64 scroll

1. **Purpose / REV12 requirement:** §13.3
2. **Exact implementation work:** Scroll exactly one logical 8-pixel tty64 row using canonical Spectrum scanline addresses: copy 32 bitmap bytes for destination rows0..22 and scan rows0..7, copy exactly 23*32 attributes upward, clear final 32 attributes and final eight bitmap scanlines. The operation runs only inside the REV12 balanced screen-mutation discipline: a nested scroll does not independently hide/show cursor XOR; outermost begin/end owns hide/reconcile and cooperative-switch exclusion.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** A 24-row labeled screen retains exactly former rows1..23, clears row23 bitmap/attributes, preserves display guards, never copies cursor XOR into retained content, and nested invocation causes no extra hide/reconcile.
9. **Negative/failure test:** Linear 6144-byte copy, 31/33 bytes per scanline, wrong attribute count, uncleared final row, unbracketed mutation, cursor XOR copied into content, or nested scroll that independently redraws fails golden hashes/state assertions.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §§13.4-13.4A
2. **Exact implementation work:** Implement the private software-cursor state from REV12 §§13.4-13.4A: real row/col only, shape, logical phase, physical-drawn state, 0..24 blink divider state as produced by IM2, one-bit pending service parity, and balanced nested screen-mutation depth. Cursor XOR is reversible and preserves neighbor nibble/attributes. `screen_begin` hides only on outermost entry; nested begins/ends do not redraw/release; outermost `screen_end` performs one safe parity consume/reconcile before releasing cooperative-switch exclusion. Underflow, unbalanced exit, or depth wrap is a deterministic failure. Shape off may span blink intervals; re-enable reconciles to current logical phase without resetting the clock.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Rows/cols/shapes/defaults are exact; XOR hide/show restores byte-identical bitmap; nested mutations hide/reconcile exactly once; parity consumption produces odd/even coalescing with no catch-up flash; shape-off/re-enable preserves timing; underflow/unbalanced/depth-wrap fixtures fail.
...
1. **Purpose / REV12 requirement:** §§13.7,17,61
...
1. **Purpose / REV12 requirement:** §20.8, §47
2. **Exact implementation work:** Implement software wall-clock state and the REV12 deterministic successful cold-boot initializer. Before first runtime EI/PID1 observability, atomically set valid wall time to `1982-04-23 00:00:00`, `wall_seconds=388368000` (`0x17260680`), TIME1 bytes `80 06 26 17 00 00`, revision 0, private subsecond 0. This initialization is not SYS_TIME_SET and does not initialize/derive SYS_TICKS. Frames 1..49 retain the second and frame 50 increments once. Preserve an explicit invalid state only for early-boot/diagnostic/failure fixtures; no host/emulator/RTC/tape/filesystem/BASIC/network source may seed time.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Successful cold boot exposes exact valid launch-epoch TIME1 before first frame, independent ticks, exact 50-frame rollover, and deterministic reboot to the same epoch/revision/subsecond; diagnostic invalid state remains testable but is not normal PID1-visible state.
9. **Negative/failure test:** Any normal boot that is unset, any hidden/persistent/external time source, boot initialization implemented as SYS_TIME_SET/revision 1, coupling ticks to epoch, or non-atomic first-frame boundary fails.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §10.1, §20.8, §47
2. **Exact implementation work:** Implement exact TIME record ABI and permissions. SYS_TIME_GET writes exact six-byte TIME1 and on normal successful boot returns the launch-epoch valid revision-0 record; it returns E_AGAIN only in explicit diagnostic/early invalid state. SYS_TIME_SET reads u32 seconds, validates 1970..2099, atomically marks valid, resets subsecond, and increments revision modulo 65536; first successful set after boot produces revision 1 even if the seconds value is unchanged. No hidden time source or timezone fiction.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Normal boot get returns `80 06 26 17 00 00` before first accepted frame; diagnostic invalid get returns E_AGAIN; valid set/get is exact; repeated same-value sets each increment revision and reset subsecond; invalid range is atomic.
...
1. **Purpose / REV12 requirement:** §35.1, §44
2. **Exact implementation work:** Instrument the fixed kernel stack and enforce the frozen guard/high-water contract. Stress every kernel/ROM/ISR path including the deepest nested CR1 screen-mutation plus scroll-or-clear path, with an interrupt injected at every interruptible point and the larger-stack ISR preservation path. Production-linked maximum consumption must be <=448 bytes, leaving >=64 bytes untouched; guard 0xFB00-0xFB0F remains intact.
...
1. **Purpose / REV12 requirement:** §41.11.1, §47
...
1. **Purpose / REV12 requirement:** §41.11.2
2. **Exact implementation work:** Close alternate-register/IM2 timing with exhaustive instruction-boundary injection for both fast and ROM-safe preservation paths. In addition to the frozen altreg_busy contract, both paths must execute the P1.13 tick/wall/cursor-divider/parity producer exactly once per accepted interrupt and remain within the measured cycle/timing budget; no path may skip, duplicate, or reorder the producer relative to state preservation.
...
1. **Purpose / REV12 requirement:** §§13.6,13.8,41.11.3
2. **Exact implementation work:** Implement raw BREAK recognition and ordinary tty decode separation, and wire the required safe cursor-service points around idle and blocked input: service due parity immediately before HALT, immediately after HALT wake before scheduling/return, and after tty-input wake before key delivery, reblock, yield, or syscall return. These safe consumers may not alter row/col/wrap_pending except through an independently requested console mutation.
...
1. **Purpose / REV12 requirement:** §41.11.4
2. **Exact implementation work:** Run the REV12 cursor/direct-screen race matrix: ordinary and bottom-right pending-wrap cursor reversibility; all direct OS graphics/UDG/scroll/clear paths under balanced nested mutation brackets; 1/2/3/4 delayed 25-frame intervals produce parity 1/0/1/0; interrupt injection immediately before/inside/after atomic parity consume loses/doubles nothing; IM2 mutates no bitmap/row/col/wrap/depth; idle and tty-input safe service points consume at first safe opportunity; shape-off intervals and re-enable preserve phase; mutation underflow/unbalance fails deterministically.
...
1. **Purpose / REV12 requirement:** §41.11.5, §35.1, §44
2. **Exact implementation work:** Repeat stack high-water/guard certification using the production-linked kernel after cursor/deferred-wrap paths exist. Include the deepest nested cursor/mutation/scroll-or-clear path with the large ISR preservation path injected at every interruptible boundary; require <=448 bytes consumed, >=64 bytes untouched, intact 0xFB00-0xFB0F guard, and PANIC KSTACK on deliberate corruption.
...
1. **Purpose / REV12 requirement:** §10.1, §13, §47
2. **Exact implementation work:** Implement `SYS_CON_GETKEY` exactly: no arguments; enforce tty-input-owner; on success return H=0,L=target byte; decoding remains outside IM2. When an input wait wakes, perform pending safe cursor parity service before key delivery, reblock, yield, or syscall return, without changing row/col/wrap_pending except through a separately requested console mutation.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Owner receives exact target byte in L with H=0; non-owner cannot steal input; IY returns 0x5C3A; an injected due cursor parity on the wake boundary is consumed at the first safe point with key and console position/pending state exact.
...
1. **Purpose / REV12 requirement:** §10.1, §13, §47
2. **Exact implementation work:** Implement `SYS_CON_PUTCHAR` exact ABI plus REV12 byte semantics. H must be zero and L is one target byte. Printable bytes use current mode deferred wrap; CR/LF/BS/TAB/FF first cancel pending wrap then apply Section13.5A semantics; all screen changes use balanced mutation brackets and real coordinates only. Return with IY canonical and no neighbor/attribute corruption.
3. **Files/artifacts created or modified:** `v1/src/kernel/console.asm`; `v1/src/kernel/syscall.asm`; console ABI tests
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P1.35`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** The same printable/control corpus in tty32 and tty64 at interior/right-edge/bottom-row positions yields exact bitmap/attributes, real row/column, wrap_pending, scroll count, neighbor nibble, reversible cursor state and canonical IY.
9. **Negative/failure test:** H nonzero, immediate last-cell wrap, phantom column/row, BS/TAB/LF using stale pending semantics, double-scroll, unbalanced mutation, ISR bitmap cursor edit, or output into a still-XORed cell fails deterministically.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §10.1, §13, §47
2. **Exact implementation work:** Implement `SYS_CON_WRITE` exact ABI and preserve console stream identity. One-byte calls, arbitrary split/chunked calls and one contiguous write of the same bytes must yield byte-identical bitmap/attributes/real row/col/wrap_pending across right-edge, controls and scroll boundaries. Buffer range validation remains widened/atomic, including legal 0x7FF0+0x0020 and wrapped/protected negatives; count-zero does not dereference the pointer.
3. **Files/artifacts created or modified:** `v1/src/kernel/console.asm`; `v1/src/kernel/syscall.asm`; console ABI tests
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P1.36`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Exact byte count returns; legal cross-boundary buffer works; invalid/wrapped/protected ranges fail before screen mutation; every chunking partition of the same byte stream reaches identical screen/cursor/wrap state.
9. **Negative/failure test:** Use wrapped end arithmetic, dereference count-zero poison pointer, reset/lose wrap_pending between calls, or make split writes differ from one call; each fails.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §10.1, §13, §47
2. **Exact implementation work:** Implement `SYS_CON_CLEAR` exact ABI using one balanced screen-mutation transaction. Clear the current terminal display and reset logical cursor to the documented real origin with `wrap_pending` clear; preserve ULA shadow/protected workspace; outermost mutation owns cursor hide/parity reconcile and nested calls must not double-redraw.
3. **Files/artifacts created or modified:** `v1/src/kernel/console.asm`; `v1/src/kernel/syscall.asm`; console ABI tests
...
1. **Purpose / REV12 requirement:** §10.1, §13, §47
2. **Exact implementation work:** Implement TTY_GETPOS to return only real row/column coordinates and never encode pending wrap as column 64/32 or row 24; retain separate private wrap_pending state.
3. **Files/artifacts created or modified:** `v1/src/kernel/console.asm`; `v1/src/kernel/syscall.asm`; console ABI tests
...
1. **Purpose / REV12 requirement:** §10.1, §13, §47
2. **Exact implementation work:** Implement TTY_SETPOS atomically: validate requested real row/column for current mode first, reject invalid values without changing position/pending/screen, and on success set the real coordinate and clear pending wrap.
3. **Files/artifacts created or modified:** `v1/src/kernel/console.asm`; `v1/src/kernel/syscall.asm`; console ABI tests
...
1. **Purpose / REV12 requirement:** §§27.2,47
...
1. **Purpose / REV12 requirement:** §47 acceptance
2. **Exact implementation work:** Run the complete revised Phase-1 aggregate: coherent four-byte SYS_TICKS; IM2 producer purity and parity handoff; deterministic boot-epoch wall-clock behavior; deferred-wrap tty64/tty32 with real coordinates and split-write identity; balanced nested screen mutation and safe cursor service; alternate-register paths; BREAK/input separation; stack high-water/guard; all existing Phase-1 ABI/memory/ROM contracts. No old normal-boot unset-time oracle is permitted.
...
1. **Purpose / REV12 requirement:** §§8,41.11.6,48
...
1. **Purpose / REV12 requirement:** §48
...
1. **Purpose / REV12 requirement:** §8.1
...
1. **Purpose / REV12 requirement:** §7, §48
...
1. **Purpose / REV12 requirement:** §§4.4,8.2,48
...
1. **Purpose / REV12 requirement:** §8.4
...
1. **Purpose / REV12 requirement:** §8.5
...
1. **Purpose / REV12 requirement:** §§6.4,8.2-8.3
...
1. **Purpose / REV12 requirement:** §§6,10.1A,41.2A
2. **Exact implementation work:** Implement the zero-side-effect `SYS_SPAWN` preflight boundary. `SYS_SPAWN` consumes HL -> exact 16-byte PROC1: +0 path_ptr u16, +2 arg1_ptr u16, +4 arg1_len u16, +6 env1_ptr u16, +8 env1_len u16, +10 stdin_handle u8, +11 stdout_handle u8, +12 stderr_handle u8, +13 flags u8 where only bit0 ALLOW_TAPE exists, +14 reserved u16=0. Before dereferencing PROC1 or any pointed range, searching/resolving an executable, moving tape, allocating arena memory, reserving a process slot, or publishing child state, scan PID2..7 for capacity. If no FREE slot exists, return E_AGAIN first with zero side effects. When capacity exists, validate the complete 16-byte PROC1 record and every pointed user range under the frozen widened-range rules, validate reserved bits/bytes, validate stdin/stdout/stderr structurally as handle numbers 0..7, and validate the bounded path string before any allocation or cassette action. Whether those parent handles currently refer to valid open descriptions is completed by P2.10. P2.09 does not classify RAM-resident versus BCAT-only executables and does not consult RAM-object metadata, `bincat`, or cassette contents. ALLOW_TAPE=0 is fail-closed at this boundary and must never enter a cassette path. Catalog-only executable classification and its spawn-specific E_AGAIN distinction are closed by P5.14/P5.19 once the production BCAT/tape path exists.
3. **Files/artifacts created or modified:** `process.asm`; `syscall.asm`.
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P2.09`.
5. **Host-side static checks:** Exact PROC1 size/offsets; only ALLOW_TAPE bit0 accepted; reserved u16 zero; structural handle values limited to 0..7; capacity scan covers exactly PID2..7; full-table return precedes PROC1 dereference; capacity-available preflight validates the complete record, path and pointed ranges; no preflight executable lookup, RAM-object/BCAT lookup, cassette access, allocator call, process reservation, publication, or live parent-handle lookup.
6. **Emulator test artifact:** Deterministic SNA fixtures using the exact production preflight code. A full-table fixture supplies a deliberately invalid/poison PROC1 pointer and must still return E_AGAIN without dereferencing it. Capacity-available fixtures exercise valid and malformed PROC1 records and pointed ranges without inventing BCAT state or requiring executable resolution.
7. **Exact FUSE assertions/checkpoints:** Through the real `SYS_SPAWN` entry, a full PID2..7 table returns E_AGAIN before PROC1 dereference and leaves process/allocator/open-description persistent state unchanged. With capacity available, the exact production preflight accepts a fully valid PROC1 to its no-side-effect PASS boundary and rejects malformed records/ranges with the ABI-specified error before any side effect. ALLOW_TAPE=0 preflight remains cassette-inert. No assertion may claim that a catalog-only executable was classified at P2.09.
8. **Expected PASS result:** Full table returns E_AGAIN before request dereference with persistent state unchanged. With capacity available, structurally valid PROC1 reaches the no-side-effect preflight PASS boundary; malformed record, flags, reserved bytes, handle values or pointed ranges are rejected before allocation or cassette activity. P2.09 makes no RAM/BCAT executable-resolution claim.
9. **Negative/failure test:** Deliberately move an allocation or other persistent mutation before the capacity decision and require the full-table oracle to detect the side effect even though the syscall still returns E_AGAIN. Separately, any preflight RAM-object/BCAT lookup, cassette access, live parent-handle lookup, process reservation/publication, or PROC1 dereference before a full-table E_AGAIN fails the P2.09 contract.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §6-7, §48
...
1. **Purpose / REV12 requirement:** §6, §11
...
1. **Purpose / REV12 requirement:** §6-7
...
1. **Purpose / REV12 requirement:** §21, §41.2
...
1. **Purpose / REV12 requirement:** §6, §41.2
...
1. **Purpose / REV12 requirement:** §§6.1,10.1
...
1. **Purpose / REV12 requirement:** §41.2-41.3, §48
...
1. **Purpose / REV12 requirement:** §48 acceptance
...
## R16.00 - Revision-12/03 to Revision-16/06 Phase-3 admission bridge

1. **Purpose / REV16 requirement:** Transition only after the complete Phase-2 gate. P2.24 is the last ordinary certification step under Revision 12 / Revision 03; this bridge preserves all E0.01-P2.24 evidence unchanged, implements and delta-recertifies the bounded Revision-16 EDIT/ESC change that touches already-completed Phase-1 keyboard/console code and ABI documentation, proves the patched current tree still satisfies the carried-forward Phase-1/Phase-2 prerequisites, advances the certification tooling to the new architecture/plan epoch, and is mandatory before P3.01 can begin. P3.01 is the first ordinary phase step certified against Revision 16 / Revision 07.
2. **Exact implementation work:** Perform one fail-closed transition transaction. It must (a) prove historical `docs/01-ZX-UX-ARCHITECTURE-REV12.md` and `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV03.md` still match Git blob IDs `56f509189de369038258cca0f61ae0f59ddd4a04` and `173c82c30c3787a0a020833b49e3e486e570840e`, SHA-256 `a90d523f62a95e8cba6af0312b596a2d5f6bc1aa2ef92f39bb391509b7c15e1b` and `067d96de9a8154d7f6e01bd5b96a3f35a9319a6c046f19125aea361182f055f7`, with canonical LF bytes; (b) prove the retained E0.01-P2.24 dependency chain, `v1/dist/certification/P2.24.build.json`, `v1/dist/certification/P2.24.test.json`, and `v1/dist/certification/phase-2.json` aggregate evidence are internally consistent and PASS under that historical identity, without modifying or relabeling them; (c) prove current `docs/01-ZX-UX-ARCHITECTURE-REV16.md` hashes exactly to `24fe9d206c2f05bbc24f11544a5cb5a0b6ab104e9fc52e654a10c2d9734b008c`, prove this Revision-07 plan names that exact architecture identity, compute the current SHA-256 of canonical `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV07.md`, and freeze that computed plan digest in every R16.00 durable record and especially `R16.00.result.json` without embedding a self-referential plan hash in this document; (d) generate complete machine-readable REV12->REV16 and REV03->REV07 delta reports against those exact historical bytes and prove every historical E0.01-P2.24 step body in REV07 is byte-identical to REV03; after normalizing only mechanical authority references from REV12/Revision 12 to REV16/Revision 16, prove the complete existing-step plan delta is substantive only in `P3.01`, `P3.03`, `P3.21`, `P9.05`, `P9.15`, `P9.23`, `P9.24`, `P11.24`, and `P11.48`, plus the new `R16.00` step and transition/traceability tables required by those changes; any other substantive existing-step or unrelated plan-semantic delta fails the bridge; (e) classify the only allowlisted pre-P3 **architecture/target-behavior** impact of Revision 16 as the EDIT/ESC delta to behavior historically owned by P1.15, P1.31, P1.34 and the P1.41 aggregate, plus the corresponding `v1/docs/abi.md` public-contract mirror; any other pre-P3 architecture, target-behavior, ABI, oracle, or evidence-strength delta fails the bridge and requires replay from the earliest affected historical step; (f) separately classify the only allowlisted **transition-tooling** changes as `tools/scripts/verify-environment.py`, `v1/tools-host/test-driver/run.py`, `v1/tools-host/test-driver/driver_core.py`, `v1/tools-host/test-driver/evidence.py`, new `v1/tools-host/test-driver/revision16_bridge.py`, `v1/docs/test-plan.md`, `v1/dist/certification/README.md`, transition-only fixtures, and R16.00 evidence; prove those diffs are limited to the REV16 architecture identity switch, REV07 plan-identity recording/enforcement, R16.00 registration/execution, prospective step-ID/evidence validation through P12, non-admitting regression mode, and tests for those changes—any unrelated tooling behavior change fails the bridge; (g) update the current kernel input path so IM2 recognizes only exact `CAPS SHIFT + SPACE` BREAK, ordinary task-context tty decoding maps only exact `CAPS SHIFT + 1` `EDIT` to target byte `0x1B`, tty32 and tty64 agree, no BASIC EDIT/extended-mode token leaks, `CAPS SHIFT + SYMBOL SHIFT` is not an ESC alias, `SYS_CON_GETKEY` returns H=0,L=0x1B for EDIT to the tty owner, BREAK remains cancellation and never returns 0x1B, and `v1/docs/abi.md` states those exact public input semantics without assigning output-control meaning to `0x1B`; (h) delta-recertify those semantics with raw-matrix, owner/non-owner, blocked-input wake, cursor-safe-service, IY-restoration, ABI-documentation consistency, and ISR call-graph assertions equivalent in strength to the affected historical Phase-1 gates; (i) after applying the complete bridge source/documentation/tooling patch, run a **non-admitting current-tree regression** of the full functional/static/format/size/emulator/negative/rollback assertion sets owned by the P1.41 and P2.24 aggregate build/test suites against the patched tree, excluding only their historical admission-identity assertions for historical architecture/plan/source-commit/evidence identities because those immutable identities are verified separately under (a)-(b); no functional, format, size, emulator, negative, rollback, or behavioral oracle may be skipped; record all regression assertions/hashes inside R16.00 scratch/test evidence rather than overwriting, regenerating, or relabeling any historical P1/P2 evidence; both aggregate assertion sets must remain PASS on the patched current tree before Phase 3 may be admitted; (j) advance `tools/scripts/verify-environment.py` and `driver_core.py` from the REV12 architecture path/hash to the exact REV16 architecture identity for R16.00 and later certification source state, while leaving historical E0/P0/P1/P2 evidence untouched; extend `evidence.py` prospectively so its step-ID grammar accepts `R16.00` and Phase IDs through P12, and so `implementation_plan_sha256` is required and validated for R16.00 and every new P3-P12 durable record while retained historical E0-P2 records remain valid without that new field; later-record validation must compare the field and the current REV07 file hash to the digest frozen in `R16.00.result.json`; (k) because the bridge changes certification tooling, run the current REV16 environment verifier and the E0.01 manifest-digest negative behavior, rerun the E0.03 root-relocation/argv-list/hard-timeout runner self-tests, and rerun the E0.04 evidence-validator positive/negative protocol against the patched tooling, including exact consistency of `v1/docs/test-plan.md` and `v1/dist/certification/README.md` with the prospective `implementation_plan_sha256`/R16.00 schema, rejection of an invalid `R16.00` step ID form, a missing/wrong prospective plan hash, and a later record whose plan hash/current REV07 bytes differ from the bridge result; record these only as R16.00 regression subresults, never as replacement historical E0 evidence; (l) prove prospective ownership continues through P3.03/P3.21, P9.05/P9.15/P9.23-P9.24, P11.24/P11.48, the architecture coverage/invariant/mandatory-test ledgers, and P12.26 release replay; and (m) use the Section-2 two-stage admission transaction: certify one clean bridge source-candidate commit, then admit only the frozen R16.00 records in a separate evidence-only commit, with no record attempting to embed the hash of the commit that contains itself.
3. **Files/artifacts created or modified:** `v1/src/kernel/interrupt.asm`; `v1/src/kernel/keyboard.asm`; `v1/src/kernel/console.asm`; `v1/src/kernel/syscall.asm`; `v1/docs/abi.md`; `tools/scripts/verify-environment.py`; `v1/tools-host/test-driver/run.py`; `v1/tools-host/test-driver/driver_core.py`; `v1/tools-host/test-driver/evidence.py`; new `v1/tools-host/test-driver/revision16_bridge.py`; `v1/docs/test-plan.md`; `v1/dist/certification/README.md`; transition keyboard/console/tooling/current-tree-regression fixtures and R16.00 evidence. Historical REV12, REV03, E0/P0/P1/P2 evidence, and Phase-0/1/2 aggregate manifests are read-only inputs; historical-only `foundation_rr.py` identity constants are not rewritten merely to make old E0 records look current.
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step R16.00`.
5. **Host-side static checks:** Exact old/new document identities; historical evidence/source-commit/prerequisite consistency through P2.24; byte-identical REV03 step bodies through P2.24; complete normalized REV03->REV07 delta restricted to the nine named substantive prospective step bodies plus R16.00/required transition tables; separate complete allowlists for the target/ABI delta and transition-tooling delta; exact `v1/docs/abi.md` mirror of the new `SYS_CON_GETKEY`/BREAK semantics; exact REV16 architecture source-state identity in the current verifier/driver; prospective REV07 digest recording/enforcement without self-reference; exact `v1/docs/test-plan.md` and `v1/dist/certification/README.md` agreement with the prospective evidence schema; evidence validator accepts R16.00/P0-P12 syntax as appropriate and preserves historical records while requiring the plan digest for R16.00/P3-P12; no mutation/relabeling of historical evidence; no unowned pre-P3 REV16 semantic change; no unrelated verifier/runner/evidence behavior change; no ISR-to-ROM keyboard/decoder edge; current-tree P1.41/P2.24 regression results are recorded only under R16.00.
6. **Emulator test artifact:** Generate/run a deterministic SNA transition fixture exercising raw keyboard rows, IM2 BREAK sampling, task-context EDIT decode, tty32/tty64 parity, `SYS_CON_GETKEY`, blocked-input wake/cursor service, tty ownership, and IY preservation; additionally execute the complete P1.41 and P2.24 aggregate emulator suites in R16.00 non-admitting regression mode against the patched current tree.
7. **Exact FUSE assertions/checkpoints:** Only `CAPS SHIFT + SPACE` sets `break_pending`; `CAPS SHIFT + 1` never sets it and yields exactly `0x1B` only in ordinary task-context owner input; no other chord yields `0x1B`; tty32/tty64 agree; BREAK never appears as `0x1B`; no BASIC EDIT token is exposed; H=0 on successful `SYS_CON_GETKEY`; non-owner cannot steal input; due cursor parity is serviced at the first safe wake/return point without changing row/column/wrap state; IY returns 0x5C3A; every non-identity functional/static/format/size/emulator/negative/rollback P1.41 and P2.24 regression assertion still passes on the patched current tree, while historical architecture/plan/source/evidence identities remain checked only against the immutable records under (a)-(b).
8. **Expected PASS result:** The transition ends exactly with `ZX-UX REV16 PHASE-3 BASELINE BRIDGE PASS`; the result records immutable REV12/REV03 identities, exact REV16 architecture identity, the computed canonical REV07 SHA-256, the certified historical P2.24 source commit and Phase-2 PASS, the new bridge source commit/candidate identity, the four affected historical owner IDs `P1.15,P1.31,P1.34,P1.41`, the exact target/ABI delta, the exact nine-step prospective plan-delta allowlist, the separate transition-tooling delta, `v1/docs/abi.md` consistency, evidence-protocol documentation consistency, current-verifier/E0.01/E0.03/E0.04 tooling-regression results, full patched-tree P1.41/P2.24 non-admitting regression results, transition test hashes, the exact clean bridge source-candidate commit certified by all three records, and an explicit statement that historical evidence remains historical while R16.00 alone certifies the bounded delta, current-tree prerequisite regression, and the REV16/REV07 identity pair before Phase 3.
9. **Negative/failure test:** Independently mutate one historical document byte, one retained evidence identity, remove/red P2.24 or the Phase-2 aggregate, change the current REV16 digest, change one REV07 byte after the bridge result, introduce any unallowlisted pre-P3 architecture/target/ABI/oracle delta, introduce an unallowlisted substantive REV07 step/table delta, introduce an unrelated verifier/test-driver/evidence change, omit `driver_core.py`'s REV16 source-state switch, make `evidence.py` reject valid R16.00 or accept a prospective record with a missing/wrong plan hash, leave `v1/docs/test-plan.md` or `v1/dist/certification/README.md` stale/contradictory for the new evidence schema, attempt to make an R16.00 record's certified source identity depend on the hash of the evidence-only commit containing that record, omit or contradict the `v1/docs/abi.md` EDIT/BREAK contract, accept EDIT as BREAK, decode EDIT in IM2, map BREAK/another chord to `0x1B`, make tty32/tty64 disagree, expose a BASIC EDIT token, permit non-owner input theft, omit the verifier baseline switch, make the E0.01/E0.03/E0.04 tooling regression fail, improperly require historical aggregate replay to satisfy current REV16/REV07 identity instead of its historical identity, skip any non-identity P1.41/P2.24 current-tree regression assertion, or make any such regression assertion fail; each must fail the bridge for the intended reason.
10. **Check-in gate:** Historical P2.24 and Phase-2 aggregate evidence PASS under exact REV12/REV03. Create one clean bridge source-candidate commit containing only the intended bridge kernel/documentation/tooling/tests changes; then, against that exact commit, bridge build + static + targeted emulator + changed-tooling regression + full non-admitting P1.41/P2.24 current-tree regression + all negative tests PASS under exact REV16 and the computed current REV07 identity. No historical doc/evidence bytes change. Scratch R16.00 records identify that exact source-candidate commit and freeze both current document identities. Admit only those exact records in a subsequent evidence-only commit; no durable record embeds or claims the hash of the commit that contains itself. End clean. Phase 3 is forbidden until the evidence-only R16.00 admission commit exists.
11. **Evidence:** Retain `v1/dist/certification/R16.00.build.json`, `v1/dist/certification/R16.00.test.json`, and `v1/dist/certification/R16.00.result.json`. All three contain `implementation_plan_sha256` and identify the same clean bridge source-candidate commit; the test/result records contain the non-admitting P1.41/P2.24 and E0.01/E0.03/E0.04 regression subresults. No regression run may write a historical step/phase evidence path. This result is the sole durable transition decision; it supplements, never rewrites, the historical E0-P2.24 evidence and is the plan/architecture identity anchor for every later certification.
...
1. **Purpose / REV16 requirement:** §§6.3,11.1,49; first ordinary Phase-3 step after mandatory R16.00 transition PASS
...
10. **Check-in gate:** `v1/dist/certification/R16.00.result.json` exists and records PASS on the same REV16 architecture identity **and** the same REV07 implementation-plan SHA-256 as the current canonical plan; P3.01 build/test evidence contains that exact `implementation_plan_sha256`; build + static + positive + negative tests PASS; dependent historical and transition gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV16 requirement:** §11.1
...
1. **Purpose / REV16 requirement:** §§11,13,32
2. **Exact implementation work:** Bind console read/write/ioctl through the generic handle layer without byte translation. A `/dev/tty` read must expose the exact ordinary tty target byte stream: physical `CAPS SHIFT + 1` `EDIT` yields `0x1B` exactly as `SYS_CON_GETKEY` does, tty32/tty64 agree, and BREAK follows cancellation rather than returning `0x1B`.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** READ/WRITE through handles equal the direct console contract byte-for-byte, including `/dev/tty` returning `0x1B` for physical `EDIT` and never translating BREAK into ESC.
9. **Negative/failure test:** Wrong direction/invalid handle errno, any `/dev/tty` byte translation, loss/change of `0x1B`, or BREAK returned as `0x1B` fails.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV16 requirement:** §32
...
1. **Purpose / REV16 requirement:** §11
...
1. **Purpose / REV16 requirement:** §11.3, §49
...
1. **Purpose / REV16 requirement:** §11, §49
...
1. **Purpose / REV16 requirement:** §§12.1,49
...
1. **Purpose / REV16 requirement:** §12
...
1. **Purpose / REV16 requirement:** §§12.1-12.2
...
1. **Purpose / REV16 requirement:** §§9.2,10.1,11.5
...
1. **Purpose / REV16 requirement:** §10.1, §49
...
1. **Purpose / REV16 requirement:** §49 acceptance
...
1. **Purpose / REV16 requirement:** §10.1, §13.5, §49
...
1. **Purpose / REV16 requirement:** §49 acceptance
2. **Exact implementation work:** Run handle limits, independent/shared state, exact `/dev/tty` byte-preserving input including `EDIT` -> `0x1B` with BREAK separation, pipe block/wake/EOF/E_PIPE, bounded stress, and the exact §41.11.6 user-range/zero-count vectors.
...
9. **Negative/failure test:** Any timeout, `/dev/tty` ESC/BREAK translation defect, or widened-range/zero-count defect blocks gate.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV16 requirement:** §18
...
1. **Purpose / REV16 requirement:** §18, §27.3
...
1. **Purpose / REV16 requirement:** §§10.1,11.2
...
1. **Purpose / REV16 requirement:** §§11.1-11.2,27.3
...
1. **Purpose / REV16 requirement:** §§11.4-11.6,18
...
1. **Purpose / REV16 requirement:** §§11.5,18,31
...
1. **Purpose / REV16 requirement:** §11, §41.3A
...
1. **Purpose / REV16 requirement:** §10.1
...
1. **Purpose / REV16 requirement:** §18, §4.3B
...
1. **Purpose / REV16 requirement:** §10.1, §18
...
1. **Purpose / REV16 requirement:** §27.3
...
1. **Purpose / REV16 requirement:** §§11.4,27.3
...
1. **Purpose / REV16 requirement:** §27.3, §41.3A
...
1. **Purpose / REV16 requirement:** §10.1, §27.3
...
1. **Purpose / REV16 requirement:** §18.5, §27.3
...
1. **Purpose / REV16 requirement:** §4.4, §27.3
...
1. **Purpose / REV16 requirement:** §7, §27.3
...
1. **Purpose / REV16 requirement:** §10.1, §33
...
1. **Purpose / REV16 requirement:** §41.3A
...
1. **Purpose / REV16 requirement:** §18.6, §32; invariants 108/115
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All three listings, types, states, zero lengths, ordering and parent IDs match REV16 byte-for-byte.
...
1. **Purpose / REV16 requirement:** §10.1, §18, §50
...
1. **Purpose / REV16 requirement:** §50 acceptance
...
1. **Purpose / REV16 requirement:** §19, §51
...
1. **Purpose / REV16 requirement:** §19
...
1. **Purpose / REV16 requirement:** §19, §27.3
...
1. **Purpose / REV16 requirement:** §5.1, §51
...
1. **Purpose / REV16 requirement:** §19.7, §29
...
1. **Purpose / REV16 requirement:** §19, §20
...
1. **Purpose / REV16 requirement:** §7, §19, §27.3
...
1. **Purpose / REV16 requirement:** §51 acceptance
...
1. **Purpose / REV16 requirement:** §51
...
1. **Purpose / REV16 requirement:** §40.3
...
1. **Purpose / REV16 requirement:** §5.5, §20, §41.1
...
1. **Purpose / REV16 requirement:** §20.1
2. **Exact implementation work:** tty64 displays `/etc/issue` exactly once, with target lines exactly `ZX-UX - Inspired by Unix for the Sinclair ZX Spectrum 48K<LF>`, `64-column shell, native tools, C compiler, cassette storage<LF>`, and `48K. One Z80. No excuses.<LF>`; it does not separately print a duplicate heading, then prompts exactly `login: ` as specified by REV16 §20.1.
...
1. **Purpose / REV16 requirement:** §20.2
...
1. **Purpose / REV16 requirement:** §20.2A
...
1. **Purpose / REV16 requirement:** §20.3
...
1. **Purpose / REV16 requirement:** §20.4, §52
...
1. **Purpose / REV16 requirement:** §20.7
...
1. **Purpose / REV16 requirement:** §18, §20.7
...
1. **Purpose / REV16 requirement:** §20.5, §33, §52
...
1. **Purpose / REV16 requirement:** §20.5, §31, §52
...
1. **Purpose / REV16 requirement:** §20.3, §31
...
1. **Purpose / REV16 requirement:** §20.12
...
1. **Purpose / REV16 requirement:** §13.5, §20.12
...
1. **Purpose / REV16 requirement:** §20.4, §20.12
...
1. **Purpose / REV16 requirement:** §§20.12,21; invariants 99/111
...
1. **Purpose / REV16 requirement:** §13, §20.12
...
1. **Purpose / REV16 requirement:** §52 acceptance
2. **Exact implementation work:** Freeze host parser/AST and target execution vectors for quoting/operators/expansion/PATH/builtins/case. Golden vectors are derived from REV16 userland contracts only; no host-shell quoting, field-splitting, globbing, substitution, precedence, PATH, environment or parent-builtin behavior may be borrowed implicitly.
...
9. **Negative/failure test:** Any unsupported construct accidentally accepted fails. Run divergence vectors chosen to behave differently in common host shells; ZX-UX must follow REV16, not the host.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV16 requirement:** §20.12; invariants 110/118
...
1. **Purpose / REV16 requirement:** §52 acceptance
2. **Exact implementation work:** Aggregate login/core parser, echo pipe, PATH matrix, builtin E_NOTSUP in pipe/bg, redirection rollback, child errors and mixed-case behavior, plus REV16 shell-facing time rows: normal boot begins with valid revision-0 launch epoch; no normal `date: not set`; diagnostic invalid fixture remains explicit; date -s revision semantics and boot-valid cron/cal expectations remain delegated to their owning utility gates.
...
1. **Purpose / REV16 requirement:** §10.1, §15, §53
...
1. **Purpose / REV16 requirement:** §3.3, §15
...
1. **Purpose / REV16 requirement:** §15.1, §15.3
...
1. **Purpose / REV16 requirement:** §15.1, §15.4
...
1. **Purpose / REV16 requirement:** §15
...
1. **Purpose / REV16 requirement:** §§10.1,17
...
1. **Purpose / REV16 requirement:** §§14.6,20.6
...
1. **Purpose / REV16 requirement:** §14.7, §20.6
...
1. **Purpose / REV16 requirement:** §20.6
...
1. **Purpose / REV16 requirement:** §§4.3,14.8A,16.1-16.2
...
1. **Purpose / REV16 requirement:** §§10.1,16.1,16.3
...
1. **Purpose / REV16 requirement:** §16.2-16.3
...
1. **Purpose / REV16 requirement:** §16.5
...
1. **Purpose / REV16 requirement:** §16.4, §53
...
1. **Purpose / REV16 requirement:** §17, §53
...
1. **Purpose / REV16 requirement:** §53 acceptance
...
1. **Purpose / REV16 requirement:** §§17,20.5-20.7,53; invariant 78
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (ls)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/ls contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (cat)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/cat contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (cp)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/cp contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (mv)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/mv contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (rm)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/rm contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (pack)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/pack contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (unpack)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/unpack contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (grep)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/grep contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (wc)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/wc contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (head)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/head contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (tail)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/tail contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (cmp)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/cmp contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (true)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/true contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (false)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/false contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (sleep)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/sleep contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (which)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/which contract; no case folding. `which ls` can report the exact external path; builtin-only name emits no bytes and status 1; catalog-only lookup does not move cassette.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (env)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/env contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (hexdump)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/hexdump contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.4,33,54 (ps)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Builtin behavior and exit status match exact REV16 Section-33/ps contract; no case folding; no external `/bin/ps` object or BCAT slot exists.
...
1. **Purpose / REV16 requirement:** §§20.4,33,54 (mem)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Builtin behavior and exit status match exact REV16 Section-33/mem contract including `-c`; no case folding; no external `/bin/mem` object or BCAT slot exists.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (udg)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/udg contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (gfxdemo)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/gfxdemo contract; no case folding.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/stty contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (date)
2. **Exact implementation work:** Implement external `date` on the REV16 wall clock. Normal successful boot reads valid TIME1 and prints exact `YYYY-MM-DD hh:mm:ss` derived from the current launch-epoch-based software clock. `date -s "YYYY-MM-DD hh:mm:ss"` validates Gregorian 1970..2099, calls SYS_TIME_SET, increments revision on every successful set, and resets subsecond. Exact literal `date: not set` is retained only for explicit diagnostic invalid-state tests, never the normal production boot oracle.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/date contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (cron)
2. **Exact implementation work:** Implement `cron` as the tiny cooperative user daemon with the full REV16 §20.9 contract. Calendar/@hourly/@daily are eligible immediately from valid revision-0 boot time; `@boot` remains wall-time-independent and once per daemon instance. Poll/reload, serial execution, shell-subset restrictions, /dev/null and /dev/tty handles, ALLOW_TAPE=0, no catch-up, and 50-tick sleep remain exact. Diagnostic invalid TIME1 suppresses calendar rules only in the explicit invalid fixture.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (crontab)
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (man)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/man contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (cal)
2. **Exact implementation work:** Implement `cal` arity/range exactly. On normal successful boot no-argument/one-argument forms use the already-valid launch-epoch-based wall clock; `cal: date not set` is diagnostic-invalid-state behavior only. Two-argument month/year works independently for 1970..2099; all other arity/ranges E_INVAL.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/cal contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (uptime)
2. **Exact implementation work:** Format independent 32-bit SYS_TICKS modulo 2^32. `uptime` must be unaffected by the launch-epoch wall seconds, TIME1 revision, or diagnostic valid/unset state and must prove tick wrap independently.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/uptime contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (whoami)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/whoami contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (uname)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/uname contract; no case folding. Output bytes are exactly ASCII `ZX-UX z80 48k` followed by one LF byte (0x0A), with no legacy `ZX48-UX` spelling.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (fortune)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/fortune contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (banner)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/banner contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (rev)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/rev contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (yes)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/yes contract; no case folding.
...
1. **Purpose / REV16 requirement:** §§20.5,33,54 (demo)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV16 Section-33/demo contract; no case folding.
...
1. **Purpose / REV16 requirement:** §54 acceptance
...
1. **Purpose / REV16 requirement:** §20.9, §41.7; invariants 105/109/119
2. **Exact implementation work:** Implement `/tmp/.cron.lock` exactly plus TIME1 dedupe. Calendar/@hourly/@daily key is exactly `(wall-minute,TIME1.revision)`, beginning with valid revision 0 on normal boot; any successful date -s changes revision, clears dedupe, and evaluates only the newly observed current minute with no catch-up. Diagnostic invalid state suppresses calendar evaluation without changing @boot semantics.
...
1. **Purpose / REV16 requirement:** §54 acceptance
2. **Exact implementation work:** Run every Section-54 command in architecture-defined form plus the revised time matrix: boot-valid `date`, diagnostic invalid-state date/cal only in dedicated fixtures, valid/invalid date -s including revision increment, boot-valid calendar cron eligibility at revision 0, @boot independence, and independent uptime. Preserve final external pipeline and builtin ps/mem regressions.
...
1. **Purpose / REV16 requirement:** §22, §55
...
1. **Purpose / REV16 requirement:** §22
...
1. **Purpose / REV16 requirement:** §22.6
...
1. **Purpose / REV16 requirement:** §22
2. **Exact implementation work:** Implement exactly three unambiguous modes: normal, insert and command-line. Normal mode uses a block cursor. Insert mode uses an underline cursor and displays the exact literal `-- INSERT --` on tty row 23 (the status/command line). `:` command-line mode uses an underline cursor. Target byte `0x1B`, supplied by the Section-13.8 physical `CAPS SHIFT + 1` `EDIT` chord, is the canonical editor escape: from insert it returns to normal, removes the exact indicator, restores the block cursor, inserts no byte, and changes no buffer bytes merely by escaping; from command-line it cancels the unfinished command without executing it and returns to normal; in normal it cancels any incomplete multi-key normal command and otherwise remains a no-op on buffer bytes. BREAK is not editor escape. Mode transitions update the cursor/status atomically with the console cursor-removal/redraw discipline.
3. **Files/artifacts created or modified:** vi.asm
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P9.05`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Golden mode transitions prove normal=block, insert=underline plus exact `-- INSERT --` on row 23, command-line=underline, and `0x1B`/`EDIT` returns insert or unfinished command-line input to normal with exact status cleanup and no unintended buffer mutation; normal-mode `0x1B` cancels incomplete multi-key state; BREAK remains cancellation. Cursor/state survive viewport redraws.
9. **Negative/failure test:** Any fourth/ambiguous mode, wrong cursor shape, altered/missing insert literal, indicator outside row 23, stale indicator after leaving insert, inability of `EDIT`/`0x1B` to escape, treating BREAK as escape, executing a cancelled command, inserting `0x1B` into the buffer, retaining incomplete normal-command state after escape, or invalid command that mutates the buffer/status contract fails.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV16 requirement:** §22
2. **Exact implementation work:** Implement required forward literal `/text` search; `n` repeats in the same direction and `N` repeats in the opposite direction. Search is case-sensitive. Interactive search-text entry uses the command-line input state; target byte `0x1B` from physical `EDIT` cancels an unfinished search, returns to normal, and preserves the previous search state and buffer. BREAK remains cancellation. No `?` reverse-search command is invented for version 1.
3. **Files/artifacts created or modified:** vi.asm
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P9.15`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Cursor/search state matches the exact required `/text`, n and N semantics; `EDIT`/`0x1B` cancels unfinished search entry without replacing prior search state or changing buffer bytes; unsupported `?` is not accepted as a hidden vi extension.
9. **Negative/failure test:** Not-found leaves state per contract; cancelled search execution/state replacement, treating BREAK as search escape, or a test expecting `?` reverse-search support must fail because it is outside the version-1 required subset.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV16 requirement:** §20.9, §55 acceptance
...
1. **Purpose / REV16 requirement:** §22.5, §22.6A
...
1. **Purpose / REV16 requirement:** §22.6A, §41.8
...
6. **Emulator test artifact:** N/A: this is a host-only contract/format/policy step; target behavior is closed by later dependent steps.
7. **Exact FUSE assertions/checkpoints:** N/A except where the host test deliberately launches a timeout/argv self-test.
8. **Expected PASS result:** `docs/vi.md` contains every required REV16 vi contract with no invented command or stronger claim than the tests prove; it documents physical `EDIT`/`0x1B` escape and BREAK separation exactly; it names `:udg` as deferred, lists all eight §22.8 deferred feature families exactly, and states that any temporary line-oriented bootstrap editor is not the shipped editor and cannot replace the `vi` acceptance gate.
...
1. **Purpose / REV16 requirement:** §55 acceptance, §44
2. **Exact implementation work:** Create/edit/save/reload hello.c; prove physical `EDIT`/`0x1B` escape from insert and cancellation of unfinished `:`/`/` input with BREAK remaining distinct; case-sensitive p/P,o/O,n/N,g/G; failed growth/write rollback; ex commands; crontab -e; vi <=8192 or measured approved exception.
...
1. **Purpose / REV16 requirement:** §23, §56
...
1. **Purpose / REV16 requirement:** §23
...
1. **Purpose / REV16 requirement:** §56
...
1. **Purpose / REV16 requirement:** §23, §38
...
1. **Purpose / REV16 requirement:** §23, §38, §56
...
1. **Purpose / REV16 requirement:** §23
2. **Exact implementation work:** Serialize deterministic OBJ1 from the fully validated assembler result. Symbol-record order is an implementation policy frozen by deterministic native-assembler tests and is NOT an architecture-prescribed sort order; relocation records MUST obey REV16's strictly increasing, non-overlapping offset rule. Compute the OBJ1 body/header CRCs over the exact stored bytes and run the independent host inspector before transactional publication.
3. **Files/artifacts created or modified:** as.asm
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P10.18`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Two identical native assemblies produce byte-identical OBJ1; the host inspector validates all records/offsets/CRCs and relocation records are strictly increasing/non-overlapping exactly as REV16 requires.
9. **Negative/failure test:** Any malformed symbol/relocation record, relocation-order/overlap violation, CRC mismatch, nondeterministic output for identical input, or documentation/test that falsely claims REV16 mandates symbol-table sorting fails before commit.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV16 requirement:** §24
...
1. **Purpose / REV16 requirement:** §24, §41.2A
...
1. **Purpose / REV16 requirement:** §24, §26, §41.2A
...
1. **Purpose / REV16 requirement:** §24, §56
...
1. **Purpose / REV16 requirement:** §56 acceptance
...
1. **Purpose / REV16 requirement:** §56, §44
...
## R17.00 - Revision-17 / Revision-08 authority transition

1. **Purpose / REV17 requirement:** Establish the sole post-P10.36 authority bridge before P11.01 without changing or recertifying completed Phase-10 evidence.
2. **Exact implementation work:** Bind the exact SHA-256 identities of `docs/01-ZX-UX-ARCHITECTURE-REV17.md` and `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV08.md`; prove REV16/REV07 and every admitted P0-P10 evidence/media byte remain unchanged; activate epoch-aware authority routing so historical E0-P2 stays REV12/REV03, R16.00/P3-P10 stays REV16/REV07, and R17.00/future P11-P12 uses REV17/REV08. Validate the prospective TZX-only release contract and native self-rebuild requirements without starting P11.
3. **Files/artifacts created or modified:** R17 bridge validator/qualification/admission/activation machinery, authority-routing selectors, current authority documentation, and R17.00 evidence only.
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step R17.00`.
5. **Host-side static checks:** exact REV17/REV08 hashes; exact unchanged REV16/REV07 hashes; unchanged P0-P10 evidence/media; no P11 state; TZX-only/no-SCREEN$/no-release-TAP current contract; Pasmo excluded from shared toolchain.
6. **Emulator test artifact:** Use only the smallest deterministic existing prerequisite artifact required to prove the authority transition does not alter completed runtime semantics. This bridge is not Phase-11 implementation.
7. **Exact FUSE assertions/checkpoints:** R17.00 may replay already-admitted runtime checkpoints non-admittingly, but must not create new P11 evidence. Release-loader real-time acceptance is closed later by the replacement P10 pre-release workflow.
8. **Expected PASS result:** exact `ZX-UX REV17 POST-PHASE10 AUTHORITY BRIDGE PASS`; REV17/REV08 become active; historical epochs remain byte-identical; no Phase-11 state exists.
9. **Negative/failure test:** mutate REV17, REV08, REV16, REV07, any historical evidence/media identity, route a P10 record to REV17, route a future record to REV16, omit/wrong bridge marker, or introduce any P11 source/workflow/evidence/media; each must fail closed.
10. **Check-in gate:** three unchanged candidate scans, license/project-policy/media-retention gates, authority/evidence positive+negative suites, exact-head regression, qualification/admission/activation validation, clean worktree.
11. **Evidence:** Retain `v1/dist/certification/R17.00.build.json`, `R17.00.test.json`, and `R17.00.result.json`; these records bind exact REV17/REV08 and never rewrite P10 evidence.

### Post-R17 pre-release migration acceptance contract

Before P11.01 is allowed to become current work, the replacement Phase-10 pre-release workflow must complete the non-phase release migration and then stop. It must:

- freshly rebuild the canonical host kernel with the pinned host path and require exactly 8192 bytes;
- build deterministic TZX-only fast-loader output with no production SCREEN$/`zx48uxscr` and no release TAP;
- independently derive/verify turbo-block integrity and reconstruct the embedded kernel from the finished TZX;
- boot the real TZX in real time with Fuse fastload/traps/loader shortcuts disabled and prove exact E003 handoff;
- use booted ZX-UX native `as` to parse source and emit real OBJ1, then native `ld` to consume that OBJ1 and produce a fixed/absolute kernel image in non-executing storage;
- compare every byte 0..8191 across host-built, TZX-embedded, and native-rebuilt kernels and record all three SHA-256 values;
- separately prove genuine `source -> native as -> OBJ1 -> native ld -> executable -> run`;
- prove a controlled source/input mutation breaks the equality proof;
- preserve historical P10.34 evidence unchanged;
- publish only the compact `P10.pre-release` TZX/manifest/README bundle after PASS; and
- verify no P11 state both before and after publication and terminate without dispatching Phase 11.
...
Z80 code and must emit the REV16 OBJ1/ABI/runtime contracts. If the SDK and REV17
differ, REV17 wins and the difference must be recorded rather than silently copied.
...
constraints remain governed by REV16.


The checked-in binary specification `docs/04-C48 Language Specification Rev 0.11.docx`
is also a mandatory Phase-11 design and acceptance input. Before compiler behavior is
implemented or accepted, review that document end-to-end alongside REV17 and the pinned
SDK Python implementation (`compiler/c48.py` and the relevant modules under
`compiler/c48/`). Correct the DOCX through the normal project SoP whenever the review
finds an omission, ambiguity, contradiction, stale rule, or language-behavior defect.
REV16 remains the controlling ZX-UX architecture authority and the SDK repository
remains read-only.

For every Phase-11 step that defines or changes preprocessing, lexical, syntactic,
type-system, semantic, numeric, pointer, storage, calling-convention, runtime, error,
resource-limit, or other observable C48 compiler behavior, perform a three-way
conformance review against REV16, the applicable DOCX clauses, and the corresponding
behavior in the pinned Python SDK implementation. No discrepancy may be silently
accepted. If the DOCX conflicts with REV16, REV16 wins and the DOCX must be corrected
before the dependent step closes. If native behavior conflicts with REV16 or the
corrected DOCX, correct the native implementation/tests. If the pinned SDK differs,
do not modify the SDK from this project; record the difference, governing ZX-UX
behavior, and proving tests in the SDK provenance/conformance mapping. If the DOCX and
SDK disagree on behavior not already decided by REV16, resolve and record the intended
C48 contract in the DOCX before the dependent native implementation step closes.

Every C source authored or modified for Phase 11, including tests, demos, examples,
conformance fixtures and compiler-validation programs, MUST also be processed by the
pinned Python SDK compiler/reference in addition to required target-native validation.
Record source identity, SDK accept/reject result, target-native result, and the mapping
between them. A genuinely target-only ABI/syscall/runtime case may use an explicitly
documented host-only adapter or difference rationale, but no Phase-11 C source may be
omitted silently. P11.39 and P11.48 MUST fail if this accounting is incomplete.
...
1. **Purpose / REV17 requirement:** §25, §57
2. **Exact implementation work:** Write the exact C48 supported subset and explicitly freeze the version-1 deferred language surface rather than saying merely “not ISO C”. In addition to the supported features implemented by P11.04-P11.10, state that compound assignments, the conditional `?:` operator and the comma operator are deferred, and that the following are not version-1 C48 features unless a later architecture revision says otherwise: `double` as a distinct format from C48 `float`, `long long`, variable-length arrays, complex initializers, variadic functions, function pointers, `switch`/`case`, `struct`/`union`, complex preprocessor macros, optimizer passes requiring large IR, and full ISO conformance. Freeze data model char1/short2/int2/pointer2/float5 with exact alignments and plain-char unsigned; `long` is unsupported. Review `docs/04-C48 Language Specification Rev 0.11.docx` end-to-end against REV17 and the pinned Python SDK language specification, conformance notes, `compiler/c48.py`, and relevant implementation modules under `compiler/c48/` while writing `c48.md`. Correct the DOCX through the normal project SoP wherever that review finds an omission, ambiguity, contradiction, stale rule, or language-behavior defect. Record the SDK commit, reviewed Python implementation surface, DOCX review/corrections, every intentional REV17-over-SDK difference, and every resolved REV17/DOCX/SDK discrepancy in the SDK provenance/conformance mapping. No unresolved three-way discrepancy may remain when P11.01 closes.
3. **Files/artifacts created or modified:** `docs/04-C48 Language Specification Rev 0.11.docx` when correction is required; `v1/docs/c48.md`; `v1/tests/compiler/sdk-reference/SDK-PROVENANCE.md`; C48 specification/SDK design and conformance mapping
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P11.01`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** N/A: this is a host-only contract/format/policy step; target behavior is closed by later dependent steps.
7. **Exact FUSE assertions/checkpoints:** N/A except where the host test deliberately launches a timeout/argv self-test.
8. **Expected PASS result:** Spec checklist against §25 names every supported/deferred language family, data-model rule and operator exclusion explicitly; no generic “subset” sentence substitutes for a missing item. The evidence records end-to-end review of `docs/04-C48 Language Specification Rev 0.11.docx`, every required correction, the pinned Python SDK commit, the reviewed SDK Python implementation/design/docs, every intentional REV17-over-SDK divergence, and every identified REV17/DOCX/SDK discrepancy with its recorded resolution.
9. **Negative/failure test:** Unsupported feature accidentally claimed, missing DOCX review/correction evidence, missing SDK provenance or Python implementation coverage, an unexplained REV16/SDK contract difference, or any unresolved REV17/DOCX/SDK discrepancy is documentation failure.
10. **Check-in gate:** build + static + positive + negative tests PASS; the C48 DOCX review/correction and pinned Python SDK implementation comparison are complete with zero unresolved three-way discrepancies; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV17 requirement:** §§25.5-25.6,57
...
1. **Purpose / REV17 requirement:** §25.7, §41.9, §57
...
1. **Purpose / REV17 requirement:** §25
...
1. **Purpose / REV17 requirement:** §25, §14
...
1. **Purpose / REV17 requirement:** §10.1, §25
...
1. **Purpose / REV17 requirement:** §25, invariant
...
1. **Purpose / REV17 requirement:** §25, §37
...
1. **Purpose / REV17 requirement:** §25.8; §§9-13
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All fifteen symbols resolve from the built-in libc48 archive and behavior matches the underlying REV16 syscall contracts, including short I/O and typed creation.
...
1. **Purpose / REV17 requirement:** §25
2. **Exact implementation work:** Implement required minimal routines; use documented Z80 block primitives where measured appropriate. The exact §25.8 public surface owned here is `getchar`, `putchar`, `puts`, `strlen`, `strcmp`, `strcpy`, `strncpy`, `memcpy`, `memmove`, `memchr`, `memset`. Implement target-byte semantics, overlap-safe memmove, bounded string/memory behavior and tty wrappers without a large general-purpose `printf`. `getchar()` is byte-preserving over stdin: with stdin bound to `/dev/tty`, physical `CAPS SHIFT + 1` `EDIT` must return integer 27/`0x1B` unchanged, while BREAK remains cancellation and is never returned as 27. Document that exact behavior in `v1/docs/c48.md`, including that no C48 text-input helper case-folds, translates, swallows, or synthesizes the ESC-equivalent byte. Provide the REV16 compact integer formatter as a small libc/compiler-runtime helper and use the ROM-backed floating formatter owned by P11.46 for floating text; neither requirement authorizes a general printf implementation.
3. **Files/artifacts created or modified:** `v1/src/libc48/string.asm`; `v1/src/libc48/memory.asm`; `v1/docs/c48.md`; focused C48 runtime/documentation tests
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P11.24`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, deterministic artifact hashes as applicable, and exact `v1/docs/c48.md` consistency with the byte-preserving `getchar()`/BREAK contract.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Golden memcpy/memmove/string cases. One C matrix links and executes every one of these eleven functions, including empty strings, NUL boundaries, overlapping memmove in both directions, memchr hit/miss, tty EOF/input/output cases, and `getchar()` observing exact 27/`0x1B` for physical `EDIT` with BREAK kept separate; `v1/docs/c48.md` states the same byte-preserving rule exactly.
9. **Negative/failure test:** Overlap memmove correctness. Missing symbol, accidental host libc dependency, unsafe overlap, out-of-bounds guard mutation, translated/swallowed `0x1B`, BREAK surfaced as `getchar()==27`, or a `v1/docs/c48.md` contract that omits/contradicts the EDIT/BREAK behavior fails the step.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV17 requirement:** §25, §42
...
1. **Purpose / REV17 requirement:** §25
2. **Exact implementation work:** Expose only documented cassette/object/zxpack APIs. Exact public cassette/time surface remains `tape_save`, `tape_load`, `ticks`, `time_get`, `time_set`. `time_get` normally observes valid launch-epoch-based revision-0 TIME1 after boot, while the explicit diagnostic invalid fixture returns the frozen error; `ticks` remains independent modulo2^32; `time_set` preserves exact revision/subsecond semantics. No RTC persistence/timezone/background I/O fiction.
...
1. **Purpose / REV17 requirement:** §2.8, §38, §57
...
1. **Purpose / REV17 requirement:** §2.8, §57
...
1. **Purpose / REV17 requirement:** §57
...
1. **Purpose / REV17 requirement:** §42, §57
...
1. **Purpose / REV17 requirement:** §57 acceptance
2. **Exact implementation work:** Run the native language, ABI, float, heap, codegen, error-transaction and demos suite AND import/run the complete test corpus from the pinned Python C48 SDK. Preserve the imported SDK test files byte-for-byte where possible, record their hashes and source commit, copy all required fixtures/support, run the copied `compiler/run_tests.py`/release gate from this repository, and maintain a mechanical mapping from every imported SDK test to the target-native C48 behavior it constrains. Also perform a complete behavioral comparison of the native compiler against the corrected `docs/04-C48 Language Specification Rev 0.11.docx` and the pinned Python SDK implementation, and enumerate every C source authored or modified for Phase 11 so each is processed by the pinned Python SDK compiler/reference with source identity, SDK accept/reject result, target-native result, and language-semantic mapping recorded. Target-only ABI/syscall/runtime cases require an explicit host-only adapter or difference rationale rather than omission. No imported failure may be converted to skip/xfail or have its expectation weakened for admission.
3. **Files/artifacts created or modified:** `v1/tests/compiler/`; `v1/tests/compiler/sdk-reference/`; SDK provenance/hash/test-mapping records; complete Phase-11 C-source SDK-validation manifest; DOCX/SDK/native behavioral comparison mapping
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P11.39`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All native exact outputs/status/hashes pass with known nondeterminism=none; the complete copied SDK corpus also passes from the ZX-UX repository at the pinned test count (205 for baseline `84d144de2721cda5075c3a6610a422663b5e2f77`); every imported SDK test has a passing target-native mapping or a documented host-only adapter that still checks the original C48 intent; the DOCX/SDK/native behavioral comparison has zero unresolved discrepancies; and the Phase-11 C-source SDK-validation manifest accounts for every authored or modified C source with a recorded SDK result and target-native mapping.
9. **Negative/failure test:** One intentionally wrong native expected result must fail the harness. Deleting, skipping, xfail-marking, weakening, silently editing, or leaving unmapped any imported SDK test must also fail the compiler golden-suite gate. Missing DOCX comparison coverage, any unresolved DOCX/SDK/native discrepancy, or omitting any Phase-11 C source from the SDK-validation manifest (including a missing SDK result or target-native mapping/explicit adapter rationale) must fail the gate.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV17 requirement:** §44, §41.3
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All FAST_REQUIRED allocations are satisfied; the <=20480 live-footprint measurement is physical, placement policy matches REV16, and the memory-short path reports `not enough memory for compiler` without corruption; no paper-only total.
...
1. **Purpose / REV17 requirement:** §25, §41.9
...
1. **Purpose / REV17 requirement:** §41.9
...
1. **Purpose / REV17 requirement:** §41.9; ZXP1/object invariants
...
1. **Purpose / REV17 requirement:** §41.9
2. **Exact implementation work:** Run the complete Section-41.9 compiler matrix and the bounded H06 SDK compatibility canary. Pin read-only SDK commit `9ca3c6d6b5dd4b6e2351c1800afbd47d1d77e411`, `usr/src` tree `f629dcc1d156b83bf08ed171b9273e3cbb621ad1`, exact `usr/src/examples/hello.c` Git blob `95fa0186bda3f7636774f5885988d78a13f6450b`, 703 bytes, SHA-256 `6f94a735f230dadf5928993f9a071f3f47e98b63d18230eef102f6ae7b63e4b2`, and direct declaration dependency `usr/src/examples/exapi.h`, 2056 bytes, SHA-256 `2fa0edc593832d3ab57bc105233f41fd81a02b1f3ea022cefc42da01a6084bb8`. Compile those exact `hello.c` bytes with target-native C48 using the pinned declarations, link with native `ld`, execute under ZX-UX, and prove `cls()`, exact `print_at(10,22,"hello from c48")`, exact `print_at(12,14,"zx-ux portable host sdk")`, clean status 0, deterministic debugger/register/RAM/process/exit state, and at least one supplemental screenshot. Retain a canary manifest with source/header identities, native compile/link/execute results and evidence references; missing, changed, skipped, host-only, or non-executed canary evidence fails H06. Also run every original Section-41.9 language/runtime/RAW-vs-PACKED requirement.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All architecture compiler rows pass; the pinned SDK `usr/src/examples/hello.c` canary and `exapi.h` identities match exactly; `hello.c` compiles with target-native C48, links with native `ld`, executes under ZX-UX with the two exact screen writes and status 0, retains deterministic proof plus a supplemental screenshot, and RAW/PACKED source OBJ1 identity plus all language/runtime failure cases remain exact.
9. **Negative/failure test:** Change one canary/header byte or identity, substitute a host compiler/linker, skip native execution, alter either exact output/position/status, or omit deterministic execution proof or the supplemental screenshot; admission must fail.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV17 requirement:** §10.1, §14, §25.8-25.9
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Formatted bytes/count/status match the REV16 ABI; no partial write on insufficient/invalid destination; IY/calculator state restored.
...
1. **Purpose / REV17 requirement:** §57, §44
2. **Exact implementation work:** Aggregate the full target-native C48 lifecycle and all REV16 compiler acceptance, including the P11.24 byte-preserving `getchar()` EDIT->27/`0x1B` and BREAK-separation proof. Admission additionally requires the bounded H06 pinned-SDK canary from P11.45: exact `usr/src/examples/hello.c` plus pinned `exapi.h`, target-native compile/link/execute, deterministic proof, exact outputs/status, and supplemental screenshot. It also requires recorded end-to-end review/correction of `docs/04-C48 Language Specification Rev 0.11.docx`, complete comparison against the pinned Python SDK implementation, and the complete P11.39 manifest proving that every C source authored or modified for Phase 11 was processed by the pinned SDK compiler/reference and mapped to its target-native result or an explicit target-only adapter/difference rationale. No compiler is certified while the H06 canary/header identity is missing or modified, the canary is skipped, host-only, non-executed, or red, while any REV17/DOCX/SDK/native discrepancy is unresolved, or while any Phase-11 C source lacks the required SDK validation/mapping.
3. **Files/artifacts created or modified:** Phase11 evidence; `v1/tests/compiler/sdk-reference/`; `v1/dist/certification/P11.48-sdk-admission.json`; complete Phase-11 C-source SDK-validation manifest; DOCX/SDK/native conformance mapping
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P11.48`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Every Phase11 bullet PASS; end-to-end DOCX review/correction and every discrepancy resolution are recorded; SDK source commit/provenance and Python implementation comparison coverage are recorded; the copied SDK corpus is complete and hash-accounted; every imported SDK test passes; every SDK-derived C48 expectation has a passing target-native mapping; every C source authored or modified for Phase 11 has a recorded pinned-SDK accept/reject result and target-native mapping or explicit target-only adapter/difference rationale; and the native compiler/runtime suite passes independently.
9. **Negative/failure test:** Any missing demo/compiler feature, missing/altered SDK test, SDK test failure, skipped/xfail admission test, missing SDK provenance, missing DOCX review/correction evidence, missing Python implementation comparison coverage, unresolved REV17/DOCX/SDK/native discrepancy, unexplained SDK/REV16 difference, unmapped SDK C48 expectation, or Phase-11 C source lacking required SDK validation/mapping blocks Phase 11.
10. **Check-in gate:** build + static + positive + negative tests PASS; the complete copied SDK admission corpus, DOCX review/correction evidence, pinned Python implementation comparison, complete Phase-11 C-source SDK-validation manifest, and all target-native mappings PASS with zero unresolved discrepancies; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV17 requirement:** §33, §42, §58
...
1. **Purpose / REV17 requirement:** §4.3B, §5.1, §20.5, §58
...
1. **Purpose / REV17 requirement:** §5.1, §58
2. **Exact implementation work:** Freeze the official `issue` logical target bytes exactly as `ZX-UX - Inspired by Unix for the Sinclair ZX Spectrum 48K<LF>`, `64-column shell, native tools, C compiler, cassette storage<LF>`, `48K. One Z80. No excuses.<LF>`, with LF after every line including the final line; freeze `crontab` as exactly zero bytes RAW.
...
9. **Negative/failure test:** Restore the legacy author/blog payload, mutate a byte, remove the final LF, introduce CRLF, or append extra data; each non-exact `issue` payload is rejected.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV17 requirement:** §4.3A, §58
2. **Exact implementation work:** Re-verify the canonical `v1/assets/font4x8-zxux.bin` logical payload already frozen and consumed since P0.10. It must remain byte-identical to the preserved/read-only Tasword reference, exactly 392 bytes, and host SHA-256 exactly `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`. P12.05 MUST NOT rewrite, promote, synthesize, or substitute the font. Target code continues to validate F4X8 structure/transport CRC only; no target SHA-256 implementation is introduced.
3. **Files/artifacts created or modified:** `v1/assets/font4x8-zxux.bin` (identity verification only; no source-byte modification expected)
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P12.05`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** N/A: this is a host-only contract/format/policy step; target behavior is closed by later dependent steps.
7. **Exact FUSE assertions/checkpoints:** N/A except where the host test deliberately launches a timeout/argv self-test.
8. **Expected PASS result:** The Phase-0 canonical `v1/assets/font4x8-zxux.bin` remains byte-identical to the preserved Tasword reference at exact 392-byte length and frozen SHA-256; tty64 goldens pass without any font promotion step.
9. **Negative/failure test:** Bad glyph count, any byte/hash drift, use of `v1/assets/font4x8.bin`, or any generated/promoted substitute blocks release.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
## P12.07 - Final reference system TZX manifest/order

1. **Purpose / REV17 requirement:** §19.8, §58
2. **Exact implementation work:** Freeze the official production TZX manifest/order with the reviewed fast-loader prefix, exact 8192-byte final kernel payload, and canonical post-kernel M48O order. There is no release TAP and no SCREEN$/`zx48uxscr`. Preserve exact `font4x8`, `issue`, `crontab`, and BCAT logical contracts.
3. **Files/artifacts created or modified:** release-tzx manifest; docs
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P12.07`.
5. **Host-side static checks:** exact loader/seed identity, kernel size/range, TZX-only contract, M48O order/type/target/name, no stale release TAP/SCREEN$ dependency.
6. **Emulator test artifact:** N/A: manifest/format gate.
7. **Exact FUSE assertions/checkpoints:** N/A.
8. **Expected PASS result:** independent manifest checker matches every fast-loader and M48O entry/order contract.
9. **Negative/failure test:** swap any later object, add SCREEN$, add release TAP, use wrong kernel length/range, or wrong loader seed => release failure.
10. **Check-in gate:** full standard gates PASS.
11. **Evidence:** Retain P12.07 build/test evidence under REV17/REV08.

## P12.08 - Byte-exact final production TZX build

1. **Purpose / REV17 requirement:** §5.6, §19.8, §58
2. **Exact implementation work:** Build the final production TZX twice from identical canonical inputs using the reviewed fast-loader path. Embed the freshly rebuilt exact 8192-byte kernel; independently parse the finished TZX and reconstruct every kernel byte; derive/prove payload integrity independently; require exact deterministic identity. Emit no release TAP.
3. **Files/artifacts created or modified:** `dist/system/` final TZX and manifest.
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P12.08`.
5. **Host-side static checks:** exact seed/loader/Pasmo identities; kernel exact 8192 bytes; no dummy payload; no SCREEN$; no release TAP; structural TZX parse.
6. **Emulator test artifact:** final TZX.
7. **Exact FUSE assertions/checkpoints:** real-time load path with fastload/traps/loader shortcuts disabled; exact 0xE003 handoff and resident kernel bytes.
8. **Expected PASS result:** two builds byte-identical; independent reconstruction equals host kernel; real-time boot reaches E003; post-kernel tape position correct.
9. **Negative/failure test:** any fixture/stub/dummy kernel, stale block integrity, accelerated acceptance path, non-determinism, or release TAP blocks release.
10. **Check-in gate:** full standard gates PASS.
11. **Evidence:** Retain P12.08 build/test evidence under REV17/REV08.

## P12.09 - Final cold boot 30-step ownership/resource sequence

1. **Purpose / REV17 requirement:** §5.5, §41.1, §58
2. **Exact implementation work:** Execute the REV17 fast-loader handoff followed by the retained kernel initialization/resource sequence in order. Step 20 atomically initializes valid `TIME1` to `80 06 26 17 00 00` (1982-04-23 00:00:00, revision0, subsecond0) without touching independent SYS_TICKS; final font validation/pinning follows at its specified step; tty64 private cursor/wrap/mutation state is initialized before PID1; `EI` is step 29 and scheduler entry step 30. Retain machine-state evidence at every boundary and prove no accepted frame can observe a half-initialized wall clock.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic TAP/TZX artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All retained post-handoff ownership/resource transitions match REV17 exactly; pre-first-frame TIME1 and independent ticks are exact; final font bytes are the P12.05 identity; first EI occurs only after all required state is valid.
9. **Negative/failure test:** Delete, reorder, merge-away, or skip any checkpoint; enable interrupts before step 29; corrupt/mistype/mistarget any bootstrap resource; create persistent `/bin/sh` during bootstrap; give PID1 wrong cwd/ARG1/ENV1/tty topology/refcounts; or allow failure to fall into BASIC/uninitialized RAM: the boot gate must fail safely.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV17 requirement:** §58 items4-8, §63
...
1. **Purpose / REV17 requirement:** §58 item9
2. **Exact implementation work:** Prove the final date/cron demonstration under REV16: normal boot begins with valid launch-epoch-based wall time and revision0; retain exact `date` output bytes with companion TIME1 state; perform a valid date -s and prove revision/subsecond semantics; exercise one @boot or calendar job as applicable; separately prove the diagnostic invalid-state suppression path without pretending it is normal cold boot.
3. **Files/artifacts created or modified:** release scenario
...
1. **Purpose / REV17 requirement:** §58 item10
...
1. **Purpose / REV17 requirement:** §58 item11
...
1. **Purpose / REV17 requirement:** §58 item12
...
1. **Purpose / REV17 requirement:** §58 item13
...
1. **Purpose / REV17 requirement:** §58 item14
...
1. **Purpose / REV17 requirement:** §58 item15
...
1. **Purpose / REV17 requirement:** §58 item16
...
1. **Purpose / REV17 requirement:** §58 item17
...
1. **Purpose / REV17 requirement:** §58 item18, §42.15
...
1. **Purpose / REV17 requirement:** §19.9, §58
...
1. **Purpose / REV17 requirement:** §35.1, §44
2. **Exact implementation work:** Measure production-linked kernel stack under the complete final stress suite, explicitly including the deepest nested cursor/mutation/scroll-or-clear path with the larger ISR preservation path injected at every interruptible point. Require <=448 bytes maximum, >=64 untouched bytes, intact 0xFB00-0xFB0F guard, and deterministic PANIC KSTACK on deliberate corruption.
...
1. **Purpose / REV17 requirement:** §41
2. **Exact implementation work:** Execute every REV16 Section-41 mandatory acceptance row exactly and retain machine-state evidence. The exact §5.5 replay ledger contains **242** stable rows, including the revised boot/font/time/cursor/deferred-wrap/nesting matrix, the six widened-arithmetic rows, and all seven Section-41.9 compiler/H06 rows. Add release-wide ABI-freeze provenance: every ABI-visible number/record/format/fixed byte contract must point to an executable conformance test existing before its freeze/export. The release matrix must execute all 242 row IDs with zero missing, duplicate, merged-away, skipped, unowned, or silently conditional rows.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic TAP/TZX artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Zero unrun/skip/xfail unless REV16 itself marks a condition and evidence explains it. Exactly 242 replay row IDs map one-to-one to the §5.5 ledger and owners; all revised CR1/CR2 and H06 rows pass; widened invalid cases reject before side effects and the valid cross-boundary control succeeds; allocation failure stays recoverable; all frozen ABI fields have test-before-freeze provenance.
9. **Negative/failure test:** Delete/renumber/duplicate/merge one replay row, remove one H06 canary identity/native-execution evidence item, retain one old normal-boot unset-time oracle, omit one final-font/cursor race/deferred-wrap row, or remove conformance provenance for one frozen ABI field; completeness must fail before release certification.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV17 requirement:** §40.3
...
1. **Purpose / REV17 requirement:** §40.4
...
1. **Purpose / REV17 requirement:** §40.4, §58
...
6. **Emulator test artifact:** FUSE may be used only as regression evidence; it cannot close this hardware/analog gate.
7. **Exact FUSE assertions/checkpoints:** Not sufficient for PASS. Record real 48K-compatible hardware or hardware-faithful EAR/MIC-loop evidence.
8. **Expected PASS result:** Every item PASS; all within 48K.  The final acceptance record has exactly twenty numbered rows in REV16 order, each with target-state/evidence references, and all twenty PASS on the same official release tape and 48K candidate.
...
1. **Purpose / REV17 requirement:** §59
...
1. **Purpose / REV17 requirement:** §5.6, §19.8-19.9, §58
...
1. **Purpose / REV17 requirement:** handover Git rule
...
1. **Purpose / REV17 requirement:** §29; §41; invariants 23,25,29,32,39,55,71,92,99,111,133-135
...
1. **Purpose / REV17 requirement:** §30; invariant 55
...
1. **Purpose / REV17 requirement:** §58, §§40-44, §63
...
contract. No Revision-16 requirement may be left to implication.

| REV16 section | Primary implementation/certification coverage |
|---|---|
...
| §5 Boot Model | Historical P0-P10 bootstrap evidence remains under REV16/REV07; R17.00 establishes the TZX-only fast-loader contract; P12.07-P12.09 final TZX/boot sequence; P12.10 session handoff; P12.30 definitive boot acceptance |
...
| §13 Keyboard, Console, 64-Column Text, Cursor, and ULA Port Discipline | Historical P0 keyboard/console proofs and P1 tty64/tty32/cursor/ULA; R16.00 delta-certifies exact `EDIT`->`0x1B`/BREAK separation before Phase 3; P3.03 byte-exact `/dev/tty` read/write forwarding; P3.20 exact tty IOCTL ABI; P6 tty owner/input; P7 attribute integration; P8.23 public `stty` modes/cursor utility; P12 tty/shared-screen gates |
...
| §21 Cooperative Cancellation and kill | Historical P2 kill/started-state/lifecycle; R16.00 delta-certifies exact BREAK/EDIT separation before Phase 3; P6 foreground tty ownership/BREAK/kill/wait/jobs; P8 `yes` cooperative cancellation; P12 pipeline/background/concurrency tests |
| §22 vi Editor | P9.01-P9.24 complete modal/editor/`EDIT`-escape/transaction/documentation/size gates; P12 edit/rebuild workflow and cassette round trip |
...
| §25 Tiny Native C Compiler | P11.01-P11.20 language/preprocessor/parser/codegen/data model/REGCALL/float rules plus compiler transaction/output; P11.24 byte-preserving `getchar()` ESC input; P11 compiler golden and native lifecycle |
...
| §41 Mandatory Test Matrix | R16.00 owns the pre-Phase-3 EDIT/BREAK delta rows; owning phase tests/correction matrices close all other rows; P12.26 performs the complete Section-41 replay; P12.27 adds the compatibility-emulator pass and physical gates where evidence class requires them |
...
| §47 Phase 1 - Resident Kernel Skeleton | Historical P1.01-P1.41 retain their REV12 evidence; R16.00 delta-certifies the REV16 EDIT/BREAK keyboard/console change that touches completed Phase-1 code before Phase 3 |
...
| §61 Architecture Invariants | exact 150-row invariant ownership ledger below; owning atomic tests plus P12.26/P12.36/P12.39 close release-wide interactions |
...
REV16**, not a copied/retyped list in this document. This avoids creating a second
...
## 5.2 Explicit REV16 §61 Invariant Ownership Ledger

The invariant prose itself remains authoritative only in canonical REV16. This table is deliberately one row per numbered invariant so an audit cannot hide a missing item inside a broad category. Each listed step must contain a positive or negative proof, or an explicit scope guard, for that invariant.

| REV16 invariant | Owning implementation/test step(s) |
|---:|---|
...
| 58 | `P1.21-P1.24` |
| 59 | `P1.21,P12.05,P12.09` |
| 60 | `P6.02` |
| 61 | `P6.03-P6.04` |
| 62 | `P4.01,P4.30,P6.04` |
| 63 | `P6.05-P6.06` |
| 64 | `P1.26-P1.27,P8.24,P12.09,P12.11` |
| 65 | `P8.25,P8.39,P12.11` |
| 66 | `P6.02,P12.04` |
| 67 | `P8.27` |
| 68 | `P5.03` |
...
| 107 | `P5.09` |
| 108 | `P4.12,P4.30` |
| 109 | `P1.26-P1.27,P8.24,P8.39,P12.11` |
| 110 | `P6.29` |
| 111 | `P1.15,P1.31,R16.00,P6.26` |
| 112 | `P11.18-P11.19` |
| 113 | `P4.20,P4.26` |
...
| 119 | `P8.39` |
| 120 | `P9.02,P9.05,P9.15,P9.17-P9.19` |
...
| 133 | `P1.16,P1.30` |
| 134 | `P1.30` |
| 135 | `R16.00,P3.03,P12.26` |
| 136 | `P1.32,P7.19` |
| 137 | `P1.28,P1.33,P12.25` |
| 138 | `P2.03,P3.19,P4.16,P5.01,P10.03,P12.26` |
| 139 | `P2.03,P3.19,P4.16,P5.01,P10.03,P12.26` |
| 140 | `P1.20,P1.35-P1.39,P1.41,P12.26` |
| 141 | `P1.20,P1.35-P1.36,P1.41,P12.26` |
| 142 | `P1.24,P1.32,P1.41,P7.19,P12.26` |
| 143 | `P1.13,P1.24,P1.30,P1.32,P12.26` |
| 144 | `P1.13,P1.24,P1.32,P12.26` |
| 145 | `P1.24,P1.31-P1.32,P12.26` |
| 146 | `P1.28,P1.33,P12.25-P12.26` |
| 147 | `P1.26-P1.27,P12.09,P12.26` |
| 148 | `P1.26,P12.09,P12.26` |
| 149 | `P1.13,P8.29,P11.27,P12.26` |
| 150 | `P11.45,P11.48,P12.26` |

## 5.3 Exact REV16 §§9.2-10 Syscall ABI and Ownership Ledger
...
## 5.4 Exact REV16 §25.8 C48 Runtime Ownership Ledger

REV16 freezes a 73-function minimum public C48 runtime. This table is one row per required public symbol so none can disappear inside the phrase "libc wrappers". The owning step must build the symbol into the linker built-in libc48 archive and execute a positive/negative target test for it.
...
## 5.5 Exact REV16 §41 Mandatory Test Replay/Ownership Ledger

Every mandatory REV16 §41 acceptance item has exactly one stable replay row below, in the same subsection order. REV16 remains the authoritative wording; the middle column is a concise, semantically equivalent replay description and must never weaken, merge, or replace the canonical requirement. The listed phase owner closes the implementation-side contract; `P12.26` replays the canonical REV16 item on the release candidate. A missing, duplicate, reordered, merged-away, or semantically weakened row is a certification failure.

| REV16 test | Stable replay description of canonical acceptance item | Owning implementation/replay step(s) |
|---|---|---|
...
| §41.1 | `font4x8` is FAST_REQUIRED, pinned, structurally valid F4X8, and final release logical bytes are exactly 392 bytes with host SHA-256 `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`. | `P0.34,P12.05,P12.09-P12.10,P12.24,P12.26,P12.30` |
| §41.1 | Before first runtime `EI`, wall time is atomically valid at exact `TIME1` bytes `80 06 26 17 00 00`, revision 0, subsecond 0, while SYS_TICKS remains independent. | `P1.26-P1.27,P12.09,P12.26,P12.30` |
...
| §41.6 | Text at columns 62/63 does not damage adjacent bitmap cells. | `P1.41,P7.21,P12.26` |
| §41.6 | Last-cell printable output sets deferred wrap without a phantom coordinate; the next printable wraps/scrolls before drawing. | `P1.20,P1.35-P1.36,P1.41,P12.26` |
| §41.6 | One contiguous write and arbitrary split writes of the same bytes produce identical bitmap/attributes/row/col/wrap_pending across right-edge and scroll boundaries. | `P1.36,P1.41,P12.26` |
| §41.6 | 64-column scroll preserves all 23 retained text rows and never copies visible cursor XOR into retained content. | `P1.32,P1.41,P7.21,P12.26` |
| §41.6 | Block and underline cursor XOR is exactly reversible and preserves neighbor nibble and attributes. | `P1.24,P1.32,P1.41,P7.21,P12.26` |
| §41.6 | Cursor blink never modifies bitmap from IM2; 25-frame producer parity coalesces odd/even delayed transitions exactly. | `P1.13,P1.24,P1.30,P1.32,P1.41,P12.26` |
| §41.6 | Nested screen mutations hide at most once, nested ends never reconcile/release, and outer end reconciles exactly once; underflow/unbalanced/depth-wrap fixtures fail. | `P1.24,P1.32,P1.41,P12.26` |
| §41.6 | Due cursor service occurs at first safe outer-end, kernel re-entry, idle before/after-HALT, and blocked tty-input wake point without changing row/col/pending wrap. | `P1.24,P1.31-P1.32,P1.41,P12.26` |
| §41.6 | `stty cols 32` and `stty cols 64` switch predictably and clear pending wrap. | `P1.35,P1.41,P7.21,P12.26` |
| §41.6 | CR/LF/BS/TAB/FF first cancel pending wrap and then match Section 13.5A exactly. | `P1.37,P1.41,P7.21,P12.26` |
...
| §41.7 | Exact three-line `/etc/issue` boot heading under tty64. | `P6.30,P8.40,P12.26` |
| §41.7 | `/etc/issue` exact three LF-terminated lines, including the fun third line. | `P6.30,P8.40,P12.26` |
...
| §41.7 | Normal cold boot exposes exact pre-first-frame TIME1 bytes `80 06 26 17 00 00`, valid revision0/subsecond0, while an explicit diagnostic invalid fixture returns E_AGAIN. | `P1.26-P1.27,P6.30,P8.40,P12.09,P12.26` |
| §41.7 | Boot-valid `date` output, valid/invalid `date -s`, repeated-set revision increment, and no normal-production `date: not set` oracle. | `P6.30,P8.24,P8.40,P12.11,P12.26` |
| §41.7 | Wall-clock frame-50 rollover, leap day, and year/month/day boundaries. | `P1.26,P6.30,P8.40,P12.26` |
| §41.7 | `@boot` remains independent of wall time and runs once per daemon instance. | `P6.30,P8.25,P8.40,P12.26` |
| §41.7 | Calendar cron is eligible from valid revision-0 boot time without prior manual set. | `P6.30,P8.25,P8.39-P8.40,P12.11,P12.26` |
| §41.7 | No duplicate cron firing within one `(wall-minute,revision)` key. | `P6.30,P8.25,P8.39-P8.40,P12.26` |
...
| §41.8 | Normal/insert/command-line mode transitions, including `EDIT`/`0x1B` escape/cancellation and BREAK separation. | `R16.00,P9.05,P9.15,P9.24,P12.26` |
...
| §41.9 | H06 pins read-only SDK commit `9ca3c6d6b5dd4b6e2351c1800afbd47d1d77e411`, `usr/src` tree `f629dcc1d156b83bf08ed171b9273e3cbb621ad1`, exact `usr/src/examples/hello.c` blob/bytes/SHA-256, and exact direct `usr/src/examples/exapi.h` bytes/SHA-256. | `P11.45,P11.48,P12.26` |
| §41.9 | Exact pinned `hello.c` is compiled by target-native C48, linked by native `ld`, and executed under ZX-UX; host-only compile/link does not satisfy H06. | `P11.45,P11.48,P12.26` |
| §41.9 | H06 execution proves `cls()`, the two exact `print_at` strings at rows/columns 10/22 and 12/14, and clean status 0. | `P11.45,P11.48,P12.26` |
| §41.9 | The executed H06 canary retains deterministic debugger/register/RAM/process/exit proof plus at least one screenshot; screenshots are supplemental, never screenshot-only PASS. | `P11.45,P11.48,P12.26` |
| §41.9 | P11.45 evidence retains exact canary/header identities plus native compile/link/execute results; P11.48/P12.26 reject missing, changed, skipped, host-only, or non-executed canary evidence. | `P11.45,P11.48,P12.26` |
...
| §41.11.3 | IM2 recognizes exact `CAPS SHIFT + SPACE` BREAK from raw matrix reads. | `P1.15,P1.31,R16.00,P12.26` |
| §41.11.3 | Non-BREAK key combinations, including `CAPS SHIFT + 1` `EDIT`, do not spuriously set `break_pending`. | `R16.00,P12.26` |
| §41.11.3 | IM2 contains no call edge to ROM KEYBOARD/decoder routines. | `P1.31,P12.26` |
| §41.11.3 | Ordinary tty input decodes `CAPS SHIFT + 1` `EDIT` as sole target byte `0x1B` in tty32/tty64; the same byte is observed through `SYS_CON_GETKEY`, `/dev/tty` reads, and C48 `getchar()` when stdin is tty; BREAK never returns `0x1B`. | `R16.00,P3.03,P11.24,P12.26` |
| §41.11.4 | Draw tty64 text and show block/underline XOR cursor at ordinary and bottom-right pending-wrap positions, then hide and prove raw bitmap bytes return exactly. | `P1.32,P12.26` |
| §41.11.4 | Direct OS graphics/UDG and scroll/clear paths execute under balanced nested mutation brackets and never preserve/copy visible cursor XOR into content. | `P1.32,P12.26` |
| §41.11.4 | Divider transition occurs only each 25 accepted frames; delays of 1/2/3/4 intervals produce parity 1/0/1/0 and the same final logical phase without catch-up flashes. | `P1.13,P1.24,P1.30,P1.32,P12.26` |
| §41.11.4 | Interrupt immediately before/within/after the interrupt-excluded parity-consume window loses or double-consumes no posted transition. | `P1.13,P1.24,P1.32,P12.26` |
| §41.11.4 | IM2 never mutates bitmap,row,column,wrap_pending,or mutation depth and both altreg preservation paths include the timing producer. | `P1.13,P1.30,P1.32,P12.26` |
| §41.11.4 | Due service before idle,on HALT wake,while tty input blocks,and immediately before key return is consumed at first safe point with row/col/pending unchanged. | `P1.31-P1.32,P12.26` |
| §41.11.4 | Shape off across blink intervals then re-enable underline/block reconciles to current logical phase without resetting the clock. | `P1.24,P1.32,P12.26` |
| §41.11.4 | Mutation-depth underflow/unbalance/depth-wrap fails deterministically rather than corrupting cursor/exclusion state. | `P1.24,P1.32,P12.26` |
| §41.11.5 | Instrumentation records high-water depth on every kernel/ROM/ISR stress test, including deepest nested cursor/mutation/scroll-or-clear with large ISR path at every interruptible point. | `P1.28,P1.33,P12.25-P12.26` |
...
REV16 §62 questions below. A non-applicable answer must state why; a blank answer is a
failure.
...
| 30 | If it is `vi`, does the change preserve documented modal, `EDIT`/`0x1B` escape, BREAK-separation, and case-sensitive command semantics? |
...
**Status: CONTROLLED-REBASELINE CERTIFICATION CANDIDATE.**

This Revision-07 implementation plan is subordinate to canonical REV16 SHA-256
`24FE9D206C2F05BBC24F11544A5CB5A0B6AB104E9FC52E654A10C2D9734B008C` and becomes active only after the project SoP plus the anti-circular certification sequence below complete on these exact final document bytes.

## 8.1 Pre-certification convergence

Before final certification may be claimed, one immutable candidate must complete three
consecutive zero-defect zero-gap audits on the same exact bytes:

1. a complete source-order REV16 Sections 1-65 architecture audit;
2. an independent invariant/test/release audit covering all 150 Section-61 invariants,
   all 242 Section-41 mandatory test rows, all 41 Section-62 module-review questions,
   ...
   architecture-defined failure/atomicity/negative-test behavior; and
3. an adversarial cross-reference/ownership replay that starts again from the final
   bytes, verifies every cited step/section/owner exists, checks historical-to-current
   baseline segmentation and R16.00 non-retroactivity, exercises contradictory/omitted
   owner detection, and proves no requirement can obtain PASS only through circular
   self-reference.

Any gap found during convergence changes the candidate bytes, resets certification credit
to zero, and requires the source-order audit to restart from Section 1.

## 8.2 Final-byte certification

Any byte change after pre-certification convergence, including any change to this Section 8,
invalidates all prior certificate credit. The final immutable document bytes must therefore
be audited again from scratch with **no inherited PASS credit**. Final delivery requires all
three audits to be retained as evidence:

1. **Final certificate 1:** complete REV16 Sections 1-65 source-order audit, zero gaps.
2. **Final certificate 2:** independent complete REV16 Sections 1-65 audit, including
   the 150 invariant owners, 242 mandatory-test rows, 41 module-review questions,
...
3. **Final certificate 3:** adversarial traceability replay over the same final bytes,
   including step/owner/reference resolution, R16.00 historical-baseline carry-forward,
   anti-circularity, contradictory-contract search, and negative ownership/orphan checks.

A mutation after any final certificate invalidates all three final certificates and requires
another complete three-pass certification on the newly changed bytes. The delivered file is
therefore the same immutable byte sequence audited by all three retained final-certificate logs.
...
for every REV17 requirement without knowingly strengthening, weakening, inventing, or
...
The implementation sequence remains, with Revision 12 / Revision 03 authoritative through
the complete Phase-2 gate and the one-time transition gate inserted only after P2.24 and
before P3.01:

`E0 -> P0 -> P1 -> P2.01..P2.24 -> R16.00 -> P3.01..P3.21 -> P4 -> P5 -> P6 -> P7 -> P8 -> P9 -> P10 -> P11 -> P12`
