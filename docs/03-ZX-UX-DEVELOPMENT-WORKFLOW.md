...
1. Through and including `P2.24` and the Phase-2 aggregate evidence, the controlling authorities remain `docs/01-ZX-UX-ARCHITECTURE-REV12.md` and `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV03.md`. Those historical authority files remain immutable. Evidence already admitted for completed E0/P0/P1/P2 work is immutable; unfinished Phase-2 steps continue to create only the new evidence required by Revision 03, and each such admitted record then becomes immutable. No certification through `P2.24` may claim Revision 13 / Revision 04 identity, and no historical evidence may be regenerated or relabeled merely to appear current.
2. Revision 13 / Revision 04 may be prepared and may be published to the canonical repository during Phase 2 as documentation-only **dormant prospective authorities**, but their presence does not make them active. They must not be used to reinterpret, change, or certify Phase-2 work. Current Phase-2 tooling and evidence continue under the Revision 12 / Revision 03 authority contract and must not be changed merely to recognize, record, or satisfy the prospective REV13/REV04 identities. Current Phase-2 execution continues under Revision 12 / Revision 03 until `P2.24` and the Phase-2 aggregate are PASS.
3. `docs/01-ZX-UX-ARCHITECTURE-REV13.md` and `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV04.md` must be present in the canonical repository before `R13.00` begins, while REV12 and REV03 are retained unchanged. They may already have been published during Phase 2; otherwise publish them after the complete Phase-2 PASS. In either case, the documentation-only publication merely makes the future baseline available: it does **not** switch the active certification authority, admit Revision 13, alter any Phase-2 certification, or authorize Phase 3.
4. Only after `P2.24` and the Phase-2 aggregate are PASS, the new documents are present, and the repository is otherwise clean may `R13.00` execute exactly as defined by Revision 04. The bridge must preserve and verify the historical REV12/REV03 identities and Phase-2 evidence, certify the bounded REV13 delta and carried-forward current-tree regressions, advance the prospective certification tooling, certify one clean bridge source-candidate commit, and admit only the frozen `R13.00` records in a subsequent evidence-only admission commit.
5. Phase 3 is forbidden until the evidence-only `R13.00` admission commit exists, `v1/dist/certification/R13.00.result.json` records PASS for the exact REV13 architecture identity and the exact REV04 implementation-plan identity, and every mandatory automated validation triggered by that admission commit has completed successfully. `P3.01` is the first ordinary phase step permitted after those conditions are all satisfied.
6. After all conditions in item 5 are satisfied, Revision 13 / Revision 04 are the controlling authorities for Phase 3 and all later prospective certification. Historical E0-P2 evidence remains historical, read-only evidence under Revision 12 / Revision 03; it is supplemented by the bridge, never rewritten as if it had originally been certified against Revision 13 / Revision 04.
7. A failed or incomplete `R13.00` leaves the completed Phase-2 history intact but leaves the Phase-3 transition closed. Correct the bridge defect through the normal SoP/review/check-in cycle and rerun the bridge; do not mutate REV12/REV03 to force admission and do not begin `P3.01`.
...
6. Any documentation-only publication that adds REV13/REV04, whether during Phase 2 or after `P2.24`, follows the same documentation-only runner exclusion. That publication is not an `R13.00` PASS and must not be treated as active authority for Phase 2 or Phase 3. The later `R13.00` source-candidate and evidence-only admission transactions follow Section 1.1 and Revision 04 in addition to this normal check-in flow.
...
GARF continuity may span the Revision 12 / Revision 03 and Revision 13 / Revision 04 epochs. Carrying an earlier result identifier forward is historical continuity only: it does not change the active architecture/plan authority, does not satisfy `R13.00`, and does not relabel or upgrade historical E0-P2 certification evidence. The admitted durable `R13.00.result.json` PASS, its exact document identities, and successful mandatory validation of the evidence-only admission commit together form the sole Phase-2-to-Phase-3 authority transition.
