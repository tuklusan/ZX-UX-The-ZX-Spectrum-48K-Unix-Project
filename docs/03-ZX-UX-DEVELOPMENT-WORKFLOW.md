...
1. Through and including `P2.24` and the Phase-2 aggregate evidence, the controlling authorities remain `docs/01-ZX-UX-ARCHITECTURE-REV12.md` and `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV03.md`. Those historical authority files remain immutable. Evidence already admitted for completed E0/P0/P1/P2 work is immutable; unfinished Phase-2 steps continue to create only the new evidence required by Revision 03, and each such admitted record then becomes immutable. No certification through `P2.24` may claim Revision 14 / Revision 05 identity, and no historical evidence may be regenerated or relabeled merely to appear current.
2. Revision 14 / Revision 05 are the dormant prospective authorities that supersede the never-activated Revision 13 / Revision 04 pair. Revision 13 / Revision 04 remain frozen historical prospective revisions and must never be edited in place. Revision 14 / Revision 05 may be published documentation-only, but their presence does not make them active. They must not be used to reinterpret, change, or certify Phase-2 work. Current Phase-2 tooling and evidence continue under the Revision 12 / Revision 03 authority contract and must not be changed merely to recognize, record, or satisfy the prospective REV14/REV05 identities. Current Phase-2 execution continues under Revision 12 / Revision 03 until `P2.24` and the Phase-2 aggregate are PASS.
3. `docs/01-ZX-UX-ARCHITECTURE-REV14.md` and `docs/02-ZX-UX-IMPLEMENTATION-STEPS-REV05.md` must be present in the canonical repository before `R14.00` begins, while REV12 and REV03 are retained unchanged. Their exact prospective identities are pinned here: REV14 SHA-256 `868d6605733061fec00fb82c051e9ab02915041e5e26ddc1563a3c488bc82324`; REV05 SHA-256 `0f3008e84c0b32ccbf8bb0a1362ff52f87e387a38f6290220532755381ec680b`. The H06 correction carried by those exact bytes is also pinned to read-only SDK commit `9ca3c6d6b5dd4b6e2351c1800afbd47d1d77e411`, `usr/src` tree `f629dcc1d156b83bf08ed171b9273e3cbb621ad1`, `usr/src/examples/hello.c` Git blob `95fa0186bda3f7636774f5885988d78a13f6450b`, 703 bytes, SHA-256 `6f94a735f230dadf5928993f9a071f3f47e98b63d18230eef102f6ae7b63e4b2`, and direct declaration dependency `usr/src/examples/exapi.h`, 2056 bytes, SHA-256 `2fa0edc593832d3ab57bc105233f41fd81a02b1f3ea022cefc42da01a6084bb8`. Any different REV14/REV05 bytes or H06 canary/header identity fail closed and require a new prospective revision pair; they must not be silently substituted. These documents may already have been published during Phase 2; otherwise publish them after the complete Phase-2 PASS. In either case, the documentation-only publication merely makes the future baseline available: it does **not** switch the active certification authority, admit Revision 14, alter any Phase-2 certification, or authorize Phase 3.
4. Only after `P2.24` and the Phase-2 aggregate are PASS, the new documents are present, and the repository is otherwise clean may `R14.00` execute exactly as defined by Revision 05. The bridge must preserve and verify the historical REV12/REV03 identities and Phase-2 evidence, certify the bounded REV14 delta and carried-forward current-tree regressions, advance the prospective certification tooling, certify one clean bridge source-candidate commit, and admit only the frozen `R14.00` records in a subsequent evidence-only admission commit.
5. Phase 3 is forbidden until the evidence-only `R14.00` admission commit exists, `v1/dist/certification/R14.00.result.json` records PASS for the exact REV14 architecture identity and the exact REV05 implementation-plan identity, and every mandatory automated validation triggered by that admission commit has completed successfully. `P3.01` is the first ordinary phase step permitted after those conditions are all satisfied.
6. After all conditions in item 5 are satisfied, Revision 14 / Revision 05 are the controlling authorities for Phase 3 and all later prospective certification. Historical E0-P2 evidence remains historical, read-only evidence under Revision 12 / Revision 03; it is supplemented by the bridge, never rewritten as if it had originally been certified against Revision 14 / Revision 05.
7. A failed or incomplete `R14.00` leaves the completed Phase-2 history intact but leaves the Phase-3 transition closed. Correct the bridge defect through the normal SoP/review/check-in cycle and rerun the bridge; do not mutate REV12/REV03 to force admission and do not begin `P3.01`.
...
6. Any documentation-only publication that adds REV14/REV05, whether during Phase 2 or after `P2.24`, follows the same documentation-only runner exclusion. That publication is not an `R14.00` PASS and must not be treated as active authority for Phase 2 or Phase 3. The later `R14.00` source-candidate and evidence-only admission transactions follow Section 1.1 and Revision 05 in addition to this normal check-in flow.
...
GARF continuity may span the Revision 12 / Revision 03 and Revision 14 / Revision 05 epochs. Carrying an earlier result identifier forward is historical continuity only: it does not change the active architecture/plan authority, does not satisfy `R14.00`, and does not relabel or upgrade historical E0-P2 certification evidence. The admitted durable `R14.00.result.json` PASS, its exact document identities, and successful mandatory validation of the evidence-only admission commit together form the sole Phase-2-to-Phase-3 authority transition.
