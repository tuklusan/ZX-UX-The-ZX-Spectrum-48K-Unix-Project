...
Heavy regression has one automatic owner per candidate SHA: `.github/workflows/exact-head-regression.yml`. It is the sole heavyweight push-triggered workflow on `main`. It uses one static-integrity job and one runtime-regression job, with `cancel-in-progress: true`, so an older unfinished candidate is superseded by a newer `main` head.
...
Push-triggered heavyweight regression is centralized in `exact-head-regression.yml`. Documentation-only changes, retained certification/media-only admission commits, historical per-step qualification workflow maintenance, and other explicitly excluded control-plane-only changes do not independently launch the heavyweight matrix. A mixed push containing executable or certification-input changes still runs normally.
