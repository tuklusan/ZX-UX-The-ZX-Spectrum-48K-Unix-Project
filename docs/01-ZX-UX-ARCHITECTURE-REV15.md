...
Revision: 15 - Phase-3 bridge authority-pair correction
...
- deferred final-cell wrap, nested mutation, safe cursor service, and the 448-byte kernel
  ...
  matrices; the Revision-15 `CAPS SHIFT + 1` `EDIT` -> `0x1B` tty mapping and its strict
  separation from `CAPS SHIFT + SPACE` BREAK are delta-certified by the mandatory R15.00
