...
Status: Revision-12 controlled-rebaseline implementation and certification plan
Revision: 03
Architecture baseline: `docs/01-ZX-UX-ARCHITECTURE-REV12.md`
Architecture SHA-256: `A90D523F62A95E8CBA6AF0312B596A2D5F6BC1AA2EF92F39BB391509B7C15E1B`
...
This document is subordinate to Revision 12. It does not amend the architecture.
If this document conflicts with Revision 12, Revision 12 wins and this document
...
`docs/01-ZX-UX-ARCHITECTURE-REV12.md` with SHA-256 `a90d523f62a95e8cba6af0312b596a2d5f6bc1aa2ef92f39bb391509b7c15e1b` and verifies
...
Revision-12 phase order. A later phase does not begin because its code looks fun.
...
The implementation must preserve, among the rest of Revision 12, these high-risk
...
- the final system-tape order is frozen by Revision 12 Section 19.8 and is not a
...
1. **Purpose / REV12 requirement:** §§6.3,11.1,49
...
1. **Purpose / REV12 requirement:** §11.1
...
1. **Purpose / REV12 requirement:** §§11,13,32
...
1. **Purpose / REV12 requirement:** §32
...
1. **Purpose / REV12 requirement:** §11
...
1. **Purpose / REV12 requirement:** §11.3, §49
...
1. **Purpose / REV12 requirement:** §11, §49
...
1. **Purpose / REV12 requirement:** §§12.1,49
...
1. **Purpose / REV12 requirement:** §12
...
1. **Purpose / REV12 requirement:** §§12.1-12.2
...
1. **Purpose / REV12 requirement:** §§9.2,10.1,11.5
...
1. **Purpose / REV12 requirement:** §10.1, §49
...
1. **Purpose / REV12 requirement:** §49 acceptance
...
1. **Purpose / REV12 requirement:** §10.1, §13.5, §49
...
1. **Purpose / REV12 requirement:** §18
...
1. **Purpose / REV12 requirement:** §18, §27.3
...
1. **Purpose / REV12 requirement:** §§10.1,11.2
...
1. **Purpose / REV12 requirement:** §§11.1-11.2,27.3
...
1. **Purpose / REV12 requirement:** §§11.4-11.6,18
...
1. **Purpose / REV12 requirement:** §§11.5,18,31
...
1. **Purpose / REV12 requirement:** §11, §41.3A
...
1. **Purpose / REV12 requirement:** §10.1
...
1. **Purpose / REV12 requirement:** §18, §4.3B
...
1. **Purpose / REV12 requirement:** §10.1, §18
...
1. **Purpose / REV12 requirement:** §27.3
...
1. **Purpose / REV12 requirement:** §§11.4,27.3
...
1. **Purpose / REV12 requirement:** §27.3, §41.3A
...
1. **Purpose / REV12 requirement:** §10.1, §27.3
...
1. **Purpose / REV12 requirement:** §18.5, §27.3
...
1. **Purpose / REV12 requirement:** §4.4, §27.3
...
1. **Purpose / REV12 requirement:** §7, §27.3
...
1. **Purpose / REV12 requirement:** §10.1, §33
...
1. **Purpose / REV12 requirement:** §41.3A
...
1. **Purpose / REV12 requirement:** §18.6, §32; invariants 108/115
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All three listings, types, states, zero lengths, ordering and parent IDs match REV12 byte-for-byte.
...
1. **Purpose / REV12 requirement:** §10.1, §18, §50
...
1. **Purpose / REV12 requirement:** §50 acceptance
...
1. **Purpose / REV12 requirement:** §19, §51
...
1. **Purpose / REV12 requirement:** §19
...
1. **Purpose / REV12 requirement:** §19, §27.3
...
1. **Purpose / REV12 requirement:** §5.1, §51
...
1. **Purpose / REV12 requirement:** §19.7, §29
...
1. **Purpose / REV12 requirement:** §19, §20
...
1. **Purpose / REV12 requirement:** §7, §19, §27.3
...
1. **Purpose / REV12 requirement:** §51 acceptance
...
1. **Purpose / REV12 requirement:** §51
...
1. **Purpose / REV12 requirement:** §40.3
...
1. **Purpose / REV12 requirement:** §5.5, §20, §41.1
...
1. **Purpose / REV12 requirement:** §20.2
...
1. **Purpose / REV12 requirement:** §20.2A
...
1. **Purpose / REV12 requirement:** §20.3
...
1. **Purpose / REV12 requirement:** §20.4, §52
...
1. **Purpose / REV12 requirement:** §20.7
...
1. **Purpose / REV12 requirement:** §18, §20.7
...
1. **Purpose / REV12 requirement:** §20.5, §33, §52
...
1. **Purpose / REV12 requirement:** §20.5, §31, §52
...
1. **Purpose / REV12 requirement:** §20.3, §31
...
1. **Purpose / REV12 requirement:** §20.12
...
1. **Purpose / REV12 requirement:** §13.5, §20.12
...
1. **Purpose / REV12 requirement:** §20.4, §20.12
...
1. **Purpose / REV12 requirement:** §§20.12,21; invariants 99/111
...
1. **Purpose / REV12 requirement:** §13, §20.12
...
1. **Purpose / REV12 requirement:** §52 acceptance
2. **Exact implementation work:** Freeze host parser/AST and target execution vectors for quoting/operators/expansion/PATH/builtins/case. Golden vectors are derived from REV12 userland contracts only; no host-shell quoting, field-splitting, globbing, substitution, precedence, PATH, environment or parent-builtin behavior may be borrowed implicitly.
...
9. **Negative/failure test:** Any unsupported construct accidentally accepted fails. Run divergence vectors chosen to behave differently in common host shells; ZX-UX must follow REV12, not the host.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §20.12; invariants 110/118
...
1. **Purpose / REV12 requirement:** §52 acceptance
2. **Exact implementation work:** Aggregate login/core parser, echo pipe, PATH matrix, builtin E_NOTSUP in pipe/bg, redirection rollback, child errors and mixed-case behavior, plus REV12 shell-facing time rows: normal boot begins with valid revision-0 launch epoch; no normal `date: not set`; diagnostic invalid fixture remains explicit; date -s revision semantics and boot-valid cron/cal expectations remain delegated to their owning utility gates.
...
1. **Purpose / REV12 requirement:** §10.1, §15, §53
...
1. **Purpose / REV12 requirement:** §3.3, §15
...
1. **Purpose / REV12 requirement:** §15.1, §15.3
...
1. **Purpose / REV12 requirement:** §15.1, §15.4
...
1. **Purpose / REV12 requirement:** §15
...
1. **Purpose / REV12 requirement:** §§10.1,17
...
1. **Purpose / REV12 requirement:** §§14.6,20.6
...
1. **Purpose / REV12 requirement:** §14.7, §20.6
...
1. **Purpose / REV12 requirement:** §20.6
...
1. **Purpose / REV12 requirement:** §§4.3,14.8A,16.1-16.2
...
1. **Purpose / REV12 requirement:** §§10.1,16.1,16.3
...
1. **Purpose / REV12 requirement:** §16.2-16.3
...
1. **Purpose / REV12 requirement:** §16.5
...
1. **Purpose / REV12 requirement:** §16.4, §53
...
1. **Purpose / REV12 requirement:** §17, §53
...
1. **Purpose / REV12 requirement:** §53 acceptance
...
1. **Purpose / REV12 requirement:** §§17,20.5-20.7,53; invariant 78
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (ls)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/ls contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (cat)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/cat contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (cp)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/cp contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (mv)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/mv contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (rm)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/rm contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (pack)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/pack contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (unpack)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/unpack contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (grep)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/grep contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (wc)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/wc contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (head)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/head contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (tail)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/tail contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (cmp)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/cmp contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (true)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/true contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (false)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/false contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (sleep)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/sleep contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (which)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/which contract; no case folding. `which ls` can report the exact external path; builtin-only name emits no bytes and status 1; catalog-only lookup does not move cassette.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (env)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/env contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (hexdump)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/hexdump contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.4,33,54 (ps)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Builtin behavior and exit status match exact REV12 Section-33/ps contract; no case folding; no external `/bin/ps` object or BCAT slot exists.
...
1. **Purpose / REV12 requirement:** §§20.4,33,54 (mem)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Builtin behavior and exit status match exact REV12 Section-33/mem contract including `-c`; no case folding; no external `/bin/mem` object or BCAT slot exists.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (udg)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/udg contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (gfxdemo)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/gfxdemo contract; no case folding.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/stty contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (date)
2. **Exact implementation work:** Implement external `date` on the REV12 wall clock. Normal successful boot reads valid TIME1 and prints exact `YYYY-MM-DD hh:mm:ss` derived from the current launch-epoch-based software clock. `date -s "YYYY-MM-DD hh:mm:ss"` validates Gregorian 1970..2099, calls SYS_TIME_SET, increments revision on every successful set, and resets subsecond. Exact literal `date: not set` is retained only for explicit diagnostic invalid-state tests, never the normal production boot oracle.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/date contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (crontab)
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (man)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/man contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (cal)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/cal contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (uptime)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/uptime contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (whoami)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/whoami contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (uname)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/uname contract; no case folding. Output bytes are exactly ASCII `ZX-UX z80 48k` followed by one LF byte (0x0A), with no legacy `ZX48-UX` spelling.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (fortune)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/fortune contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (banner)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/banner contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (rev)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/rev contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (yes)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/yes contract; no case folding.
...
1. **Purpose / REV12 requirement:** §§20.5,33,54 (demo)
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Behavior and exit status match exact REV12 Section-33/demo contract; no case folding.
...
1. **Purpose / REV12 requirement:** §54 acceptance
...
1. **Purpose / REV12 requirement:** §20.9, §41.7; invariants 105/109/119
...
1. **Purpose / REV12 requirement:** §22, §55
...
1. **Purpose / REV12 requirement:** §22
...
1. **Purpose / REV12 requirement:** §22.6
...
1. **Purpose / REV12 requirement:** §20.9, §55 acceptance
...
1. **Purpose / REV12 requirement:** §22.5, §22.6A
...
1. **Purpose / REV12 requirement:** §22.6A, §41.8
...
1. **Purpose / REV12 requirement:** §22, including §§22.7-22.8, and §55
...
1. **Purpose / REV12 requirement:** §55 acceptance, §44
...
1. **Purpose / REV12 requirement:** §23, §56
...
1. **Purpose / REV12 requirement:** §23
...
1. **Purpose / REV12 requirement:** §56
...
1. **Purpose / REV12 requirement:** §23, §38
...
1. **Purpose / REV12 requirement:** §23, §38, §56
...
1. **Purpose / REV12 requirement:** §23
2. **Exact implementation work:** Serialize deterministic OBJ1 from the fully validated assembler result. Symbol-record order is an implementation policy frozen by deterministic native-assembler tests and is NOT an architecture-prescribed sort order; relocation records MUST obey REV12's strictly increasing, non-overlapping offset rule. Compute the OBJ1 body/header CRCs over the exact stored bytes and run the independent host inspector before transactional publication.
3. **Files/artifacts created or modified:** as.asm
4. **Build command:** `<project-local-python> v1/tools-host/test-driver/run.py build --step P10.18`.
5. **Host-side static checks:** root-marker/path policy, architecture constants used by this step, file/format sizes, duplicate/forbidden symbols or literals, and deterministic artifact hashes as applicable.
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Two identical native assemblies produce byte-identical OBJ1; the host inspector validates all records/offsets/CRCs and relocation records are strictly increasing/non-overlapping exactly as REV12 requires.
9. **Negative/failure test:** Any malformed symbol/relocation record, relocation-order/overlap violation, CRC mismatch, nondeterministic output for identical input, or documentation/test that falsely claims REV12 mandates symbol-table sorting fails before commit.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
1. **Purpose / REV12 requirement:** §24
...
1. **Purpose / REV12 requirement:** §24, §41.2A
...
1. **Purpose / REV12 requirement:** §24, §26, §41.2A
...
1. **Purpose / REV12 requirement:** §24, §56
...
1. **Purpose / REV12 requirement:** §56 acceptance
...
1. **Purpose / REV12 requirement:** §56, §44
...
Z80 code and must emit the REV12 OBJ1/ABI/runtime contracts. If the SDK and REV12
differ, REV12 wins and the difference must be recorded rather than silently copied.
...
constraints remain governed by REV12.
...
implemented or accepted, review that document end-to-end alongside REV12 and the pinned
...
REV12 remains the controlling ZX-UX architecture authority and the SDK repository
remains read-only.
...
conformance review against REV12, the applicable DOCX clauses, and the corresponding
behavior in the pinned Python SDK implementation. No discrepancy may be silently
accepted. If the DOCX conflicts with REV12, REV12 wins and the DOCX must be corrected
before the dependent step closes. If native behavior conflicts with REV12 or the
...
SDK disagree on behavior not already decided by REV12, resolve and record the intended
...
6. **Emulator test artifact:** N/A: this is a host-only contract/format/policy step; target behavior is closed by later dependent steps.
7. **Exact FUSE assertions/checkpoints:** N/A except where the host test deliberately launches a timeout/argv self-test.
8. **Expected PASS result:** Spec checklist against §25 names every supported/deferred language family, data-model rule and operator exclusion explicitly; no generic “subset” sentence substitutes for a missing item. The evidence records end-to-end review of `docs/04-C48 Language Specification Rev 0.11.docx`, every required correction, the pinned Python SDK commit, the reviewed SDK Python implementation/design/docs, every intentional REV12-over-SDK divergence, and every identified REV12/DOCX/SDK discrepancy with its recorded resolution.
9. **Negative/failure test:** Unsupported feature accidentally claimed, missing DOCX review/correction evidence, missing SDK provenance or Python implementation coverage, an unexplained REV12/SDK contract difference, or any unresolved REV12/DOCX/SDK discrepancy is documentation failure.
...
1. **Purpose / REV12 requirement:** §§25.5-25.6,57
...
1. **Purpose / REV12 requirement:** §25.7, §41.9, §57
...
1. **Purpose / REV12 requirement:** §25
...
1. **Purpose / REV12 requirement:** §25, §14
...
1. **Purpose / REV12 requirement:** §10.1, §25
...
1. **Purpose / REV12 requirement:** §25, invariant
...
1. **Purpose / REV12 requirement:** §25, §37
...
1. **Purpose / REV12 requirement:** §25.8; §§9-13
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All fifteen symbols resolve from the built-in libc48 archive and behavior matches the underlying REV12 syscall contracts, including short I/O and typed creation.
...
1. **Purpose / REV12 requirement:** §25
2. **Exact implementation work:** Implement required minimal routines; use documented Z80 block primitives where measured appropriate. The exact §25.8 public surface owned here is `getchar`, `putchar`, `puts`, `strlen`, `strcmp`, `strcpy`, `strncpy`, `memcpy`, `memmove`, `memchr`, `memset`. Implement target-byte semantics, overlap-safe memmove, bounded string/memory behavior and tty wrappers without a large general-purpose `printf`. Provide the REV12 compact integer formatter as a small libc/compiler-runtime helper and use the ROM-backed floating formatter owned by P11.46 for floating text; neither requirement authorizes a general printf implementation.
...
1. **Purpose / REV12 requirement:** §25, §42
...
1. **Purpose / REV12 requirement:** §2.8, §38, §57
...
1. **Purpose / REV12 requirement:** §2.8, §57
...
1. **Purpose / REV12 requirement:** §57
...
1. **Purpose / REV12 requirement:** §42, §57
...
1. **Purpose / REV12 requirement:** §57 acceptance
...
1. **Purpose / REV12 requirement:** §44, §41.3
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All FAST_REQUIRED allocations are satisfied; the <=20480 live-footprint measurement is physical, placement policy matches REV12, and the memory-short path reports `not enough memory for compiler` without corruption; no paper-only total.
...
1. **Purpose / REV12 requirement:** §25, §41.9
...
1. **Purpose / REV12 requirement:** §41.9
...
1. **Purpose / REV12 requirement:** §41.9; ZXP1/object invariants
...
1. **Purpose / REV12 requirement:** §10.1, §14, §25.8-25.9
...
6. **Emulator test artifact:** Generate/run the smallest deterministic SNA artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Formatted bytes/count/status match the REV12 ABI; no partial write on insufficient/invalid destination; IY/calculator state restored.
...
1. **Purpose / REV12 requirement:** §33, §42, §58
...
1. **Purpose / REV12 requirement:** §4.3B, §5.1, §20.5, §58
...
1. **Purpose / REV12 requirement:** §5.1, §58
...
1. **Purpose / REV12 requirement:** §4.3A, §58
...
1. **Purpose / REV12 requirement:** §19.8, §58
...
1. **Purpose / REV12 requirement:** §5.6, §19.8, §58
...
1. **Purpose / REV12 requirement:** §5.5, §41.1, §58
2. **Exact implementation work:** Execute the exact REV12 §5.5 thirty-step cold-boot sequence in order. Step 20 atomically initializes valid `TIME1` to `80 06 26 17 00 00` (1982-04-23 00:00:00, revision0, subsecond0) without touching independent SYS_TICKS; final font validation/pinning follows at its specified step; tty64 private cursor/wrap/mutation state is initialized before PID1; `EI` is step 29 and scheduler entry step 30. Retain machine-state evidence at every boundary and prove no accepted frame can observe a half-initialized wall clock.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic TAP/TZX artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** All 30 ordered boot ownership/resource transitions match REV12 exactly; pre-first-frame TIME1 and independent ticks are exact; final font bytes are the P12.05 identity; first EI occurs only after all required state is valid.
...
1. **Purpose / REV12 requirement:** §58 items4-8, §63
...
1. **Purpose / REV12 requirement:** §58 item10
...
1. **Purpose / REV12 requirement:** §58 item11
...
1. **Purpose / REV12 requirement:** §58 item12
...
1. **Purpose / REV12 requirement:** §58 item13
...
1. **Purpose / REV12 requirement:** §58 item14
...
1. **Purpose / REV12 requirement:** §58 item15
...
1. **Purpose / REV12 requirement:** §58 item16
...
1. **Purpose / REV12 requirement:** §58 item17
...
1. **Purpose / REV12 requirement:** §58 item18, §42.15
...
1. **Purpose / REV12 requirement:** §19.9, §58
...
1. **Purpose / REV12 requirement:** §40.4
...
1. **Purpose / REV12 requirement:** §40.4, §58
...
1. **Purpose / REV12 requirement:** §58 acceptance
...
6. **Emulator test artifact:** FUSE may be used only as regression evidence; it cannot close this hardware/analog gate.
7. **Exact FUSE assertions/checkpoints:** Not sufficient for PASS. Record real 48K-compatible hardware or hardware-faithful EAR/MIC-loop evidence.
8. **Expected PASS result:** Every item PASS; all within 48K.  The final acceptance record has exactly twenty numbered rows in REV12 order, each with target-state/evidence references, and all twenty PASS on the same official release tape and 48K candidate.
...
1. **Purpose / REV12 requirement:** §63
...
1. **Purpose / REV12 requirement:** §59
...
1. **Purpose / REV12 requirement:** §5.6, §19.8-19.9, §58
...
1. **Purpose / REV12 requirement:** handover Git rule
...
1. **Purpose / REV12 requirement:** §29; §41; invariants 23,25,29,32,39,55,71,92,99,111,133-135
...
1. **Purpose / REV12 requirement:** §30; invariant 55
...
1. **Purpose / REV12 requirement:** §58, §§40-44, §63
...
contract. No Revision-12 requirement may be left to implication.

| REV12 section | Primary implementation/certification coverage |
|---|---|
...
REV12**, not a copied/retyped list in this document. This avoids creating a second
...
## 5.2 Explicit REV12 §61 Invariant Ownership Ledger

The invariant prose itself remains authoritative only in canonical REV12. This table is deliberately one row per numbered invariant so an audit cannot hide a missing item inside a broad category. Each listed step must contain a positive or negative proof, or an explicit scope guard, for that invariant.

| REV12 invariant | Owning implementation/test step(s) |
|---:|---|
...
## 5.3 Exact REV12 §§9.2-10 Syscall ABI and Ownership Ledger
...
## 5.4 Exact REV12 §25.8 C48 Runtime Ownership Ledger

REV12 freezes a 73-function minimum public C48 runtime. This table is one row per required public symbol so none can disappear inside the phrase "libc wrappers". The owning step must build the symbol into the linker built-in libc48 archive and execute a positive/negative target test for it.
...
## 5.5 Exact REV12 §41 Mandatory Test Replay/Ownership Ledger

Every mandatory REV12 §41 acceptance item is written literally below. The listed phase owner closes the implementation-side contract; `P12.26` replays the item on the release candidate. A missing row is a certification failure.

| REV12 test | Exact mandatory acceptance item | Owning implementation/replay step(s) |
|---|---|---|
...
REV12 §62 questions below. A non-applicable answer must state why; a blank answer is a
failure.
...
This Revision-03 implementation plan is subordinate to canonical REV12 SHA-256
`A90D523F62A95E8CBA6AF0312B596A2D5F6BC1AA2EF92F39BB391509B7C15E1B` and becomes active only after the project SoP plus the anti-circular certification sequence below complete on these exact final document bytes.
...
1. a complete source-order REV12 Sections 1-65 architecture audit; and
...
1. **Final certificate 1:** complete REV12 Sections 1-65 source-order audit, zero gaps.
2. **Final certificate 2:** independent complete REV12 Sections 1-65 audit, including
...
for every REV12 requirement without knowingly strengthening, weakening, inventing, or
