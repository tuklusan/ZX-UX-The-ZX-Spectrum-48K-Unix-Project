...
Architecture SHA-256: `EA23EB1C4815490830325B235E885D11B475A27CE6DCB9C70F4716D5C604FEA0`
...
`docs/01-ZX-UX-ARCHITECTURE-REV12.md` with SHA-256 `ea23eb1c4815490830325b235e885d11b475a27ce6dcb9c70f4716d5c604fea0` and verifies
...
- final release `font4x8` logical bytes are host-frozen at SHA-256
  `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`; the Phase-0
  fixture remains explicitly non-final until P12.05.
...
8. **Expected PASS result:** `docs/01-ZX-UX-ARCHITECTURE-REV12.md` hashes exactly to `ea23eb1c4815490830325b235e885d11b475a27ce6dcb9c70f4716d5c604fea0` and no alternate/superseded architecture is consumed.
...
2. **Exact implementation work:** Add contract-valid Phase-0 stubs and validate exact M48O names/order/types/targets: sh,font4x8,issue,crontab,bincat. Freeze the `issue` logical target bytes exactly as `<0x7F> Supratim Sanyal, SANYALnet Labs<LF>`, `https://supratim-sanyal.blogspot.com/<LF>`, `48K. One Z80. No excuses.<LF>`; freeze `crontab` as zero-length RAW and freeze the BCAT shape even before final binaries exist. `v1/assets/font4x8.bin` is explicitly a non-final 392-byte structurally valid F4X8 Phase-0 fixture (current fixture SHA-256 `1f29bb201828bbb6f0c0945c9677eb495b7b99592bd9c05ed10c0597caa69114`); this step must not claim or require the final release SHA-256. P12.05 alone owns promotion to the byte-frozen final 392-byte payload SHA-256 `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`.
...
6. **Emulator test artifact:** Generate/run the smallest deterministic TAP/TZX artifact that exercises only this contract plus already-frozen prerequisites.
7. **Exact FUSE assertions/checkpoints:** Use debugger breakpoints, registers, RAM/expression assertions and deterministic process exit; no screenshot-only PASS.
8. **Expected PASS result:** Host validator checks the exact five-resource contract, rejects case-folding, and accepts the explicitly non-final but structurally valid Phase-0 F4X8 fixture without confusing it with final release identity.
...
2. **Exact implementation work:** Validate the F4X8 payload structurally before PID1: magic/version/first_code/glyph_count/flags and exact 392-byte layout with 384 packed glyph bytes, high nibble earlier row, bit3 leftmost. Pin the validated resource FAST_REQUIRED and expose no physical address. This target step deliberately does not compute SHA-256: during Phase 1 the still-explicit Phase-0 fixture may be used. Final host-side byte identity SHA-256 `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339` is owned only by P12.05.
...
6. **Emulator test artifact:** N/A: this is a host-only contract/format/policy step; target behavior is closed by later dependent steps.
7. **Exact FUSE assertions/checkpoints:** N/A except where the host test deliberately launches a timeout/argv self-test.
8. **Expected PASS result:** Spec checklist against §25 names every supported/deferred language family, data-model rule and operator exclusion explicitly; no generic “subset” sentence substitutes for a missing item. The evidence records the pinned Python SDK commit, the reviewed SDK design/docs, and every intentional REV12-over-SDK divergence.
9. **Negative/failure test:** Unsupported feature accidentally claimed, missing SDK provenance, or an unexplained REV12/SDK contract difference is documentation failure.
10. **Check-in gate:** build + static + positive + negative tests PASS; dependent earlier phase gates remain PASS; commit only the intended source/docs/evidence and end with a clean Git worktree.
...
9. **Negative/failure test:** Any missing demo/compiler feature, missing/altered SDK test, SDK test failure, skipped/xfail admission test, missing SDK provenance, unexplained SDK/REV12 difference, or unmapped SDK C48 expectation blocks Phase 11.
...
2. **Exact implementation work:** Promote and freeze the final canonical `v1/assets/font4x8.bin` logical payload. It must be byte-identical to the preserved/read-only Tasword reference, exactly 392 bytes, and host SHA-256 exactly `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`. Explicitly reject the earlier Phase-0 fixture SHA-256 `1f29bb201828bbb6f0c0945c9677eb495b7b99592bd9c05ed10c0597caa69114` as final identity. Target code continues to validate F4X8 structure/transport CRC only; no target SHA-256 implementation is introduced.
...
`EA23EB1C4815490830325B235E885D11B475A27CE6DCB9C70F4716D5C604FEA0` and becomes active only after the project SoP plus the anti-circular certification sequence below complete on these exact final document bytes.
