...
Revision: 17 - post-Phase-10 TZX release transport and native self-rebuild
...
Revision-17 controlled release-transport rebaseline changes:

- preserves Revision 16 / Revision 07 and all admitted P0-P10 evidence/media byte-for-byte as historical authority;
- inserts R17.00 after P10.36 and before P11.01; no Phase-11 work may precede that bridge;
- makes product/pre-release media TZX-only, with no release TAP, no production SCREEN$/`zx48uxscr`, and no active dependency on `assets/loading.scr`;
- uses the reviewed fast-loader design, loader-owned 24x32 display, exact 8192-byte kernel at 0xE000-0xFFFF, and exact handoff at 0xE003;
- requires deterministic TZX construction, independently derived/verified turbo-block integrity fields, independent reconstruction of the embedded kernel, and real-time Fuse proof with fastload/traps/loader shortcuts disabled;
- requires correct post-kernel tape position at the first M48O object where present;
- requires exact three-way identity for all 8192 bytes: host-built kernel == TZX-embedded kernel == native-rebuilt kernel;
- requires genuine `source -> native as -> OBJ1 -> native ld -> executable -> run`; native `as` must parse source and emit real OBJ1, native `ld` must consume native OBJ1, and no host-preconstructed OBJ1/MEX1/kernel payload may satisfy the proof;
- permits only assembler/linker facilities needed for this proof, including fixed/absolute kernel output if required; any deterministic native-buildable projection must be source-only and contain no preassembled kernel payload;
- native rebuild output must not overwrite the executing kernel; every byte offset 0..8191 is compared, all three SHA-256 values are recorded, and a controlled source/input mutation must make equality fail;
- keeps Pasmo 0.5.3-7 release-scoped and outside the shared certified toolchain/bootstrap/runtime-cache machinery; and
- leaves unrelated Revision-16 ABI/runtime/object/userland contracts unchanged.

Historical descriptions of the completed P0-P10 SCREEN$/TAP epoch remain historical. For current product/pre-release transport and future P11/P12 release acceptance, Revision-17 clauses supersede conflicting Revision-16 SCREEN$/TAP wording.

Revision-16 controlled rebaseline changes:

- carries forward every Revision-15 architecture contract unchanged, including the
  EDIT/ESC input contract and bounded H06 SDK hello-canary requirement;
- supersedes dormant prospective Revision 15 / Revision 06 before either became active;
  frozen Revision 15 remains SHA-256 `c80b0626d58d79b133de90ee64e598421618ce5c503d6f5d34c866e332e4c398` and frozen Revision 06 remains
  SHA-256 `aeba14b8baa5387b98c158ac00ee9d81e93a162807769497068dac1adc0c8624`;
- records that frozen Revision 06 Section 8 bound itself to obsolete Revision-13
  SHA-256 `8a4a2767d7d7f1a7568f5f5b660ce3dad737767f69311366d1f6ec3c62ec940a`
  despite its correct Revision-15 header binding, so R15.00 was never admissible;
- makes no target-behavior, ABI, evidence-strength, or H06 change relative to Revision 15;
- requires matching Revision-07 to place R16.00 after P2.24 and before P3.01; and
- requires R16.00 to preserve historical REV12/REV03 evidence unchanged while admitting
  only the exact Revision-16 / Revision-07 authority pair.

Revision-15 controlled rebaseline changes:

- carries forward every Revision-14 architecture contract, including the EDIT/ESC input
  contract and the bounded H06 SDK hello-canary correction, but supersedes the dormant
  prospective Revision 14 / Revision 05 pair before either became active authority;
- records that frozen Revision 05 was published with stale Revision-13 architecture
  identity text and stale `Revision-14/04` bridge labels, so R14.00 was never admissible
  and Revision 14 / Revision 05 remain frozen, never-activated historical prospective
  authorities;
- makes no target-behavior change relative to Revision 14;
- requires the matching Revision-06 implementation plan to place one explicit R15.00
  transition bridge after P2.24 and before P3.01; and
- requires R15.00 to preserve historical REV12/REV03 evidence unchanged while pinning
  the exact Revision-15 / Revision-06 authority pair before Phase 3.

Revision-14 controlled rebaseline changes:

- carries forward every Revision-13 contract, including the EDIT/ESC input contract and
  fail-closed Phase-2/Phase-3 authority transition, but supersedes dormant prospective
  Revision 13 / Revision 04 before either became active authority;
- corrects H06 so target-native SDK compatibility is proved by one bounded portable
  canary rather than by attempting to compile and execute the entire host-oriented SDK
  source corpus on the 48K target;
- pins that canary to SDK commit `9ca3c6d6b5dd4b6e2351c1800afbd47d1d77e411`,
  `usr/src` tree `f629dcc1d156b83bf08ed171b9273e3cbb621ad1`, exact
  `usr/src/examples/hello.c` Git blob `95fa0186bda3f7636774f5885988d78a13f6450b`,
  703 bytes, SHA-256 `6f94a735f230dadf5928993f9a071f3f47e98b63d18230eef102f6ae7b63e4b2`,
  with its direct declaration dependency `usr/src/examples/exapi.h`, 2056 bytes,
  SHA-256 `2fa0edc593832d3ab57bc105233f41fd81a02b1f3ea022cefc42da01a6084bb8`;
- requires the matching Revision-05 implementation plan to place one explicit R14.00
  transition bridge after P2.24 and before P3.01; and
- retains every other Revision-13 architecture contract unchanged.

Revision-13 controlled rebaseline changes:

- freezes the physical 48K `EDIT` chord, `CAPS SHIFT + 1`, as the sole tty input
  ESC-equivalent target byte `0x1B`, distinct from the `CAPS SHIFT + SPACE` `BREAK`
  cancellation chord, and freezes the corresponding `vi` escape/cancellation semantics;
- establishes Revision 13 prospectively only after the complete Phase-2 acceptance gate.
  Existing certification evidence through P2.24 remains historical evidence against
  Revision 12 / implementation-plan Revision 03 and MUST NOT be relabeled as having
  been executed against Revision 13. The matching Revision-04 plan MUST place its
  explicit one-time R13.00 transition bridge after P2.24 and before P3.01. That bridge
  must preserve the historical evidence unchanged, delta-implement and delta-recertify
  any Revision-13 obligation that touches already-completed code, and fail closed if an
  unallowlisted earlier requirement or oracle is found to have changed; and
- retains every other Revision-12 architecture contract unchanged.
...
- freezes one balanced nested screen-mutation discipline and a 25-frame IM2
  divider/parity producer whose bitmap cursor reconciliation occurs only at safe
  non-ISR service points;
- changes successful normal cold boot from an unset wall clock to deterministic
  valid `1982-04-23 00:00:00`, `TIME1` bytes `80 06 26 17 00 00`, revision 0,
  subsecond 0, while preserving independent modulo-2^32 `SYS_TICKS`;
- preserves explicit wall-clock-invalid behavior only for early-boot, diagnostic,
  and failure-path fixtures rather than the normal production boot oracle;
- requires the matching implementation plan to retain H06 SDK `usr/src` corpus
  admission as a separate compiler-acceptance obligation.
...
The official product/pre-release distribution is one deterministic TZX started with ordinary Spectrum `LOAD ""`.
The reviewed fast loader owns the visible loading display and the kernel transfer. There is no production SCREEN$ logical file,
no `zx48uxscr` header, no production dependency on `assets/loading.scr`, and no release TAP artifact.

The loader initializes its own 24x32 loading display, transfers exactly 8192 freshly rebuilt kernel bytes into 0xE000-0xFFFF,
and dispatches only to 0xE003. The kernel bytes are explicit build input and may never be inherited from the seed/dummy payload.
Any post-kernel M48O stream follows the fast-loader blocks in the canonical order, and the loader must leave the first object
positioned for the kernel to read exactly once.

The BASIC-visible bootstrap label `ZX-UX Unix`, if retained, is a ROM/BASIC label only and does not relax the lower-case,
case-sensitive ZX-UX object namespace.
...
The final version-1 logical payload is byte-frozen at exactly 392 bytes. Host-side
release/certification SHA-256 is exactly:
...
## 5.1 TZX-only fast-loader bootstrap

The official product/pre-release cassette image is TZX-only. The canonical fast-loader implementation is promoted from the
reviewed package preserved at `scratch/ZX-UX-LOADER-1.0.0-portable.zip`, outer SHA-256
`4c83a681f718192db9ebb32f0c8bc565baf2e68eb85de480322a1102d32f00c7`.

It begins from ordinary `LOAD ""`, owns the loading display, transfers exactly 8192 kernel bytes to 0xE000-0xFFFF,
and dispatches exactly to 0xE003. There is no production SCREEN$ file.

The reviewed kernel payload uses exactly 24 chunks: chunks 0-7 are 342 bytes each and chunks 8-23 are 341 bytes each.
The builder must derive and regenerate every payload-dependent header/check/CRC field; guessing or copying stale dummy values is forbidden.

The fixed post-kernel M48O bootstrap sequence remains `sh`, `font4x8`, `issue`, `crontab`, `bincat`.
Their logical M48O contracts are unchanged.

## 5.2 Loader-owned display

The loader initializes bitmap/attributes and renders the reviewed 24x32 status text and colored status cells itself.
No `assets/loading.scr`, SCREEN$ block, or `zx48uxscr` logical file is part of the current product path.

## 5.3 Kernel identity, handoff, and real-time proof

The production kernel is freshly rebuilt from canonical source using the pinned host path and must be exactly 8192 bytes.
Those exact bytes are embedded into the TZX and independently reconstructed from the finished TZX. Real-time acceptance
uses the project-pinned Fuse/ROM path with fastload, tape traps, loader detection shortcuts, and equivalent acceleration disabled.
It must prove the real loader runs and reaches 0xE003 only after the exact rebuilt kernel is resident.

## 5.4 Permanent kernel ownership

At 0xE003, `zx_boot_entry` executes `DI`, abandons the BASIC return frame, establishes SP=0xFD00, and never returns to BASIC.
Subsequent kernel initialization and M48O bootstrap semantics remain the Revision-16 contracts except that no preloaded SCREEN$ exists.

## 5.5 Native self-rebuild proof

Release/pre-release acceptance must prove all of the following:

1. host build produces exactly 8192 kernel bytes;
2. independent finished-TZX reconstruction produces those exact 8192 bytes;
3. after real-time boot, ZX-UX native `as` genuinely parses canonical kernel source, or a deterministic source-only projection,
   and emits real OBJ1;
4. native `ld` genuinely consumes that OBJ1 and emits a fixed/absolute 8192-byte kernel image into non-executing storage;
5. the executing kernel is never overwritten;
6. host-built, TZX-embedded, and native-rebuilt images are compared at every byte offset 0..8191 and all three SHA-256 values are recorded;
7. no host-preconstructed OBJ1, MEX1, or kernel payload may substitute for native assembly/link;
8. a separate genuine `source -> native as -> OBJ1 -> native ld -> executable -> run` chain passes; and
9. a controlled source/input mutation makes the three-way equality proof fail.

A native-buildable projection is allowed only when canonical SjASMPlus-only constructs require it; it must be deterministic,
derived from canonical source, contain no preassembled kernel payload, and be genuinely assembled and linked natively.

## 5.6 Deterministic production TZX creation

Canonical `tools-host/release-tzx` tooling owns current product/pre-release construction. Product mode requires an explicit
8192-byte kernel, rejects the known dummy payload, patches all 24 fast blocks, independently regenerates integrity fields,
appends the canonical M48O stream where applicable, and emits TZX only. Two builds from identical inputs must be byte-identical.

Pasmo 0.5.3-7 is release-scoped only and must not be added to the shared certified toolchain lock, bootstrap, or runtime cache.

## 5.7 Boot failure

Any malformed loader block, integrity mismatch, wrong kernel size/range, dummy payload, incorrect handoff, or incorrect post-kernel
position is a hard release failure. Post-handoff kernel failures retain the explicit `PANIC <code>` behavior.
...
The Spectrum maskable interrupt provides a periodic heartbeat. `SYS_TICKS` is a
32-bit unsigned frame count modulo 2^32 and is independent of wall-clock seconds,
wall-clock validity, and `TIME1.revision`.

On the 50 Hz version-1 PAL baseline, every accepted frame interrupt performs only the
bounded timing producer work: increment `SYS_TICKS`; mirror ROM `FRAMES` as specified in
Section 14.8A; when wall time is valid advance its private 0..49 subsecond accumulator
and increment `TIME1.wall_seconds` on each 50th accepted frame; update sleep-deadline
wake bookkeeping; perform only the minimum direct keyboard-matrix reads needed for the
Phase-0-frozen BREAK chord; advance `cursor_blink_divider`; on each 25th accepted frame
reset the divider and toggle the one-bit `cursor_service_parity`; and set other
interrupt-safe scheduler/event wake flags.

Successful production boot establishes valid wall time at Section-5.5 step 20 before
`EI`. Before the first accepted frame its exact public state is `wall_seconds=388368000`,
revision 0, with six-byte `TIME1` `80 06 26 17 00 00`. Frames 1..49 retain that second;
frame 50 produces 388368001 exactly once. The internal invalid state remains only for
early-boot, diagnostic, and failure-path fixtures.

IM2 never performs full key translation, repeat processing, line editing, console
dispatch, cursor bitmap XOR, glyph drawing, scrolling, terminal coordinate/wrap
mutation, screen-mutation nesting, or arbitrary process preemption/context switching.
A safe non-ISR cursor consumer atomically fetches-and-clears `cursor_service_parity`;
odd accumulated 25-frame intervals toggle logical phase once, even intervals produce no
net toggle, and there is no catch-up flashing.

Wraparound comparisons for ticks use modular arithmetic. Long approved ROM critical
sections that disable interrupts (notably tape and ROM BEEPER) cause honestly missed
frame ticks and wall-clock advancement; the system does not fabricate elapsed time from
a host, emulator, RTC, network, tape timestamp, filesystem metadata, or BASIC time
source.
...
`SYS_TIME_GET` uses HL -> writable 6-byte `TIME1`:

    +0 u32 wall_seconds    local-session seconds since 1970-01-01 00:00:00
    +4 u16 revision

Successful normal cold boot initializes valid `TIME1` before PID1 observability to
`wall_seconds=388368000`, revision 0, so before the first accepted frame the exact six
public bytes are `80 06 26 17 00 00`. `SYS_TIME_GET` still returns E_AGAIN when the
kernel is in the explicit early-boot/diagnostic invalid state; that state is not the
successful production cold-boot oracle.
...
65536. Boot initialization is not a set and leaves revision 0. The first successful
set, including one that repeats the current seconds value, therefore produces revision 1.
...
The operation executes inside the balanced screen-mutation discipline of Section
13.4A. A nested scroll does not independently hide/show the cursor; only the outer
mutation bracket may remove/reconcile physical cursor XOR.

## 13.4 Software cursor

Private console/cursor state is conceptually:
    ...
    cursor_service_parity    parity of unserviced 25-frame transitions
    screen_mutation_depth    balanced private nesting depth, zero when idle

`last_col=63` in tty64 and `31` in tty32. No persistent col64, tty32 col32, or row24 is
permitted. `wrap_pending=1` implies `cursor_col==last_col`; the row remains the row of
the last printable written there.

Required cursor shapes are block, underline, and off. In tty64, block XORs only the
selected 4-pixel nibble on all eight scanlines; underline XORs only that nibble on
scanline 7; off draws nothing. The neighboring nibble and attribute byte are preserved.
tty32 uses the same logical model at 8-pixel width. XOR is reversible and no background
buffer is required.

`cursor_phase` and `cursor_drawn` are distinct. Cold console init starts logical phase
on, divider 0, service parity 0, mutation depth 0, and pending wrap clear. Temporary
cursor removal, clear, positioning, mode/shape change, graphics/UDG mutation, or shape
off does not reset logical phase or the blink clock.

Each accepted PAL frame advances `cursor_blink_divider`; on the 25th accepted frame IM2
resets it to 0 and toggles `cursor_service_parity`. IM2 performs no bitmap XOR. A safe
consumer atomically fetches-and-clears parity; consumed 1 toggles logical phase exactly
once and consumed 0 leaves it unchanged. Thus delays of 1/2/3/4 blink intervals produce
net phase parity 1/0/1/0 without replaying historical flashes. An interrupt posted after
the atomic consume belongs to the next service opportunity and cannot be erased by the
completed consume.

Default shapes remain:
...
## 13.4A Console-managed rendering and exact mutation nesting

All kernel-owned screen-content mutation uses one balanced nesting counter. There is no
alternative cursor-bracketing policy.
...
Conceptually:
    ...
    screen_end():
        ...
        with cooperative-switch exclusion still held:
            atomically fetch-and-clear cursor_service_parity
...
Only the outermost begin may hide a visible cursor, and only its matching outermost end
may consume deferred parity, reconcile physical XOR, and release cooperative-switch
exclusion. Nested glyph, scroll, clear, OS graphics/UDG, or terminal-mode operations do
not perform independent cursor XOR or exclusion release. Underflow, unbalanced exit, or
silent depth wrap is a defect and must be detected deterministically.

Direct deferred cursor service outside a content mutation is the sole non-nesting path:
it enters the same cooperative-switch exclusion, atomically consumes parity, reconciles
the cursor, and leaves the exclusion without changing `screen_mutation_depth`.

A pending parity request is serviced at the first safe non-ISR point with
`screen_mutation_depth==0` at which bitmap mutation is legal. At minimum these service
points are: outermost `screen_end`; kernel re-entry after kernel-stack/exclusion
preconditions are established; immediately before idle HALT when a request is already
pending and immediately after HALT/interrupt wake before ordinary scheduler/input work;
and the blocked tty input-owner path after each wake/re-entry and before key delivery,
re-block/yield, or return. Arbitrary non-yielding user code may delay physical service
until it eventually re-enters the kernel; parity preserves the exact net phase without
catch-up flashing.
...
For both tty modes, printable bytes 0x20..0x7F use one deferred-wrap byte-state machine.
Before drawing a printable, if `wrap_pending` is set, clear it and advance to column 0
of the next row, or at row 23 scroll exactly one text row and remain on row 23. Draw at
the resulting real cell. If the drawn cell is before `last_col`, advance one column and
leave pending clear; if it is `last_col`, keep that real final-cell coordinate and set
`wrap_pending=1`. Filling the final cell therefore never itself wraps or scrolls; only
the next printable resolves the pending wrap. PUTCHAR, one WRITE, split WRITEs, and mixed
WRITE/PUTCHAR delivery must be byte/state identical.

A cursor-moving control first cancels pending wrap, then applies its own semantics:

- CR (0x0D): column 0, row unchanged, no implicit scroll;
- LF (0x0A): column 0 and next row; at row 23 scroll exactly once and remain row 23;
- BS (0x08): decrement column when greater than 0; col0 remains col0 and never wraps
  backward;
- TAB (0x09): move to the next logical multiple-of-8 column if it is within
  `last_col`; otherwise move to col0 of the next row or scroll exactly once at row23;
- FF (0x0C): clear the text display and home to (0,0).

Successful `SYS_CON_SETPOS`, clear/home, mode switch, and reset clear pending wrap.
`SETPOS` validates before side effects; invalid row24, tty64 col64, or tty32 col32 leaves
coordinates, pending state, cursor XOR, and screen bytes unchanged. Setting position to
the last real cell alone leaves pending clear. `SYS_CON_GETPOS` returns only real row/col
and neither exposes nor changes pending state. Shape/blink changes do not clear pending.
An ignored unsupported control follows its existing policy and does not resolve pending
merely by being observed. Returning from WRITE never resolves pending wrap by itself.

The public text encoding remains byte-oriented. Host-built system text assets map the
Unicode source character `©` to target byte 0x7F; bytes 0x20-0x7E retain their normal
printable ASCII-like values.
...
cursor editing keys mapped from available combinations, the ESC-equivalent `EDIT` chord,
and BREAK/cancel. Command history is deferred from version 1. A waiting console read
yields cooperatively instead of busy-spinning.

Version 1 freezes the physical 48K `EDIT` chord, `CAPS SHIFT + 1`, as the canonical
input ESC-equivalent. Ordinary task-context keyboard decoding presents that chord as
target byte `0x1B` through `SYS_CON_GETKEY` and reads from `/dev/tty`. No other key or
key chord produces `0x1B`. The mapping is identical in tty32 and tty64 and does not
expose a Sinclair BASIC `EDIT` or extended-mode token; `CAPS SHIFT + SYMBOL SHIFT` is
not an ESC alias. Byte `0x1B` is an input value only: Section 13.5A gives it no special
output/control behavior.

The physical 48K BREAK chord is `CAPS SHIFT + SPACE`. BREAK is sampled minimally by
IM2 into `break_pending` as specified in Section 21 and never decodes as target byte
`0x1B`. At the next safe kernel boundary, PID1 tty ownership cancels shell line input;
a live PID2..7 tty owner receives CANCEL_PENDING. BREAK does not imply pipeline
process-group cancellation. Because scheduling is cooperative, a process that never
calls the kernel cannot be safely killed without preemption.
...
## 19.2 Fast-loader prefix versus M48O stream

The production TZX begins with the reviewed fast-loader bootstrap and exact 8192-byte kernel payload at 0xE000-0xFFFF.
There is no production SCREEN$/`zx48uxscr` object and no release TAP counterpart. After handoff at 0xE003, ZX-UX reads
its existing sequential M48O stream. The loader must leave the first M48O object positioned for exactly one kernel read.
All M48O logical format, CRC, RAW/PACKED, case, type, and target semantics remain unchanged.
...
Fixed boot prefix:
    ...
    reviewed fast-loader TZX bootstrap
    exact 8192-byte kernel payload at 0xE000-0xFFFF
    sh
    font4x8
    issue
    crontab
...
No separate SCREEN$ logical file exists in the current production order.
...
The first five M48O objects after the fast-loaded kernel are fixed as exact lower-case
...
A base 48K Spectrum has no persistent calendar RTC. ZX-UX keeps independent frame uptime
and a deterministic, non-persistent software wall clock. On every successful normal cold
boot the kernel initializes valid wall time to exactly `1982-04-23 00:00:00`:
...
This is deterministic session state and restarts from the same value on every cold boot;
it is not a claim of historical midnight launch time and is not recovered from any RTC,
host, emulator, tape, BASIC, filesystem, or network clock. Version 1 still supports only
1970..2099 Gregorian dates and no timezone database. An internal invalid state remains
available only before boot initialization or for explicit diagnostic/failure fixtures.

Required forms remain:

    date
    date -s "YYYY-MM-DD hh:mm:ss"

Display is exactly `YYYY-MM-DD hh:mm:ss`. Normal successful boot never prints
`date: not set`; that literal remains valid only for an explicit diagnostic invalid-state
fixture. `date -s` validates all fields, calls SYS_TIME_SET, increments revision on every
successful set including a repeated value, and resets the private subsecond accumulator.
On the PAL baseline the clock advances one second per 50 accepted IM2 frames; approved
ROM critical sections may cause honestly missed time, so this is not RTC-grade.
...
Calendar schedules and @hourly/@daily require valid TIME1. Successful normal boot
provides valid revision-0 wall time before a user cron daemon can evaluate rules, so they
do not wait for manual `date -s`; an explicit diagnostic/early-invalid fixture still
suppresses those calendar forms. Cron remembers the last `(wall-minute,revision)` key it
evaluated and runs a matching calendar rule at most once per such key. The deterministic
boot epoch therefore cannot double-fire one revision-0 minute. When TIME1 revision
changes after `date -s`, the dedupe key is cleared and only the newly observed current
minute is evaluated; there is no catch-up. Long tape/ROM interruptions likewise produce
no catch-up. `@boot` remains independent of wall time and means once when that particular
...
`cal [month [year]]`
    prints a Gregorian month. With no arguments it uses the current valid wall-clock
    month/year; one `month` argument uses the current valid wall year. Successful normal
    boot satisfies that validity from the deterministic epoch. In an explicit
    early-boot/diagnostic invalid fixture these forms retain the fail-closed date-not-set
    behavior. Two arguments require month 1..12 and year 1970..2099 and do not require
    wall time. Other arity/ranges return E_INVAL.
...
recognize the frozen physical BREAK chord, `CAPS SHIFT + SPACE`, and only sets
`break_pending`; full key decoding remains task context. BREAK is never the ESC-equivalent
and never produces target byte `0x1B`. The distinct physical `EDIT` chord,
`CAPS SHIFT + 1`, is decoded only in ordinary task context as specified in Section 13.8.
At the next syscall/yield/input boundary, if tty input owner is PID1, BREAK cancels the
current shell input line. If owner is a live PID2..7, the kernel sets CANCEL_PENDING on
that owner. BREAK does not promise process-group/pipeline cancellation for stages that do
not own tty input; `kill` is the explicit control.
...
The current mode must be unambiguous. Insert mode displays the compact literal
`-- INSERT --` status indicator on the bottom status line. The canonical editor escape
input is target byte `0x1B`, produced by the physical `CAPS SHIFT + 1` `EDIT` chord as
defined in Section 13.8. In insert mode, `0x1B` returns to normal mode, removes the exact
insert indicator, restores the block cursor, and inserts no byte. In command-line mode,
`0x1B` cancels the unfinished command without executing it and returns to normal mode.
In normal mode, `0x1B` remains in normal mode, cancels any incomplete multi-key normal
command, and does not modify the buffer. BREAK is not an editor escape and retains the
cooperative-cancellation meaning from Section 21.
...
case-sensitive by default, consistent with the rest of ZX-UX. Interactive `/text` entry
uses the command-line input state; `0x1B` cancels an unfinished search, returns to normal
mode, and leaves the previous search state and buffer unchanged.
...
Revision 17 additionally requires the minimum source facilities needed to assemble canonical kernel source or a deterministic source-only projection into real OBJ1; preassembled kernel payload substitution is forbidden.
...
- resolve symbols;
- provide the minimum fixed/absolute kernel-image output mode required by the Revision-17 self-rebuild proof, consuming native OBJ1 and writing exactly 8192 bytes away from the resident executing kernel;
...
`getchar()` is a byte-preserving wrapper over process stdin. When stdin is `/dev/tty`,
the Section-13.8 physical `CAPS SHIFT + 1` `EDIT` chord is therefore returned as integer
value `0x1B`/27 exactly. The `CAPS SHIFT + SPACE` BREAK chord remains cooperative
cancellation and is not returned by `getchar()` as 27. No C48 text-input helper may
case-fold, translate, swallow, or synthesize this ESC-equivalent byte.
...
On the PAL/50 Hz version-1 target, the Spectrum frame interrupt performs only bounded
producer/bookkeeping work:

- increment the independent 32-bit monotonic frame counter modulo 2^32;
- exact mirror increment of ROM `FRAMES` at 23672-23674;
- when wall time is valid, advance the private 0..49 subsecond accumulator and wall
  seconds; successful production boot makes this valid from the deterministic epoch
  before the first `EI`;
- sleep deadline wake flags;
- minimal direct keyboard-matrix sampling sufficient only to recognize the
  Phase-0-frozen BREAK chord;
- increment `cursor_blink_divider` and, each 25th accepted frame, reset the divider and
  toggle `cursor_service_parity`;
- scheduler-needed wake bookkeeping proven interrupt-safe.

The IM2 handler never invokes ROM KEYBOARD/keyboard decoding and never performs full key
translation, repeat processing, line editing, console dispatch, bitmap cursor XOR, glyph
draw, scroll, row/column/wrap mutation, screen-mutation nesting, allocation, cassette
I/O, shell/compiler logic, or arbitrary process context switching. BREAK only sets its
pending state. Cursor blink only produces divider/parity state; safe ordinary kernel
context atomically consumes parity and reconciles the bitmap.

Both alternate-register preservation paths must include this timing producer and retain
an exact instruction-boundary/cycle and stack contract. Cooperative task switching
...
`date` implements Section 20.8. `cal` with no arguments requires valid wall time;
successful normal boot supplies it, while an explicit diagnostic/early-invalid fixture
prints `cal: date not set`. `cal month year` works independently for valid 1970..2099
input. `uptime` formats the independent 32-bit frame counter modulo 2^32 (about 994 days
at 50 Hz), so it remains usable even in an explicit invalid-wall-clock fixture but
deliberately wraps.
...
- the deepest CR-1 console path: outer and nested screen-mutation brackets, cursor
  hide/reconcile/direct-service helpers, a control that scrolls or clears, and an
  interrupt at every interruptible point using whichever ISR preservation path consumes
  more stack;
...
Current product/pre-release construction is owned by deterministic `tools-host/release-tzx`.
It consumes a freshly rebuilt exact 8192-byte kernel, injects it into the reviewed fast-loader TZX template,
regenerates payload-dependent integrity fields, appends the canonical M48O stream where applicable, and emits TZX only.

Historical `tools-host/maketap` remains replay-only for the completed historical SCREEN$/TAP epoch.
Pasmo 0.5.3-7 is release-scoped and is not part of the shared certified toolchain/bootstrap/runtime cache.

Host tooling is not target runtime. Final release/pre-release acceptance additionally requires the native self-rebuild proof in §5.5.
...
- ordinary Spectrum `LOAD ""` starts the official TZX;
- reviewed fast-loader executes in real time with fastload/traps/loader shortcuts disabled;
- no SCREEN$/`zx48uxscr` block is present;
- loader owns the reviewed 24x32 display;
- exact TZX-reconstructed kernel is 8192 bytes at 0xE000-0xFFFF;
- final dispatch is exactly 0xE003;
- loader leaves the first post-kernel M48O object correctly positioned where present;
- deterministic double-build is byte-identical;
- host-built kernel == TZX-embedded kernel == native-rebuilt kernel at every byte offset 0..8191;
- all three kernel SHA-256 values are recorded and equal;
- native `as` parses source and emits real OBJ1; native `ld` consumes native OBJ1 into non-executing storage;
- genuine `source -> native as -> OBJ1 -> native ld -> executable -> run` passes;
- controlled source/input mutation makes the equality proof fail.
...
- CRC-16/CCITT-FALSE still matches `123456789` -> 0x29B1;
- product distribution is TZX-only and emits no release TAP;
- no production SCREEN$/`zx48uxscr` block exists;
- fast kernel transport is exactly 24 payload chunks totaling 8192 bytes and covering exactly 0xE000-0xFFFF;
- payload-dependent integrity fields are independently derived and verified;
- stale-integrity corrupted payload is rejected and a correctly recomputed controlled fixture is accepted;
- final handoff is exactly 0xE003;
- first post-kernel M48O object is positioned/consumed exactly once;
- all other Revision-16 M48O save/load/scan/RAW/PACKED/case/BREAK semantics remain unchanged.
...
- all 96 F4X8 printable codes render in even and odd tty64 columns with the selected
  nibble exact and a nonzero neighbor-nibble/attribute sentinel preserved;
- tty64 row 0/23 and col 0/63 plus tty32 col 0/31 boundaries remain real cells only;
- deferred right-edge wrap: writing the final cell sets pending with no wrap/scroll, and
  the next printable wraps or scrolls exactly once before drawing;
- full-screen 1536/1537 tty64 and 768/769 tty32 printable streams produce exact
  bottom-right pending state then one exact next-byte scroll;
- one WRITE, split WRITEs, and mixed WRITE/PUTCHAR streams are byte/state identical
  across right-edge and scroll boundaries;
- 64-column scroll preserves all 23 retained text rows and never copies a visible cursor
  XOR into retained content;
- block and underline cursor XOR is exactly reversible and preserves neighbor nibble and
  attributes;
- cursor blink never modifies bitmap from IM2 interrupt context; 25-frame producer
  parity coalesces odd/even delayed transitions exactly;
- nested screen mutations hide at most once, nested ends never reconcile/release, and the
  outer end reconciles exactly once; underflow/unbalanced/depth-wrap fixtures fail;
- due cursor service is performed at first safe outer-end, kernel re-entry, idle
  before/after-HALT, and blocked tty-input wake service point without changing row/col/
  pending wrap;
- `stty cols 32` and `stty cols 64` switch predictably and clear pending wrap;
- CR/LF/BS/TAB/FF first cancel pending wrap and then match Section 13.5A exactly;
...
- normal cold boot exposes exact pre-first-frame TIME1 bytes `80 06 26 17 00 00`,
  valid revision 0/subsecond 0, while an explicit diagnostic invalid fixture returns
  E_AGAIN;
- boot-valid `date` output, valid/invalid `date -s`, repeated-set revision increment,
  and no normal-production `date: not set` oracle;
- wall-clock frame-50 rollover, leap day, and year/month/day boundaries;
- `@boot` remains independent of wall time and runs once per daemon instance;
- calendar cron is eligible from valid revision-0 boot time without prior manual set;
- no duplicate cron firing within one `(wall-minute,revision)` key;
...
- normal/insert/command-line mode transitions, including physical `EDIT`
  (`CAPS SHIFT + 1`) -> target byte `0x1B`, insert-mode exit, unfinished `:`/`/` input
  cancellation, normal-mode pending-command cancellation, and BREAK remaining distinct;
...
- H06 pins the read-only SDK at commit `9ca3c6d6b5dd4b6e2351c1800afbd47d1d77e411`
  with `usr/src` tree `f629dcc1d156b83bf08ed171b9273e3cbb621ad1` and the exact
  target-native compatibility canary `usr/src/examples/hello.c`: Git blob
  `95fa0186bda3f7636774f5885988d78a13f6450b`, 703 bytes, SHA-256
  `6f94a735f230dadf5928993f9a071f3f47e98b63d18230eef102f6ae7b63e4b2`;
- the canary is compiled from those exact `hello.c` bytes by target-native C48, linked
  by native `ld`, and executed under ZX-UX; host-only compile/link does not satisfy H06;
- its direct declaration dependency is the pinned SDK `usr/src/examples/exapi.h`,
  2056 bytes, SHA-256 `2fa0edc593832d3ab57bc105233f41fd81a02b1f3ea022cefc42da01a6084bb8`;
- execution proves `cls()`, exact `print_at(10,22,"hello from c48")`,
  exact `print_at(12,14,"zx-ux portable host sdk")`, clean return status 0, deterministic
  debugger/register/RAM/process/exit evidence, and at least one supplemental screenshot;
- P11.45 evidence records the pinned SDK commit/tree, exact canary/header identities,
  native compile/link/execute results and evidence references; P11.48/P12.26 reject a
  missing, changed, host-only, skipped, or non-executed canary.
...
- IM2 recognizes exact `CAPS SHIFT + SPACE` BREAK from raw matrix reads;
- non-BREAK key combinations, including `CAPS SHIFT + 1` `EDIT`, do not spuriously set
  `break_pending`;
- IM2 contains no call edge to ROM KEYBOARD/decoder routines;
- ordinary tty input still decodes the required key set outside interrupt context, with
  `CAPS SHIFT + 1` `EDIT` producing exactly target byte `0x1B` in tty32 and tty64, the
  same byte observable through `SYS_CON_GETKEY`, `/dev/tty` reads, and C48 `getchar()`
  when stdin is tty, no other chord producing `0x1B`, and BREAK never producing `0x1B`.

### 41.11.4 Cursor/direct screen

Acceptance must include:

1. draw tty64 text and show block/underline XOR cursor at ordinary and bottom-right
   pending-wrap positions, then hide and prove raw bitmap bytes return exactly;
2. direct OS graphics/UDG and scroll/clear paths execute under balanced nested mutation
   brackets and never preserve/copy visible cursor XOR into content;
3. divider transition occurs only each 25 accepted frames; delays of 1/2/3/4 intervals
   produce parity 1/0/1/0 and the same final logical phase without catch-up flashes;
4. inject an interrupt immediately before, within the interrupt-excluded consume window,
   and immediately after cursor parity consumption; no posted transition is erased or
   double-consumed;
5. prove IM2 never mutates bitmap, row, column, `wrap_pending`, or mutation depth and
   both `altreg_busy` preservation paths include the timing producer;
6. prove a due request before idle, on HALT wake, while tty input blocks, and immediately
   before key return is serviced at the first safe point with row/col/pending unchanged;
7. exercise shape off across multiple blink intervals, then re-enable underline/block
   and reconcile against the current logical phase without resetting the clock;
8. deliberately underflow/unbalance mutation depth and require deterministic failure
   rather than silent cursor/exclusion corruption.

### 41.11.5 Kernel stack

- instrumentation records high-water depth on every kernel/ROM/ISR stress test,
  including the deepest nested cursor/mutation/scroll-or-clear path with an interrupt at
  every interruptible point using the larger-stack ISR preservation path;
...
Mandatory release/size gates:
    ...
    pinned font4x8 payload     = 392 bytes FAST-required; final host SHA-256
                                  90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339
...
a concrete measured-memory proof, not a paper total. CR-1 cursor/wrap/mutation state
and helpers must fit these existing kernel/stack gates; they may not borrow the
0xFB00-0xFB0F guard, the mandatory >=64-byte stack margin, IM2/vector bytes, or fixed
fast/emergency reserves.
...
- tty64/tty32 console core with deferred wrap, nested mutation discipline, and parity-serviced cursor;
- FRAMES mirror + monotonic ticks;
- deterministic valid cold-boot wall clock plus diagnostic invalid state and SYS_TIME_GET/SYS_TIME_SET.
...
- SYS_TICKS is coherent four-byte modulo-2^32 state independent of wall time; IM2
  cursor timing is producer-only and safe consumers preserve parity across races;
- deferred final-cell wrap, nested mutation, safe cursor service, and the 448-byte kernel
  stack high-water limit remain satisfied by the retained historical Revision-12 Phase-1
  matrices; the Revision-16 `CAPS SHIFT + 1` `EDIT` -> `0x1B` tty mapping and its strict
  separation from `CAPS SHIFT + SPACE` BREAK are delta-certified by the mandatory R16.00
  transition bridge after Phase 2 and before Phase 3, without relabeling historical P1 evidence;
- once the P1.26 owner is implemented, normal boot-time wall state is exact epoch
  `80 06 26 17 00 00` before first accepted frame and before PID1 observability.
...
- normal, insert, and command-line modes with the Section-13.8 physical `EDIT`
  (`CAPS SHIFT + 1`) -> `0x1B` escape contract;
...
- create/edit/save/reload source entirely on target, proving `EDIT` exits insert mode
  and cancels unfinished command-line/search input while BREAK remains cancellation;
...
- `tune.c` proves the C48 BASIC-compatible `beep` wrapper including fractional
  pitch;
- C48 `getchar()` preserves tty input bytes exactly: physical `EDIT` returns 27/`0x1B`
  when stdin is `/dev/tty`, while BREAK remains cancellation and never masquerades as ESC.
...
- byte-exact final reference system TZX following Section 19.8, including the frozen
...
From a powered/reset 48K configuration and the official production TZX:
...
2. verify the real-time reviewed fast-loader path, loader-owned display, exact 8192-byte kernel transfer, and permanent 0xE003 handoff with fastload/traps/loader shortcuts disabled;
...
9. capture one valid-format `date` line from the deterministic boot-valid wall clock
   and retain companion machine-state evidence proving the exact TIME1 returned to that
   invocation; exercise a valid `date -s` revision change and execute one `@boot` or
   calendar `cron`/`crontab` job, including revision-0 calendar eligibility;
...
15. broader native rebuild coverage beyond the mandatory exact kernel self-rebuild.
16. increasingly self-hosted whole-system rebuild beyond the mandatory kernel proof.
...
4. The official product release is one deterministic TZX started with ordinary Spectrum `LOAD ""`.
5. The production bootstrap is the reviewed fast-loader path; there is no production SCREEN$/`zx48uxscr` file and no release TAP.
6. The loader owns the reviewed 24x32 loading display.
7. The freshly rebuilt kernel is exactly 8192 bytes at 0xE000-0xFFFF and is embedded exactly in the TZX.
8. Successful dispatch at 0xE003 is permanent; ZX-UX does not return to BASIC.
9. `zx_boot_entry` immediately establishes SP=0xFD00.
10. The loader consumes exactly its own blocks and leaves the first post-kernel M48O object positioned for the kernel.
11. Release acceptance proves exact 8192-byte host/TZX/native identity, genuine native source->as->OBJ1->ld->run, and a controlled mismatch negative.
...
63. The default environment includes exact `PATH=/bin:.`, `HOME`, `USER`, and
    `SHELL=/bin/sh`.
64. Successful normal cold boot initializes valid wall time to exactly
    `1982-04-23 00:00:00`, TIME1 `80 06 26 17 00 00`, revision 0/subsecond 0;
...
111. BREAK (`CAPS SHIFT + SPACE`) is a cooperative tty-owner cancellation request
    sampled by IM2, not hardware preemption, a process-group signal, or the ESC-equivalent.
...
119. `/tmp/.cron.lock` is exactly ASCII PID digit `2`..`7` plus LF; any other payload is
    stale/invalid.
120. `vi` closes its source input handle after load and obeys the exact mode/escape,
    dirty-state, retarget, transactional-write, and quit semantics in Section 22.
...
135. IM2 performs mandatory minimal direct `CAPS SHIFT + SPACE` BREAK sampling but
    never full ROM keyboard decoding; ordinary task-context tty decoding maps only the
    physical `CAPS SHIFT + 1` `EDIT` chord to target byte `0x1B`, identically in tty32
    and tty64.
...
140. tty32/tty64 printable output uses deferred right-margin wrap; no persistent
    cursor coordinate ever lies outside a real cell.
141. `wrap_pending` implies the current column is the mode's final real column; writing
    that final cell does not itself wrap or scroll.
142. All kernel-owned screen-content mutations use one balanced nesting counter; only
    the outermost begin/end pair may hide/reconcile cursor XOR and release exclusion.
143. IM2 produces cursor blink timing only through a 0..24 divider and one-bit service
    parity; bitmap cursor XOR never occurs in interrupt context.
144. Cursor parity consumption is atomic with respect to IM2 and coalesces delayed blink
    intervals by parity without catch-up flashing.
145. Safe cursor service includes outer mutation end, kernel re-entry, idle before/after
    HALT, and blocked tty-input wake/return paths.
146. CR-1 console/cursor paths must retain kernel-stack worst-case consumption <=448
    bytes with the FB00-FB0F guard and >=64-byte margin untouched.
147. Cold-boot epoch initialization occurs exactly once after wall-clock storage is
    initialized and before first runtime EI/PID1 observability; it is not SYS_TIME_SET.
148. No host/emulator/RTC/tape/filesystem/BASIC/network source may alter the deterministic
    cold-boot epoch.
149. `SYS_TICKS` remains independent coherent modulo-2^32 frame state and is never
    initialized from epoch seconds or TIME1 revision/validity.
150. H06 SDK `usr/src/examples/hello.c` target-native canary admission is a separate
    P11.45/P11.48/P12.26 compiler acceptance obligation; host-only compilation or
    screenshot-only evidence cannot satisfy it.
...
29. If the module ships a command/tool/demo, is its canonical user-facing name
    lower-case?
30. If it is `vi`, does the change preserve documented modal, `EDIT`/`0x1B` escape,
    BREAK-separation, and case-sensitive command semantics?
...
host computer participating in the runtime workflow:
    ...
    execute the reviewed fast loader and loader-owned 24x32 loading display;
    fast-load the exact 8192-byte kernel image into 0xE000-0xFFFF;
...
The user presses PLAY. The reviewed fast loader runs in real time without fastload/trap shortcuts, initializes its loading display, transfers the exact rebuilt 8192-byte kernel, dispatches permanently to 0xE003, then the five bootstrap M48O resources load. `tty64` displays:
    ...
    Search for ZXUS
    $ date
    <captured YYYY-MM-DD hh:mm:ss matching retained exact TIME1 evidence>
