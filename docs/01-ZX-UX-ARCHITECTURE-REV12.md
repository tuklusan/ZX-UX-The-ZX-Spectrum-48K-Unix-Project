...
Revision: 12 - combined CR-1/CR-2 architecture rebaseline with H06 plan synchronization
...
- preserves explicit wall-clock-invalid behavior only for early-boot, diagnostic,
  and failure-path fixtures rather than the normal production boot oracle; and
...
- deferred final-cell wrap, nested mutation, safe cursor service, and the 448-byte kernel
  stack high-water limit pass their deterministic Phase-1 matrices;
