...
Revision: 16 - Phase-3 bridge plan-identity correction
...
20. atomically initialize the software wall clock to valid
    `1982-04-23 00:00:00`: `wall_seconds=388368000` (`0x17260680`), revision 0,
    private subsecond frame accumulator 0. The public pre-first-frame `TIME1` bytes
    are exactly `80 06 26 17 00 00`. This is deterministic session state, not a
    `SYS_TIME_SET`, and it does not initialize or derive `SYS_TICKS`;
21. create PID 0 idle/kernel context;
22. construct the canonical cold-boot process blocks for PID 1: ARG1 has
...
23. load and validate `font4x8`; a PACKED tape representation is decoded directly
...
25. load `crontab` into `/etc/crontab` and validate its syntax or empty state;
    its mutable ETC object may remain RAW or validated PACKED storage;
26. load and validate `bincat`; a PACKED tape representation is decoded directly
    into final pinned COLD BCAT metadata and the packed transport bytes are
    discarded;
27. initialize `tty64` as the default interactive console using `font4x8`; initialize
    cursor phase on, blink divider 0, cursor-service parity 0, wrap pending clear,
    and screen-mutation depth 0;
28. create the already validated/allocated `sh` image as PID 1 with cwd ROOT and
...
29. `EI`; this is the first point at which a normal accepted frame may advance the
    already-valid boot epoch or the independent frame counter;
30. enter the scheduler.

The wall-clock initialization in step 20 is atomic with respect to accepted IM2 frames:
no observable state may combine launch-epoch seconds with stale validity, revision, or
subsecond state, and no first accepted frame may be lost or double-counted. An internal
invalid wall-clock state may exist before step 20 or on an explicit diagnostic/failure
path, but it is never PID1-observable on successful production boot.

The IM2 table/trampoline, boot trampolines, deterministic wall-clock state, `sh`,
`font4x8`, `issue`, `crontab`, `bincat`, and ownership bounds are validated before
interrupts are enabled. Any mismatch is a boot failure.
...
- the exact post-kernel M48O sequence is `sh`, `font4x8`, `issue`, `crontab`,
  `bincat`;
- `font4x8` is FAST_REQUIRED, pinned, structurally valid F4X8, and final release
  logical bytes are exactly 392 bytes with host SHA-256
  `90f6818cf81cf3f13509cff32c091075691195d9638dbe801d12daceec1c9339`;
- before first runtime `EI`, wall time is atomically valid at exact `TIME1` bytes
  `80 06 26 17 00 00`, revision 0, subsecond 0, while SYS_TICKS remains independent;
