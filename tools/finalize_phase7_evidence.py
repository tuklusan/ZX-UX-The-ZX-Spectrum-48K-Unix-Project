...
def main():
    ...
    try:
        ...
        exact={"step":"P7.21","action":"result","status":"PASS","pass_marker":P721,
        ...
        if not isinstance(source,str) or len(source)!=40: raise E("P7.21 result source")
        phase_lock=res.get("toolchain_lock_sha256")
        if not isinstance(phase_lock,str) or len(phase_lock)!=64: raise E("P7.21 result toolchain")
        names=[]; manifest={}
        for n in range(1,22):
            step=f"P7.{n:02d}"
            for action in ("build","test"):
                ...
                if n==21 and rs!=source: raise E(f"{name}: P7.21 source mismatch")
                if not isinstance(r.get("toolchain_lock_sha256"),str): raise E(f"{name}: toolchain provenance")
                if n==21 and r.get("toolchain_lock_sha256")!=phase_lock: raise E(f"{name}: P7.21 toolchain")
                names.append(name); manifest[name]=sha(p)
        names.append("P7.21.result.json")
        manifest["P7.21.result.json"]=sha(cert/"P7.21.result.json")
        agg={
            ...
            "assertions":[
                {"name":"p7-21-build-test-result-pass","passed":True},
                {"name":"all-p7-01-through-p7-21-durable-evidence-pass","passed":True},
                {"name":"rev16-rev07-authority-pass","passed":True},
                {"name":"phase7-record-manifest-byte-exact","passed":True},
                {"name":"phase7-acceptance-bullets-pass","passed":True},
                {"name":"phase7-gate-stops-before-phase8","passed":True},
            ],
        }
        out.parent.mkdir(parents=True,exist_ok=True)
        out.write_text(json.dumps(agg,indent=2,sort_keys=True)+"\n",encoding="utf-8",newline="\n")
        print("ZX-UX PHASE 7 EVIDENCE FINALIZATION PASS")
        return 0
    except Exception as exc:
        print(f"ZX-UX PHASE 7 EVIDENCE FINALIZATION FAIL: {exc}",file=sys.stderr)
        return 1
