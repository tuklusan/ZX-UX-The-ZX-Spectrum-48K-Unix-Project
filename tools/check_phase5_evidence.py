...
PREFIX="phase5: activate Phase-5 aggregate evidence for "
P519="ZX-UX PHASE 5 ACCEPTANCE PASS"; PHASE="ZX-UX PHASE 5 CERTIFICATION PASS"
...
def activation():
 xs=git("log","--format=%H","--","v1/dist/certification/phase-5.json").splitlines()
 return xs[0] if xs else None
def validate():
 a=load(CERT/"phase-5.json")
 for k,v in {"schema":2,"phase":5,"action":"phase-result","status":"PASS","pass_marker":PHASE,"worktree_clean":True,"architecture_sha256":ARCH,"implementation_plan_sha256":PLAN}.items():
  if a.get(k)!=v: raise E(f"phase-5 {k}")
 ...
 names=[f"P5.{n:02d}.{action}.json" for n in range(1,20) for action in ("build","test")]+["P5.19.result.json"]
...
def main():
 ...
 try:
  act=activation()
  if act is None:
   if args.require_active: raise E("Phase-5 aggregate evidence has not been activated")
   print("ZX-UX PHASE 5 EVIDENCE PRE-ACTIVATION PASS"); return 0
  ...
  if changed!=["v1/dist/certification/phase-5.json"]: raise E("activation must be aggregate-only")
  original=subprocess.run(["git","show",f"{act}:v1/dist/certification/phase-5.json"],cwd=ROOT,capture_output=True).stdout
  if (CERT/"phase-5.json").read_bytes()!=original: raise E("aggregate changed after activation")
  print("ZX-UX PHASE 5 DURABLE EVIDENCE PASS"); return 0
 except Exception as exc:
  print(f"ZX-UX PHASE 5 EVIDENCE FAIL: {exc}",file=sys.stderr); return 1
