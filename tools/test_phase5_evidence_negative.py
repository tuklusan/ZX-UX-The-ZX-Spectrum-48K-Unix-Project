...
def main():
 current=run(sys.executable,"tools/check_phase5_evidence.py")
 if "PRE-ACTIVATION PASS" in current.stdout:
  required=run(sys.executable,"tools/check_phase5_evidence.py","--require-active")
  if required.returncode==0: return 1
  print("ZX-UX PHASE 5 EVIDENCE NEGATIVE PRE-ACTIVATION PASS"); return 0
 if current.returncode: return 1
 work=Path(tempfile.mkdtemp(prefix="zxux-phase5-negative-"))
 try:
  shutil.rmtree(work)
  if run("git","worktree","add","--detach",str(work),"HEAD").returncode: return 1
  target=work/"v1/dist/certification/P5.19.result.json"
  for name,change in [
   ...
   if run(sys.executable,"tools/check_phase5_evidence.py","--require-active",cwd=work).returncode==0:
    raise RuntimeError("negative case passed: "+name)
  print("ZX-UX PHASE 5 EVIDENCE NEGATIVE PASS"); return 0
 finally:
