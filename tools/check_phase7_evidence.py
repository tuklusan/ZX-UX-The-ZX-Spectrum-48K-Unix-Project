...
PREFIX="phase7: activate Phase-7 aggregate evidence for "
P721="ZX-UX PHASE 7 ACCEPTANCE PASS"
PHASE="ZX-UX PHASE 7 CERTIFICATION PASS"
...
def pre(n): return {"P6.30":"PASS"} if n==1 else {f"P7.{n-1:02d}":"PASS"}
def activation():
    xs=git("log","--format=%H","--","v1/dist/certification/phase-7.json").splitlines()
    return xs[0] if xs else None

def validate():
    a=load(CERT/"phase-7.json")
    for k,v in {"schema":2,"phase":7,"action":"phase-result","status":"PASS","pass_marker":PHASE,
        ...
        if a.get(k)!=v: raise E(f"phase-7 {k}")
    ...
    names=[f"P7.{n:02d}.{action}.json" for n in range(1,22) for action in ("build","test")]+["P7.21.result.json"]
    ...
    for n in range(1,22):
        ...
        for action in ("build","test"):
            ...
            if n==21 and r.get("toolchain_lock_sha256")!=lock: raise E(f"{name}: P7.21 toolchain")
    ...
    res=load(CERT/"P7.21.result.json")
    for k,v in {"step":"P7.21","action":"result","status":"PASS","pass_marker":P721,"source_commit":source,
                "architecture_sha256":ARCH,"implementation_plan_sha256":PLAN,"toolchain_lock_sha256":lock,
                "worktree_clean":True,"prerequisites":{"P7.20":"PASS"}}.items():
        if res.get(k)!=v: raise E(f"P7.21.result {k}")
    if manifest.get("P7.21.result.json")!=sha(CERT/"P7.21.result.json"): raise E("P7.21.result bytes")
    req={"p7-21-build-test-result-pass","all-p7-01-through-p7-21-durable-evidence-pass",
         "rev16-rev07-authority-pass","phase7-record-manifest-byte-exact",
         "phase7-acceptance-bullets-pass","phase7-gate-stops-before-phase8"}
...
def main():
    ...
    try:
        act=activation()
        if act is None:
            if args.require_active: raise E("Phase-7 aggregate evidence has not been activated")
            print("ZX-UX PHASE 7 EVIDENCE PRE-ACTIVATION PASS")
            return 0
        ...
        if changed!=["v1/dist/certification/phase-7.json"]: raise E("activation must be aggregate-only")
        original=subprocess.run(["git","show",f"{act}:v1/dist/certification/phase-7.json"],cwd=ROOT,capture_output=True).stdout
        if (CERT/"phase-7.json").read_bytes()!=original: raise E("aggregate changed after activation")
        print("ZX-UX PHASE 7 DURABLE EVIDENCE PASS")
        return 0
    except Exception as exc:
        print(f"ZX-UX PHASE 7 EVIDENCE FAIL: {exc}",file=sys.stderr)
        return 1
