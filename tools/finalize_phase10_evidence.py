...
def main():
    ...
    try:
        ...
        res=load(cert/"P10.36.result.json"); source=res.get("source_commit")
        exact={"step":"P10.36","action":"result","status":"PASS","pass_marker":P1036,
        ...
        if not isinstance(source,str) or len(source)!=40: raise E("P10.36 result source")
        phase_lock=res.get("toolchain_lock_sha256")
        if not isinstance(phase_lock,str) or len(phase_lock)!=64: raise E("P10.36 result toolchain")
        names=[]; manifest={}
        for n in range(1,37):
            step=f"P10.{n:02d}"
            for action in ("build","test"):
                ...
                if n==36 and rs!=source: raise E(f"{name}: P10.36 source mismatch")
                if not isinstance(r.get("toolchain_lock_sha256"),str): raise E(f"{name}: toolchain provenance")
                if n==36 and r.get("toolchain_lock_sha256")!=phase_lock: raise E(f"{name}: P10.36 toolchain")
                names.append(name); manifest[name]=sha(p)
        names.append("P10.36.result.json"); manifest["P10.36.result.json"]=sha(cert/"P10.36.result.json")
        agg={"schema":2,"phase":10,"action":"phase-result","status":"PASS","pass_marker":PHASE,
             ...
             "assertions":[
               {"name":"p10-36-build-test-result-pass","passed":True},
               {"name":"all-p10-01-through-p10-36-durable-evidence-pass","passed":True},
               {"name":"rev16-rev07-authority-pass","passed":True},
               {"name":"phase10-record-manifest-byte-exact","passed":True},
               {"name":"phase10-acceptance-bullets-pass","passed":True},
               {"name":"phase10-gate-stops-before-phase11","passed":True},
             ]}
        out.parent.mkdir(parents=True,exist_ok=True)
        out.write_text(json.dumps(agg,indent=2,sort_keys=True)+"\n",encoding="utf-8",newline="\n")
        print("ZX-UX PHASE 10 EVIDENCE FINALIZATION PASS"); return 0
    except Exception as exc:
        print(f"ZX-UX PHASE 10 EVIDENCE FINALIZATION FAIL: {exc}",file=sys.stderr); return 1
