...
PREFIX="phase9: activate Phase-9 aggregate evidence for "
...
def activation():
    xs=git("log","--format=%H","--","v1/dist/certification/phase-9.json").splitlines()
...
def validate():
    ...
    for k,v in {"step":"P9.24","action":"result","status":"PASS","pass_marker":P924,"source_commit":source,
                ...
                "worktree_clean":True,"prerequisites":{"P9.23":"PASS"}}.items():
...
def main():
    ...
    try:
        act=activation()
        if act is None:
            if args.require_active: raise E("Phase-9 aggregate evidence has not been activated")
            print("ZX-UX PHASE 9 EVIDENCE PRE-ACTIVATION PASS")
            return 0
        ...
        if changed!=["v1/dist/certification/phase-9.json"]: raise E("activation must be aggregate-only")
        original=subprocess.run(["git","show",f"{act}:v1/dist/certification/phase-9.json"],cwd=ROOT,capture_output=True).stdout
        if (CERT/"phase-9.json").read_bytes()!=original: raise E("aggregate changed after activation")
        print("ZX-UX PHASE 9 DURABLE EVIDENCE PASS")
        return 0
    except Exception as exc:
        print(f"ZX-UX PHASE 9 EVIDENCE FAIL: {exc}",file=sys.stderr)
        return 1
