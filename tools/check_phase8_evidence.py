...
PREFIX="phase8: activate Phase-8 aggregate evidence for "
P840="ZX-UX PHASE 8 ACCEPTANCE PASS"
PHASE="ZX-UX PHASE 8 CERTIFICATION PASS"
...
def pre(n): return {"P7.21":"PASS"} if n==1 else {f"P8.{n-1:02d}":"PASS"}
def activation():
    xs=git("log","--format=%H","--","v1/dist/certification/phase-8.json").splitlines()
...
def validate():
    ...
    names=[f"P8.{n:02d}.{action}.json" for n in range(1,41) for action in ("build","test")]+["P8.40.result.json"]
...
def main():
    ...
    try:
        act=activation()
        if act is None:
            if args.require_active: raise E("Phase-8 aggregate evidence has not been activated")
            print("ZX-UX PHASE 8 EVIDENCE PRE-ACTIVATION PASS")
            return 0
        ...
        if changed!=["v1/dist/certification/phase-8.json"]: raise E("activation must be aggregate-only")
        original=subprocess.run(["git","show",f"{act}:v1/dist/certification/phase-8.json"],cwd=ROOT,capture_output=True).stdout
        if (CERT/"phase-8.json").read_bytes()!=original: raise E("aggregate changed after activation")
        print("ZX-UX PHASE 8 DURABLE EVIDENCE PASS")
        return 0
    except Exception as exc:
        print(f"ZX-UX PHASE 8 EVIDENCE FAIL: {exc}",file=sys.stderr)
        return 1
