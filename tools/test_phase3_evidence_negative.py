...
def main() -> int:
    current = run(sys.executable, "tools/check_phase3_evidence.py")
    if "PRE-ACTIVATION PASS" in current.stdout:
        required = run(sys.executable, "tools/check_phase3_evidence.py", "--require-active")
        if required.returncode == 0:
            return 1
        print("ZX-UX PHASE 3 EVIDENCE NEGATIVE PRE-ACTIVATION PASS")
        return 0
    if current.returncode:
        return 1

    work = Path(tempfile.mkdtemp(prefix="zxux-phase3-evidence-negative-"))
    try:
        shutil.rmtree(work)
        ...
        target = work / "v1/dist/certification/P3.21.result.json"
        for name, change in cases:
            run("git", "reset", "--hard", "HEAD", cwd=work)
            run("git", "clean", "-fd", cwd=work)
            change(target)
            check = run(sys.executable, "tools/check_phase3_evidence.py", "--require-active", cwd=work)
            if check.returncode == 0:
                raise RuntimeError(f"negative case passed: {name}")
        print("ZX-UX PHASE 3 EVIDENCE NEGATIVE PASS")
        return 0
    finally:
