...
def main():
    ...
    try:
        ...
        exact={"step":"P6.30","action":"result","status":"PASS","pass_marker":P630,
        ...
        if not isinstance(source,str) or len(source)!=40: raise E("P6.30 result source")
        phase_lock=res.get("toolchain_lock_sha256")
        if not isinstance(phase_lock,str) or len(phase_lock)!=64: raise E("P6.30 result toolchain")
        names=[]; manifest={}
        for n in range(1,31):
            step=f"P6.{n:02d}"
            for action in ("build","test"):
                ...
                if n==30 and rs!=source: raise E(f"{name}: P6.30 source mismatch")
                if not isinstance(r.get("toolchain_lock_sha256"),str): raise E(f"{name}: toolchain provenance")
                if n==30 and r.get("toolchain_lock_sha256")!=phase_lock: raise E(f"{name}: P6.30 toolchain")
                names.append(name); manifest[name]=sha(p)
        names.append("P6.30.result.json")
        manifest["P6.30.result.json"]=sha(cert/"P6.30.result.json")
        agg={
            "schema":2,"phase":6,"action":"phase-result","status":"PASS","pass_marker":PHASE,
            ...
            "assertions":[
                {"name":"p6-30-build-test-result-pass","passed":True},
                {"name":"all-p6-01-through-p6-30-durable-evidence-pass","passed":True},
                {"name":"rev16-rev07-authority-pass","passed":True},
                {"name":"phase6-record-manifest-byte-exact","passed":True},
                {"name":"phase6-acceptance-bullets-pass","passed":True},
                {"name":"phase6-gate-stops-before-phase7","passed":True},
            ],
        }
        out.parent.mkdir(parents=True,exist_ok=True)
        out.write_text(json.dumps(agg,indent=2,sort_keys=True)+"\n",encoding="utf-8",newline="\n")
        print("ZX-UX PHASE 6 EVIDENCE FINALIZATION PASS")
        return 0
    except Exception as exc:
        print(f"ZX-UX PHASE 6 EVIDENCE FINALIZATION FAIL: {exc}",file=sys.stderr)
        return 1
