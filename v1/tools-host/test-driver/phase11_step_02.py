...
def dispatch(root, action, step, *, sha256_file, run_command, require_project_tool):
    ...
    for marker in (
        "compiler/c48/preprocessor.py","compiler/c48/lexer.py",
        "compiler/c48/parser.py","compiler/c48/typesys.py",
        "compiler/c48/semantics.py","compiler/c48/limits.py",
        "compiler/c48/errors.py","compiler/c48/float5.py",
        "compiler/c48/memory.py","compiler/c48/vm.py",
...
''',encoding="utf-8",newline="\n")
    ...
    result=run_command([assembler,"--nologo","--sym=p1102-streaming.sym",fixture.name],cwd=build,timeout_seconds=30)
    require(not result.timed_out and result.exit_code==0,f"P11.02 assemble: {result.stderr or result.stdout}")
    ...
    syms=phase3_open_descriptions._symbols(
        ...
        ("p1102_positive","p1102_capacity","p1102_overlength","p1102_expr",
         "p1102_window_overflow","cc_workspace_begin","cc_workspace_end")
    )
    require(syms["cc_workspace_end"]-syms["cc_workspace_begin"]<=2048,"P11.02 measured workspace >2KiB")

    assertions=[
        {"name":"source-window-fixed-64","passed":c["CC_SOURCE_WINDOW_SIZE"]==64},
        {"name":"separate-bounded-symbol-tables","passed":all(c[k]==32 for k in ("CC_GLOBAL_CAPACITY","CC_LOCAL_CAPACITY","CC_LABEL_CAPACITY"))},
        {"name":"identifier-visible-limit-15","passed":c["CC_IDENT_MAX"]==15},
        {"name":"expression-tree-fixed-bound","passed":c["CC_EXPR_CAPACITY"]==8},
        {"name":"measured-core-workspace-under-2k","passed":syms["cc_workspace_end"]-syms["cc_workspace_begin"]<=2048},
    ...
    if action=="test":
        ...
        def good(symbol):
            return b"\xF3"+phase1._ld_sp(0xBFC0)+phase1._call(syms[symbol])+phase1._jp_c(FAIL_PC)+phase1._jp(PASS_PC)
        def expected_fail(symbol):
            return b"\xF3"+phase1._ld_sp(0xBFC0)+phase1._call(syms[symbol])+b"\xD2"+phase1._word(FAIL_PC)+phase1._jp(PASS_PC)
        commands.append(run_sna(root,good("p1102_positive"),patch=patch))
        for symbol in ("p1102_capacity","p1102_overlength","p1102_expr","p1102_window_overflow"):
            commands.append(run_sna(root,expected_fail(symbol),patch=patch))
        assertions += [
            {"name":"fuse-stream-to-obj1-stage","passed":True},
            {"name":"fuse-case-sensitive-identifiers-coexist","passed":True},
            {"name":"fuse-15-char-identifier-accepted","passed":True},
            {"name":"fuse-16-char-identifier-rejected","passed":True},
            {"name":"fuse-symbol-capacity-fails-closed","passed":True},
            {"name":"fuse-expression-capacity-released","passed":True},
            {"name":"fuse-source-window-overflow-fails-closed","passed":True},
            {"name":"failure-preserves-output-marker-and-guards","passed":True},
        ]

    hashes={
        "v1/src/tools/cc.asm":sha256_file(cc_path),
        "v1/build/p1102-main.bin":sha256_file(build/"p1102-main.bin"),
        "v1/tools-host/test-driver/phase11_step_02.py":sha256_file(root/"v1/tools-host/test-driver/phase11_step_02.py"),
        "v1/tests/compiler/sdk-reference/SDK-PROVENANCE.md":sha256_file(root/"v1/tests/compiler/sdk-reference/SDK-PROVENANCE.md"),
        "v1/dist/certification/P11.01.build.json":sha256_file(root/"v1/dist/certification/P11.01.build.json"),
        "v1/dist/certification/P11.01.test.json":sha256_file(root/"v1/dist/certification/P11.01.test.json"),
    }
