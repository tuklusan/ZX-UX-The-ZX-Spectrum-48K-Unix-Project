...
def _source_contract(root: Path) -> list[dict[str, object]]:
    ...
    return [
        ...
        {"name": "cold-defaults-phase-divider-parity-depth", "passed": all(token in init for token in ("ld a,tty_cursor_underline", "ld (tty_cursor_shape),a", "ld a,1", "ld (cursor_phase),a", "ld (cursor_service_parity),a", "ld (tty_wrap_pending),a", "ld (screen_mutation_depth),a", "ld (cursor_blink_divider),a"))},
        {"name": "outer-begin-hides-only-drawn-cursor", "passed": "inc (hl)" in screen_begin and "jr nz,zx48_screen_begin_done" in screen_begin and "tty_cursor_visible" in screen_begin and "zx48_cursor_xor" in screen_begin},
        ...
        {"name": "atomic-parity-fetch-clear", "passed": all(token in service for token in ("ld a,(screen_mutation_depth)", "zx48_cursor_service_core:", "ld a,i", "di", "ld a,(cursor_service_parity)", "ld (cursor_service_parity),a", "ld a,(cursor_phase)", "xor b", "ld (cursor_phase),a", "jp po,zx48_cursor_reconcile", "ei"))},
        {"name": "safe-service-detects-unbalanced-depth", "passed": "jr nz,zx48_cursor_depth_panic" in service},
        {"name": "screen-brackets-preserve-coordinate-pairs", "passed": all(token in screen_begin for token in ("push bc", "push de", "push hl", "pop hl", "pop de", "pop bc")) and all(token in screen_end for token in ("push bc", "push de", "push hl", "pop hl", "pop de", "pop bc"))},
        {"name": "shape-phase-reconcile-is-xor", "passed": "zx48_cursor_reconcile:" in service and "jr zx48_cursor_xor" in service},
