...
def _source(root:Path):
    ...
    return [
      ...
      {"name":"single-stage-foreground-before-tokenizer","passed":m.index("cp 1")<m.index("call sh_p710_tokenize") and m.index("sh_p710_notsup")>m.index("call sh_p710_tokenize")},
      {"name":"calc-lookup-is-exact-and-no-path-or-tape","passed":"sh_p710_lookup_calc:" in m and "SYS_STAT" not in m and "SYS_OPEN" not in m and "tape" not in m.lower()},
