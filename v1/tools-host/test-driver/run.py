...
import phase6_shell_golden
import phase6_pid1_exit
import phase6_acceptance
import phase7_graphics_abi
import phase7_pixel_helper
import phase7_plot
import phase7_draw
import phase7_circle
import phase7_point
import phase7_attr
import phase7_border
import phase7_beep
import phase7_calc
import phase7_rom_info
import phase7_shell_graphics
...
import phase8_which
import phase8_env
import phase8_hexdump
import phase8_ps
import phase8_mem
import phase8_udg
import phase8_gfxdemo
import phase8_stty
import phase8_date
import phase8_cron
...
import phase8_man
import phase8_cal
import phase8_uptime
import phase8_whoami
import phase8_uname
import phase8_fortune
import phase8_banner
import phase8_rev
import phase8_yes
import phase8_demo
import phase8_echo_regression
...
import phase9_vi_gap
import phase9_vi_unnamed
import phase9_vi_modes
import phase9_vi_motion_basic
import phase9_vi_motion_word
import phase9_vi_motion_gg
import phase9_vi_insert
import phase9_vi_open
import phase9_vi_delete
import phase9_vi_yank_put
import phase9_vi_replace_join
import phase9_vi_undo
import phase9_vi_search
import phase9_vi_ex_read
import phase9_vi_write_tx
import phase9_vi_write_state
import phase9_vi_quit
import phase9_crontab_vi
...
def dispatch(root: Path, action: str, step: str):
    ...
    if step == "P6.28":
        return phase6_shell_golden.dispatch(root, action, step, **kwargs)
    if step == "P6.29":
        return phase6_pid1_exit.dispatch(root, action, step, **kwargs)
    if step == "P6.30":
        return phase6_acceptance.dispatch(root, action, step, **kwargs)
    if step.startswith("P6."):
        raise DriverError(f"numbered Phase-6 step is not registered: {step}")
    if step == "P7.01":
        return phase7_graphics_abi.dispatch(root, action, step, **kwargs)
    if step == "P7.02":
        return phase7_pixel_helper.dispatch(root, action, step, **kwargs)
    if step == "P7.03":
        return phase7_plot.dispatch(root, action, step, **kwargs)
    if step == "P7.04":
    ...
    if step == "P7.07":
        return phase7_attr.dispatch(root, action, step, **kwargs)
    if step == "P7.08":
        return phase7_border.dispatch(root, action, step, **kwargs)
    if step == "P7.09":
        return phase7_beep.dispatch(root, action, step, **kwargs)
    if step == "P7.10":
        return phase7_calc.dispatch(root, action, step, **kwargs)
    if step == "P7.11":
        return phase7_rom_info.dispatch(root, action, step, **kwargs)
    if step == "P7.12":
        return phase7_shell_graphics.dispatch(root, action, step, **kwargs)
    if step == "P7.13":
        return phase7_udg_bank.dispatch(root, action, step, **kwargs)
    if step == "P7.14":
    ...
    if step == "P7.18":
        return phase7_beep_c48.dispatch(root, action, step, **kwargs)
    if step == "P7.19":
    ...
    if step == "P8.02":
        return phase8_cat.dispatch(root, action, step, **kwargs)
    if step == "P8.03":
        return phase8_cp.dispatch(root, action, step, **kwargs)
    if step == "P8.04":
        return phase8_mv.dispatch(root, action, step, **kwargs)
    if step == "P8.05":
        return phase8_rm.dispatch(root, action, step, **kwargs)
    if step == "P8.06":
        return phase8_pack.dispatch(root, action, step, **kwargs)
    if step == "P8.07":
    ...
    if step == "P8.10":
        return phase8_head.dispatch(root, action, step, **kwargs)
    if step == "P8.11":
        return phase8_tail.dispatch(root, action, step, **kwargs)
    if step == "P8.12":
        return phase8_cmp.dispatch(root, action, step, **kwargs)
    if step == "P8.13":
        return phase8_true.dispatch(root, action, step, **kwargs)
    if step == "P8.14":
        return phase8_false.dispatch(root, action, step, **kwargs)
    if step == "P8.15":
        return phase8_sleep.dispatch(root, action, step, **kwargs)
    if step == "P8.16":
        return phase8_which.dispatch(root, action, step, **kwargs)
    if step == "P8.17":
        return phase8_env.dispatch(root, action, step, **kwargs)
    if step == "P8.18":
        return phase8_hexdump.dispatch(root, action, step, **kwargs)
    if step == "P8.19":
        return phase8_ps.dispatch(root, action, step, **kwargs)
    if step == "P8.20":
        return phase8_mem.dispatch(root, action, step, **kwargs)
    if step == "P8.21":
        return phase8_udg.dispatch(root, action, step, **kwargs)
    if step == "P8.22":
        return phase8_gfxdemo.dispatch(root, action, step, **kwargs)
    if step == "P8.23":
        return phase8_stty.dispatch(root, action, step, **kwargs)
    if step == "P8.24":
        return phase8_date.dispatch(root, action, step, **kwargs)
    if step == "P8.25":
        return phase8_cron.dispatch(root, action, step, **kwargs)
    if step == "P8.26":
    ...
    if step == "P8.29":
        return phase8_uptime.dispatch(root, action, step, **kwargs)
    if step == "P8.30":
        return phase8_whoami.dispatch(root, action, step, **kwargs)
    if step == "P8.31":
        return phase8_uname.dispatch(root, action, step, **kwargs)
    if step == "P8.32":
        return phase8_fortune.dispatch(root, action, step, **kwargs)
    if step == "P8.33":
        return phase8_banner.dispatch(root, action, step, **kwargs)
    if step == "P8.34":
        return phase8_rev.dispatch(root, action, step, **kwargs)
    if step == "P8.35":
        return phase8_yes.dispatch(root, action, step, **kwargs)
    if step == "P8.36":
        return phase8_demo.dispatch(root, action, step, **kwargs)
    if step == "P8.37":
        return phase8_echo_regression.dispatch(root, action, step, **kwargs)
    if step == "P8.38":
        return phase8_pipe_matrix.dispatch(root, action, step, **kwargs)
    if step == "P8.39":
        return phase8_cron_transaction.dispatch(root, action, step, **kwargs)
    if step == "P8.40":
    ...
    if step == "P9.03":
        return phase9_vi_gap.dispatch(root, action, step, **kwargs)
    if step == "P9.04":
        return phase9_vi_unnamed.dispatch(root, action, step, **kwargs)
    if step == "P9.05":
        return phase9_vi_modes.dispatch(root, action, step, **kwargs)
    if step == "P9.06":
        return phase9_vi_motion_basic.dispatch(root, action, step, **kwargs)
    if step == "P9.07":
        return phase9_vi_motion_word.dispatch(root, action, step, **kwargs)
    if step == "P9.08":
        return phase9_vi_motion_gg.dispatch(root, action, step, **kwargs)
    if step == "P9.09":
        return phase9_vi_insert.dispatch(root, action, step, **kwargs)
    if step == "P9.10":
        return phase9_vi_open.dispatch(root, action, step, **kwargs)
    if step == "P9.11":
    ...
    if step == "P9.14":
        return phase9_vi_undo.dispatch(root, action, step, **kwargs)
    if step == "P9.15":
        return phase9_vi_search.dispatch(root, action, step, **kwargs)
    if step == "P9.16":
        return phase9_vi_ex_read.dispatch(root, action, step, **kwargs)
    if step == "P9.17":
        return phase9_vi_write_tx.dispatch(root, action, step, **kwargs)
    if step == "P9.18":
        return phase9_vi_write_state.dispatch(root, action, step, **kwargs)
    if step == "P9.19":
        return phase9_vi_quit.dispatch(root, action, step, **kwargs)
    if step == "P9.20":
        return phase9_crontab_vi.dispatch(root, action, step, **kwargs)
    if step == "P9.21":
        return phase9_vi_set.dispatch(root, action, step, **kwargs)
    if step == "P9.22":
    ...
    if step == "P10.02":
        return phase10_obj1_symbol.dispatch(root, action, step, **kwargs)
    if step == "P10.03":
        return phase10_obj1_reloc.dispatch(root, action, step, **kwargs)
    if step == "P10.04":
        return phase10_obj1_inspector.dispatch(root, action, step, **kwargs)
    if step == "P10.05":
        return phase10_as_parser.dispatch(root, action, step, **kwargs)
    if step == "P10.06":
...
def main() -> int:
    ...
    try:
        ...
        if args.step == "P8.40" and args.action == "test":
            ...
            if not build_path.is_file():
                raise DriverError("P8.40 result requires matching build evidence")
            build_record = json.loads(build_path.read_text(encoding="utf-8"))
            test_record = json.loads(evidence_path.read_text(encoding="utf-8"))
            for record in (build_record, test_record):
                if record.get("status") != "PASS" or record.get("source_commit") != source_state.source_commit:
                    raise DriverError("P8.40 source-candidate mismatch")
                if record.get("architecture_sha256") != source_state.architecture_sha256:
                    raise DriverError("P8.40 architecture identity mismatch")
                if record.get("implementation_plan_sha256") != source_state.implementation_plan_sha256:
                    raise DriverError("P8.40 plan identity mismatch")
            result = {
                "schema": 2, "step": "P8.40", "action": "result", "status": "PASS",
                "pass_marker": phase8_acceptance.PASS_MARKER,
                "source_commit": source_state.source_commit,
                "toolchain_lock_sha256": source_state.toolchain_lock_sha256,
                "architecture_sha256": source_state.architecture_sha256,
                "implementation_plan_sha256": source_state.implementation_plan_sha256,
                ...
                "commands": test_record["commands"], "hashes": test_record["hashes"],
                "assertions": test_record["assertions"] + [
                    {"name":"same-clean-phase8-acceptance-source-candidate-all-records","passed":True}
                ],
            }
            result_path = evidence_dir / "P8.40.result.json"
            result_path.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8", newline="\n")
            print(phase8_acceptance.PASS_MARKER)
            print(f"result={result_path}")
        ...
        if args.step == "P6.30" and args.action == "test":
            ...
            if not build_path.is_file():
                raise DriverError("P6.30 result requires matching build evidence")
            build_record = json.loads(build_path.read_text(encoding="utf-8"))
            test_record = json.loads(evidence_path.read_text(encoding="utf-8"))
            for record in (build_record, test_record):
                if record.get("status") != "PASS" or record.get("source_commit") != source_state.source_commit:
                    raise DriverError("P6.30 source-candidate mismatch")
                if record.get("architecture_sha256") != source_state.architecture_sha256:
                    raise DriverError("P6.30 architecture identity mismatch")
                if record.get("implementation_plan_sha256") != source_state.implementation_plan_sha256:
                    raise DriverError("P6.30 plan identity mismatch")
            result = {
                "schema": 2, "step": "P6.30", "action": "result", "status": "PASS",
                "pass_marker": phase6_acceptance.PASS_MARKER,
                "source_commit": source_state.source_commit,
                "toolchain_lock_sha256": source_state.toolchain_lock_sha256,
                "architecture_sha256": source_state.architecture_sha256,
                "implementation_plan_sha256": source_state.implementation_plan_sha256,
                "worktree_clean": True, "prerequisites": {"P6.29": "PASS"},
                "commands": test_record["commands"], "hashes": test_record["hashes"],
                "assertions": test_record["assertions"] + [
                    {"name":"same-clean-phase6-acceptance-source-candidate-all-records","passed":True}
                ],
            }
            result_path = evidence_dir / "P6.30.result.json"
            result_path.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8", newline="\n")
            print(phase6_acceptance.PASS_MARKER)
            print(f"result={result_path}")
