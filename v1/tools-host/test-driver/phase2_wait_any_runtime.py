...
class Phase2WaitAnyRuntimeError(DriverError):
    """Raised when the P2.16 target runtime fixture regresses."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise Phase2WaitAnyRuntimeError(message)
...
def _symbols(path: Path, names: tuple[str, ...]) -> dict[str, int]:
    ...
    for name in names:
        ...
        require(match is not None, f"P2.16 runtime symbol missing: {name}")
...
def _assemble_fixture(
    ...
    source = build / "p216-wait-any.asm"
    binary = build / "p216-wait-any.bin"
    symbols = build / "p216-wait-any.sym"
    source.write_text(
        "    DEVICE ZXSPECTRUM48\n"
        "    DEFINE ZX48_P2_15_WAIT_ENABLED\n"
        "    DEFINE ZX48_P2_16_WAIT_ANY_ENABLED\n"
        ...
        "p216_process_ptr_loop:\n"
        "    add ix,de\n"
        "    djnz p216_process_ptr_loop\n"
        "    xor a\n"
        "    ret\n"
        "zx48_process_lookup:\n"
        ...
        "    ret\n"
        "\n"
        "; Unused syscall surfaces are fail-closed stubs; WAIT itself is real.\n"
        ...
        "zx48_panic: ld (p216_panic_code),a : jp $B001\n"
        "\n"
        ...
        "p216_end:\n"
        "    ASSERT p216_end <= KERNEL_STACK_START\n"
        "    SAVEBIN \"p216-wait-any.bin\",p216_start,p216_end-p216_start\n",
        encoding="utf-8",
        newline="\n",
    )
    result = runner(
        [assembler, "--nologo", "--sym=p216-wait-any.sym", "p216-wait-any.asm"],
        cwd=build,
        timeout_seconds=30.0,
    )
    require(
        not result.timed_out and result.exit_code == 0,
        f"P2.16 runtime fixture assembly failed: {result.stderr or result.stdout}",
    )
    require(binary.is_file() and 0 < binary.stat().st_size < 0x3B00, "P2.16 runtime fixture missing or too large")
    require(symbols.is_file(), "P2.16 runtime fixture symbols missing")
...
def _write_wait_any() -> bytes:
    code = bytearray()
    code += _set_word(WAIT1, 0xFFFF)
...
def _two_zombies_lowest_pid_first(root: Path, symbols: dict[str, int], fixture: bytes) -> None:
    table = symbols["process_table"]
    ...
    code += _set_byte(p2 + PROC_EXIT_STATUS, 0x22)
    code += _link_child(symbols, 3, symbols["PROC_ZOMBIE"])
    code += _set_byte(p3 + PROC_EXIT_STATUS, 0x33)

    code += _write_wait_any()
    code += _ld_hl(WAIT1)
    code += bytes((0x3E, symbols["SYS_WAIT"] & 0xFF))
    code += _call(symbols["zx48_syscall"]) + _jp_c(FAIL_PC)
    code += _expect_hl(2)
    code += _expect_byte(STATUS_BASE, 0xA5)
    code += _expect_byte(STATUS_PTR, 0x22)
    ...
    code += _expect_byte(child_mask + 1, 0x08)
    ...
    code += _expect_byte(symbols["p216_panic_code"], 0)
    code += _jp(PASS_PC)
    run_sna(root, bytes(code), patch=_fixture_patch(fixture))


def _block_wake_any_child(root: Path, symbols: dict[str, int], fixture: bytes) -> None:
    table = symbols["process_table"]
    p1 = table + PROC_DESC_SIZE
    p3 = table + 3 * PROC_DESC_SIZE
    wait_any = symbols["process_wait_any"]
    wait_status_ptr = symbols["process_wait_status_ptr"]

    frame = b"\x00" * 10 + _word(CHILD_PC)
    child = bytearray()
    child += _expect_byte(symbols["current_pid"], 3)
    child += _expect_byte(p1 + PROC_STATE, symbols["PROC_WAIT_CHILD"])
    child += _expect_byte(wait_any + 1, 1)
    child += _expect_word(wait_status_ptr + 2, STATUS_PTR)
    child += _set_byte(p3 + PROC_EXIT_STATUS, 0x5A)
    child += _set_byte(p3 + PROC_STATE, symbols["PROC_ZOMBIE"])
    child += _set_byte(symbols["process_zombie_pid"], 3)
    ...
    code += _set_word(p3 + PROC_SAVED_SP, CHILD_FRAME)
    code += _write_wait_any()
    code += _ld_hl(WAIT1)
    code += bytes((0x3E, symbols["SYS_WAIT"] & 0xFF))
    code += _call(symbols["zx48_syscall"]) + _jp_c(FAIL_PC)
    ...
    code += _expect_byte(STATUS_PTR + 1, 0x7E)
    code += _expect_byte(p3 + PROC_STATE, symbols["PROC_FREE"])
    code += _expect_byte(wait_any + 1, 0)
...
def _no_child_echild(root: Path, symbols: dict[str, int], fixture: bytes) -> None:
    table = symbols["process_table"]
    p1 = table + PROC_DESC_SIZE
    wait_any = symbols["process_wait_any"]
    wait_status_ptr = symbols["process_wait_status_ptr"]
...
def _stale_parent_generation_is_not_a_child(root: Path, symbols: dict[str, int], fixture: bytes) -> None:
    table = symbols["process_table"]
    p2 = table + 2 * PROC_DESC_SIZE
    ...
    code += _parent_init(symbols)
    code += _link_child(symbols, 2, symbols["PROC_ZOMBIE"])
    code += _set_byte(p2 + PROC_EXIT_STATUS, 0x44)
    code += _set_word(parent_generation + 4, 2)
    code += _write_wait_any()
    code += _ld_hl(WAIT1)
    code += bytes((0x3E, symbols["SYS_WAIT"] & 0xFF))
    code += _call(symbols["zx48_syscall"]) + _jp_nc(FAIL_PC)
...
def run(
    ...
    *,
    ...
    assertions: list[dict[str, object]] = [
        {"name": "p216-runtime-fixture-assembles-real-wait-any-scheduler-wake-composition", "passed": True},
        {"name": "p216-runtime-fixture-fits-below-kernel-stack", "passed": len(fixture) < 0x3B00, "bytes": len(fixture)},
    ]
    if execute:
        _two_zombies_lowest_pid_first(root, symbols, fixture)
        assertions.append({"name": "wait-any-reaps-lowest-pid-zombie-first-and-exactly-one-per-call", "passed": True})
        _block_wake_any_child(root, symbols, fixture)
        assertions.append({"name": "wait-any-blocks-wakes-on-any-generation-qualified-child-and-resumes", "passed": True})
        _no_child_echild(root, symbols, fixture)
        assertions.append({"name": "wait-any-with-no-child-returns-echild-without-block-or-status-write", "passed": True})
        _stale_parent_generation_is_not_a_child(root, symbols, fixture)
        assertions.append({"name": "wait-any-rejects-stale-parent-generation-edge", "passed": True})
...
def main() -> int:
    ...
    try:
        ...
        if failed:
            raise Phase2WaitAnyRuntimeError(f"P2.16 runtime assertions failed: {failed}")
        print(f"P2.16 WAIT ANY RUNTIME PASS fixture={binary} command_exit={command.exit_code}")
        for item in assertions:
            print(f"PASS {item['name']}")
        return 0
    except (DriverError, OSError, ValueError) as exc:
        print(f"P2.16 WAIT ANY RUNTIME FAIL: {exc}")
        return 1
