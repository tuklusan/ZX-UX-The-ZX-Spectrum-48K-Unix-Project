...
def _target(root: Path, s: dict[str, int], module: bytes) -> None:
    ...
    base_remove = phase1._ld_hl(addresses["/tmp/gone"]) + phase1._call(s["zx48_p413_sys_remove"]) + phase1._jp_c(FAIL_PC)
    execute("closed-remove-return", base_remove, patch("/tmp/gone", records=closed, seed_zx=True))
    execute("closed-remove-slot-clear", base_remove + mem_eq(table, bytes(20)), patch("/tmp/gone", records=closed, seed_zx=True))
    execute("closed-remove-free-count", base_remove + _ld_a_mem(s["p413_test_free_count"]) + b"\xFE\x01" + phase1._jp_nz(FAIL_PC), patch("/tmp/gone", records=closed, seed_zx=True))
    execute("closed-remove-free-ptr", base_remove + word_eq(s["p413_test_free_ptr"], PAYLOAD_BASE), patch("/tmp/gone", records=closed, seed_zx=True))
    execute("closed-remove-free-len", base_remove + word_eq(s["p413_test_free_len"], 6), patch("/tmp/gone", records=closed, seed_zx=True))
    zx_checks = word_eq(s["zx_object_ptr"], 0) + word_eq(s["zx_new_ptr"], 0) + word_eq(s["zx_encoded_len"], 0)
    zx_checks += _ld_a_mem(s["zx_slot"]) + b"\xB7" + phase1._jp_nz(FAIL_PC)
    execute("closed-remove-zxpack-clear", base_remove + zx_checks, patch("/tmp/gone", records=closed, seed_zx=True))
