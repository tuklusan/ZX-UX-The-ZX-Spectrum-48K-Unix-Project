...
def dispatch(root: Path, action: str, step: str, *, sha256_file: Callable[[Path], str], run_command: Callable[..., Any], require_project_tool: Callable[[Path, str | Path], Path]):
    ...
    if action=="test":
        ...
        phase2_spawn._run_preflight_case(root,syms,fixture,proc=valid)
        phase2_spawn._run_preflight_case(root,syms,fixture,proc=phase2_spawn._proc1(flags=2),expected_error=syms["E_INVAL"])
        assertions += [
