...
""",encoding="utf-8",newline="\n")
    ...
    if action=="test":
        ...
        code+=expect(OUT+len(list_expected),0xA5)+expect(STATUS,0)+phase1._jp(PASS_PC); run_sna(root,bytes(code),patch=patch(ub,gb,args,0),timeout=30)
        for mode,loads,status,spawned in ((0,0,0,1),(1,1,0,1),(2,0,sy["E_FORMAT"]&255,0)):
            args=[b"demo",b"ship"]; ab=arg1(args); code=bytearray(b"\xF3"+phase1._ld_sp(0xBFC0)+phase1._ld_hl(ARG)+b"\x01"+word(len(ab))+b"\x11"+word(ENV)+phase1._call(sy["demo_entry"])+expect(STATUS,status)+expect(LOADS,loads)+expect(SPAWNED,spawned)+phase1._jp(PASS_PC)); run_sna(root,bytes(code),patch=patch(ub,gb,args,mode),timeout=30)
        args=[b"demo",b"SHIP"]; ab=arg1(args); code=bytearray(b"\xF3"+phase1._ld_sp(0xBFC0)+phase1._ld_hl(ARG)+b"\x01"+word(len(ab))+b"\x11"+word(ENV)+phase1._call(sy["demo_entry"])+expect(STATUS,sy["E_NOENT"]&255)+expect(SPAWNED,0)+phase1._jp(PASS_PC)); run_sna(root,bytes(code),patch=patch(ub,gb,args,0),timeout=30)
