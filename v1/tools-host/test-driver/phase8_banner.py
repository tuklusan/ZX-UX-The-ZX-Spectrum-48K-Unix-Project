...
""",encoding="utf-8",newline="\n")
    ...
    if action=="test":
        ...
        code+=expect(STATUS,0)+b"\x3A"+word(COUNT)+b"\xB7"+phase1._jp_z(FAIL_PC)+b"\x3A"+word(MAXX)+bytes((0xFE,8))+jp_nc(FAIL_PC)+b"\x3A"+word(MAXY)+bytes((0xFE,16))+jp_nc(FAIL_PC)+phase1._jp(PASS_PC)
