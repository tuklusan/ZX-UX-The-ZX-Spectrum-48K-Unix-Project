...
; P6.12 post-expansion argument/pipeline bounds. These helpers are invoked
; immediately before any redirection or spawn side effect.
    MACRO EMIT_P612_BOUND_ROUTINES
P612_ARG_MAX             EQU 16
P612_PIPE_MAX            EQU 6

; B=argc after expansion. Accept 1..16.
sh_p612_validate_argc:
    ld a,b
    or a
    jr z,sh_p612_invalid
    cp P612_ARG_MAX+1
...
; B=syntactic pipeline stage count after parsing. Accept 1..6.
sh_p612_validate_pipeline:
    ld a,b
    or a
    jr z,sh_p612_invalid
    cp P612_PIPE_MAX+1
...
; B=argc, C=stage count. Combined pre-side-effect gate.
sh_p612_validate_command:
    push bc
    call sh_p612_validate_argc
    pop bc
    ret c
    ld b,c
    jp sh_p612_validate_pipeline
